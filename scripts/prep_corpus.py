#!/usr/bin/env python3
"""
prep_corpus.py — read a corpus, draft its arm, refuse to guess the rest.
...
def main() -> int:
    ...
    try:
        ...
        # REFUSE, do not continue. The predicted lane is the one section that
        # can stop a card being booked, and a tool that silently omits its own
        # headline check when a file is missing is the exact failure it exists
        # to prevent. LINNAEUS printed a one-line note and a full profile, and
        # the profile read as if the arm had been assessed.
        sys.exit(f"\n  REFUSING: no vocabulary — {exc}\n\n  The predicted lane "
                 f"needs it, and without that number this tool has not assessed "
                 f"anything. Build or fetch the index, then re-run.\n")
    p = profile(docs, reg)
    ...
    if "lane_ceiling" in p and p["lane_ceiling"] < 0.15:
