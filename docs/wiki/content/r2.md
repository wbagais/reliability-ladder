...
`ladder/rungs/r2.py` · owner A · **zero model calls**
...
- `sweep()` produces the risk-coverage curve. It takes the correctness oracle as an argument rather than importing a scorer, so the one shared scorer stays owner B's file and there is still only one of it.
