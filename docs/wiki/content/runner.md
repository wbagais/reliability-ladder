...
`ladder/run.py`. **A rung is registered by adding `ladder/rungs/rN.py`, never by editing this file.**
...
- `--predictions out/r0.jsonl` — rung-0 output. Enforces **split discipline**: a file carrying a document outside the split is refused, exit 1. Blank `record_id`s are backfilled.
