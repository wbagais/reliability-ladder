# Rung 5 · abstention {{done|BUILT}}

`ladder/rungs/r5.py` · **zero model calls**
...
- Rung 5 is where a [[r1]] verdict is **finally allowed to cost coverage**. Rung 1 in observe mode defers that cost; rung 5 pays it.
...
| `rejected` | rung 1 called it provably wrong and [[r2]] never rescued it |
...
- `sweep()` produces the risk-coverage curve. It takes the correctness oracle as an argument rather than importing a scorer, so the one shared scorer stays a single file and there is still only one of it.
...
Run alone, rung 5 abstains **nothing** — it reads `r1_verdict`, and with no rung 1 upstream there is nothing to withdraw on:
...
- [[r1]] · [[r6]] · [[record]] · [[measurement]] · [[glossary]]
### Dev split, reactions only

169 rung 0 mode A records: **169 ABSTAIN, 0 kept a code.** Rung 0 produced
0/105 correct codes, so the correct abstention rate was 100%.

226 single-code gold reaction mentions: **76 VERIFIED / 150 ABSTAIN**, no
crossover in either direction. Coverage 1.000 → 0.336.

That 33.6% against the 43.5% above is the drug/reaction split: this arm is
reactions only, the test-split figure is pooled. Independent reproduction of
the finding in [[r1]].

**Reason vocabulary diverges from the spec.** The model arm reported
`code_unknown 155, schema_invalid 8, wrong_semantic_type 3, unresolved 3` —
rung 1 reasons passing through, not the three above. Either `reason = r1_reason`
on the REJECT path is deliberate and the table needs a fourth row, or code and
spec disagree. **A to confirm.**
