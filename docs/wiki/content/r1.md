...
```mermaid
flowchart TD
    subgraph DISK["on disk · licensed · gitignored"]
        CAD["cadec/<br/>text · original · sct · meddra"]
        RF2["SnomedCT_Release_.../<br/>Snapshot/Terminology/*.txt"]
        CSV["meddra_codes.csv"]
    end

    CAD --> LC["corpus.load_corpus()"]
    LC --> SRC["sources<br/>{doc_id: text}"]
    LC --> GM["GoldMention"]
    GM -->|"--source gold"| G2R["gold_to_record()"] --> REC
    R0J["out/r0.jsonl"] -->|"--predictions"| RP["read_predictions()"] --> REC
    R0["rungs/r0.py<br/>unwritten"] -->|"sees sources only"| REC["Record"]

    RF2 -->|"registry --build<br/>once"| DB[("snomed.sqlite<br/>concept · description")]
    DB --> REG["Registry"]
    CSV --> MT["MeddraTable"]
...
| Node | Full path |
|---|---|
| `cadec/` | `data/cadec/data/cadec/{text,original,sct,meddra}/<doc>.{txt,ann}` |
...
| `meddra_codes.csv` | `data/meddra_codes.csv` — 666 rows, `meddra_code,meddra_term,...` |
...
- **`original/`** supplies `entity_type` and the span offsets. Same caveat.
