...
flowchart TD
    ...
    AB["rung0_ab.py + stub_llm.py<br/>real model, Records in memory"] -.->|"no serialiser"| R0J
...
- `ladder/rung0_ab.py` **does** drive a real model through `ladder/stub_llm.py` and build `list[Record]` — see [[r0]] for the measured A/B — but `run()` keeps them in memory and `compare()` prints a table. Nothing serialises them.
- So there is a working model→Record path and a working Record→ladder path, and **nothing joins them**. Writing `ladder/rungs/r0.py`, or a wrapper that dumps `rung0_ab.run()` output with `schema.dumps()`, closes the gap.
