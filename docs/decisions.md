...
- 2026-09-07 — **THE EIGHT ARM MANIFESTS NOW CARRY THEIR BASE'S `vocabulary.gate`, VERBATIM, AND NO RUN MOVES.** Commit `4d5a21c` put `vocabulary.gate` on `manifest.json` and `manifest.finer.json` and not on their arms, and the eight one-key-diff tests (`test_lexarm`, `test_menu_order`, `test_r4_menu` ×4, `test_rung0_encoder`, `test_rungs_audit`) went red. Decided for the arms carrying it, not for the tests excusing it: the gate is a fact about the VOCABULARY — which code it must hold and which it must not — and an arm shares its base's vocabulary by definition, so nothing about an arm can change the answer. The alternative was live, too: `checks.cross.vocabulary_gate` FAILs a manifest with no gate declared, and `cmd_init` falls back to CADEC's SNOMED constants, so a FiNER arm without one was gated on codes its tag set cannot hold — the same inherited-constant shape the 2026-09-01 entry found in `cmd_init` itself. Appended at the end of each `vocabulary` block (append-only), `_note` included. **No re-run:** the key is read by `cmd_init` and the crosscheck only — never by a prompt, a retriever, the LLM cache key (which hashes the call payload, not the manifest) or the scorer — so the archived arm runs stand, and the manifest copy saved beside each differs from the tracked one by this key alone. `tests/test_arm_gate.py` pins the reason beside the diff tests that pin the fact. Five more manifests still declare no gate (`finer.llama`, `finer.mistral`, `finer.typecheck`, `spine.cadec`, `spine.finer`); none is pinned as a one-key diff, so they are out of this change's scope and the crosscheck will say so if they are run.
