...
- 2026-08-26 — **Dashboard M1 built (`dashboard/`, `python -m dashboard`): the read-only Workbench — tabs R1/R3/R4/R5, visuals V1–V6+V10, no launcher, no writes.** Thin FastAPI backend importing `ladder.*` (scores ONLY via `score_run`/`bootstrap_ci`; records via `read_predictions`; ledger via `Ledger.read` + `Ledger.cost_by_rung` — one accounting path; gold via `load_corpus`; exclusions via `load_exclusions`; V4 via `ladder.calibrate.run`), serving a vanilla-JS SPA. Every route is GET and no dashboard module can spawn a process (both TESTED — C2); server binds loopback only, refusing any other host (tested — C1); every number-carrying payload carries run id · split · span mode · backend · manifest-copy hash (tested — C3); caveat texts live in one module (`dashboard/caveats.py`) and attach from the run's own ledger facts, so a disabled rung 3 gets no samples caveat (tested — C4); cost renders as three separate panels with usd alongside and nothing fused (tested — C5). Golden test pins the app's rendering of phaseF-test-1 to the recorded values (F1 exact 0.204 [0.150–0.260], overlap 0.215, outcomes 60/0/91/2/0, reviews_per_100 77.07, flow 72 shipped / 242 routed / 16 unlocatable / 45 withheld-correct); 44 new tests, suite 636 green. Tab bar carries the M2–M4 tabs as labeled placeholders so later milestones land without rework.
- 2026-08-26 — **Dashboard: a run's SPLIT is inferred, never assumed — and the scoring denominator is the split's full document list.** The saved `<run>.manifest.json` does not record the split, so the app matches the run's own doc_ids against the frozen `data/splits/*.json` (subset ⇒ that split; otherwise "unknown", displayed as such). Golds for scoring come from the split's FULL doc list, not the documents that produced records — first cut used record-touched docs and scored phaseF-test-1 at 0.205, because a document the extractor answered nothing for had quietly left the denominator with its false negatives; the golden test caught it.
- 2026-08-26 — **Dashboard exports (C1): scrub is structural + checked, and MODEL free text counts as unsafe.** All downloads live under `/api/export/`; payloads pass `scrub_payload` (drops every text-bearing key recursively) then `assert_clean` against the run's actual source texts. Scrubbed beyond corpus quotes: `sct_label` and judge `why` — model output that routinely echoes the post verbatim; retrieval candidates' `label`/`fsn` stay, being SNOMED's own words (the desk-file rule). Prompt/reply views are local-only and 404 under `/api/export/`. Viewing corpus text in the loopback browser is not an export.
- 2026-08-26 — **Dashboard R5: prompts are RECONSTRUCTED through the rungs' own builders, because `.llm_cache` stores replies keyed by request hash and never retains the prompt.** The drill-down rebuilds rung 0's extract call (`r0._extraction_prompt`, with the pool few-shot block re-rendered via `r0.pool_fewshot_block` — it is rendered from the licensed corpus at runtime and can never sit in the manifest), rung 0's pick call (`r0._blocks` over the records' recorded `checks.candidates`), and rung 4's judge call (`r4.PROMPT` with the withheld answer restored), then looks the hash up — verified against phaseF-test-1's real cache, all three calls retained. Any divergence renders "not retained", never an empty reply. Rung 2 (fired 0 times on every measured run) and rung 3's k samples are not reconstructed in M1.
- 2026-08-26 — **Dashboard M1 scope choices, logged:** V4 zone strip replays gold through rung 1 on the local-rf2 backend only (OLS4 is a network backend; the strip's caveat and provenance name the backend); the archived baselines are found through the worktree's `.git` file (`gitdir: <main>/.git/worktrees/…` ⇒ `<main>/out/archive`), so no absolute path is baked into the repo; Phase E's `phaseE-r6-*.r6.ledger.jsonl` ablate stems list as their own run entries — the app groups files exactly as they sit on disk rather than inventing a merge rule; test documents remain reachable in the explorer behind the explicit "spent split" selection with a banner (spec R1's stated criterion; the stricter results-only reveal stays open).
- 2026-08-26 — **Dashboard R1 (user feedback): document annotations now show their gold code(s) WITH vocabulary labels.** Each mention in the document detail carries one entry per gold code (post-coordinated and disjunction mentions list all of them, with the kind stated) labelled through `ladder.registry.Registry.label` — the desk's display rule: never a bare SCTID alone when the registry can label it, and when it cannot (code absent from the release, or no index on the machine) the view SAYS so instead of showing nothing. CONCEPT_LESS renders as an explicit state, never an empty cell. TDD'd (4 tests incl. the registry-absent degradation); exports unchanged — codes and vocabulary labels were already the allowed fields.
- 2026-08-26 — **Dashboard R3 (user feedback): the rungs' dependence on each other is now shown, computed from each run's own artifacts.** New `/api/run/dependencies` + a "how the rungs connect" diagram at the top of Results: structure (which rung feeds which, what each denominator means) lives as data in `dashboard/dependencies.py` like the caveats; every count, mode and state comes from the run — rung 1's mode from its own ledger rows (observe = judges, records pass untouched; gate = ROUTES, rendered differently and carrying a confounds-rungs-3–6 caveat), rung 2's trigger annotated with the run's actual counts (phaseF: 16 REJECT, 0 correctable under r2's correctable set, 0 attempts), verdict edges into rung 5 drawn as signals distinct from the records path, rung 6's queue sized from the ledger (242). The per-rung table now carries the stack semantics as a caption — each row is a CUMULATIVE state ("would this ship if the run stopped here"), per-rung attribution is `ablate` — and an ablate run is DETECTED from its ledger (rungs present ≠ full order; the Phase E r6 ledgers read `run_kind: ablate`, queue 208) and labeled instead. Each table row also names its denominator's source rung (`r6_queue ← r5`, `r2_offered ← r1`) per the ledger-named-denominator rule. TDD'd (5 tests + golden pins on phaseF-test-1).
- 2026-08-26 — **Dashboard R3 redesign (user feedback): the Results tab is now a narrative, and "how the rungs connect" shows what happens to EACH rung-1 bucket.** Six sections in story order — what shipped (headline + a plain-language reading composed from the run's numbers), how it got there, the three result layers, the records' journey, what it cost, what a person still has to do — with caveats attached beside the numbers they qualify instead of a banner wall at the top. The rung explainer is folded into the dependency cards as a per-rung MEANING sentence (what the rung can and cannot do — rung 1 proves wrong never right, rung 5 refuses rather than answers; data in `dashboard/dependencies.py`, TDD'd) plus a "how to read this page" intro — no separate tab. The three layers render in the user's terms: correct annotation = detection, code extraction = coding (both from `score_run`), vocabulary label = rung 1's `label_check` counts read from the recorded `checks.label_verified` (new in the score payload, TDD'd; phaseF: an absent check is `unchecked`, never agreement). Exact-vs-overlap is explained inline where first used with the repo's canonical example (extreme rectal bleed / rectal bleed), and V3 states the computed reading (detection 0.521 exact vs 0.808 overlap = boundary disagreement, not missed mentions).
- 2026-08-26 — **Dashboard: bucket-level verdict flow, computed as a crosstab from the run's records — and V2 kept as its disposition-stage sibling.** New `dependencies.verdict_flow`: r1_verdict × downstream fate (settled/abstained/queued, r3 changed, r4 pass/fail per bucket), never assumed — golden-pinned on phaseF-test-1: ACCEPT 72 all settled (r4 pass 51 / fail 21 — rung 5 shipped them regardless of rung 4, which is exactly the interaction the lanes make visible), BAND 226 and REJECT 16 all abstained and queued, REJECT's lane carrying "16 offered, 0 correctable, 0 attempted". Rendered as proportional-width lanes in the zone tokens flowing UNBROKEN through rungs 2–4 (observe mode: verdicts are signals; annotations per lane) and branching only at r5 into shipped/queue sinks; in gate mode (fixture-tested) the REJECT lane visibly leaves the stack at rung 1. DECIDED: V2 stays as the sibling view (disposition stages + correctness vs gold, now with every stage column titled with the rung(s) that produce it) with a one-line pointer each way — merging them would fuse verdict lanes with gold outcomes into one unreadable diagram. Cost view per feedback: routed-to-a-person renders as the single rung-6 fact it is, and an all-zero usd table collapses to one line (usd still carried in exports — C5 unchanged, tests green).
- 2026-08-26 — **Dashboard: the "huge blank region when scrolling" report is the embedded preview pane's capture artifact, not a layout bug.** Reproduced at the reported ~724px viewport, then isolated: a trivial static page of 300 plain <p> lines shows the same full-viewport blank when captured past ~1000px of scroll in the pane, while the dashboard's own DOM audit shows zero layout gaps (content bounds continuous, 5,734 of 5,750px). Nothing to fix in the dashboard; noted here so the next session does not chase it again.
- 2026-08-27 — **Dashboard "How it got there" v3 (user feedback): ONE integrated dataflow diagram, with the possible paths drawn beside the actual ones.** The rung-cards row and the lane strip are merged into a single SVG: rungs as node columns (each folding in the old card's name, mode chip, model, one-line behaviour, with the full meaning + counts on hover), records as Sankey-style ribbons in the zone tokens — the r2 node sits ON the REJECT ribbon only, so ACCEPT/BAND visibly bypass it. New `dependencies.flow_map` (TDD'd, observe + gate fixtures, golden-pinned on phaseF-test-1) derives the STRUCTURALLY POSSIBLE path set per bucket and marks each leg actual/possible: untaken options render as thin dashed ribbons labeled 0 (BAND settling at r5, ACCEPT withdrawn at r5, r2's rescue rejoining as changed), a REJECT→shipped leg is OMITTED outright when `abstain_on_reject` makes it impossible (an impossibility is not an untaken option), and gate mode replaces REJECT's downstream path with an exit at rung 1 that observe runs never draw.
- 2026-08-27 — **Dashboard round-4 feedback: hierarchical batch drill-down ON the integrated diagram (Sankey kept over icicle — the bypass-r2 relationship and the dashed option paths have no icicle equivalent; logged as the choice), plus per-layer outcome composition.** Every ribbon is a clickable subset: selection dims the rest, opens the subset's own partition with counts AND % of batch and of subset at every split, and links read-only into Traceability filtered to those records — `/api/run/records` gained `verdict` and `disposition` filters (TDD'd; a ribbon is a (r1-verdict, disposition) subset by definition). The standalone outcome-composition section is gone: each result layer now carries a composition bar in ITS OWN vocabulary — detection matched/missed/spurious, coding the five outcomes in report order (never folded), label verified/flagged/unchecked — exact and overlap side by side, from a new `composition` block computed purely from score_run output + the recorded label check (TDD'd, golden-pinned: detection 153/137/144 on phaseF-test-1). "Routed to a person" renders with its share of the batch everywhere it appears (242 of 314 · 77.1%). V2's overlapping gray labels at narrow widths fixed with collision-avoided leader-line labels.
...
- 2026-08-27 — **FOUND: 17.3% of dev gold is DISCONTINUOUS and rung 0 cannot express it — 0 of 233 predictions carry more than one span.** `r0` builds `spans=[(start, end)]`, one tuple per mention, at both construction sites; gold carries 39 discontinuous mentions of 226 scorable on dev, and they account for 24 of the 68 boundary errors and 15 of the 55 false negatives. Splitting the exact-F1 headroom accordingly (draw 1, baseline 0.309): perfect boundaries on CONTIGUOUS gold only reaches **0.383**, perfect boundaries everywhere **0.423** — so ~4.0 points of exact F1 are behind a structural limit of the record shape, not behind prompt quality, and no trimmer can reach them (the trimmer cuts edges; this needs one coordinated phrase to become N discontinuous mentions). The rest of the picture, same draw: an oracle pick inside the EXISTING 20-candidate menu reaches 0.397 exact and **0.597 overlap** (from 0.423), so coding is the larger lever on overlap and boundaries the larger one on exact; both together 0.536 (contiguous) / 0.597 (all). Menu recall on overlap-matched spans is 137/165 = 83.0%, gold at rank 0 in 85 of those 137, and 40 records had the gold code in the menu and did not pick it.
- 2026-08-27 — **FOUND: rung 0's detection is stratified by CADEC entity type, and the non-ADR types are near-total misses.** Overlap detection recall by the type CADEC assigned, across three draws: ADR 82/80/85% (163–170 of 200), Symptom 62/62/75% (5–6 of 8), **Finding 20/20/20% (2 of 10), Disease 12/12/25% (1–2 of 8)**. `CADEC_TYPE_MAP` folds Disease, Symptom and Finding into REACTION, so 26 of the 226 scorable dev gold mentions (11.5%) are conditions rather than drug reactions, and the model finds about 9 of them. This is NOT a missing instruction — Phase B's arm 3 already put "Report EVERY symptom, condition or health problem the writer says they experienced — including the condition the drug was taken for, and conditions they merely compare themselves to" into the shared scope block, and the miss survives it. It is non-compliance under an opening line that says "adverse reaction", against posts where these appear as history, comorbidity or risk comparison. Ceiling if Disease/Finding reached the ADR rate: ~+13 detected mentions, overlap detection recall 0.756 → ~0.81. Not attempted: Phase B arm 4 measured that adding work to the FIND call costs spans elsewhere, so a scope patch is the exact shape of change that has already backfired once here, and there is no held-out split left to confirm a win on.
- 2026-08-27 — **Four rung-0 fixes implemented and measured incrementally, TDD-first (24 new tests, 613 green). Chunking the pick and filling from the menu WIN; the looser cut LOSES again.** All four are declared arms in `r0.DEFAULTS`, off or at their frozen value unless asked for, so Phase F's config is byte-unchanged and none of its numbers move. (1) `rung0_pick_batch: 7` splits the DECIDE call — `_decide` now chunks and `_decide_batch` is the old body, each chunk renumbering its reactions from 0 because menu position is the answer key for the call it appears in. NOT a retry (that would be rung 2): the same single pass, batched smaller, and the extra api_calls go to the ledger (dev 77 -> 94). (2) `rung0_pick_fallback: True` fills a record that would leave rung 0 uncoded from its own menu position 0, flagged `pick_fallback` as `"gap"` (`no_pick`/`bad_pick`/`pick_parse_failed` — the reply never answered) or `"decline"` (the model answered null), never over CONCEPT_LESS, which is a positive claim the scorer grades. (3) `rung0_split: True` + new `ladder/split.py`: a coordinated quote becomes the DISCONTINUOUS mentions gold keeps, in two structural shapes only (head-final "muscle and joint pain"; head-initial "pain in hips and legs"), run BEFORE retrieval so each half gets its own menu and its own pick — the halves carry different gold codes, and a split after the pick would copy one onto both. (4) `rung0_cut_rate` relaxes the trimmer's interior cut, made newly testable by `trim.CUT_NEVER`, which holds coordinators out of `cut_drop` unconditionally so a looser rate cannot truncate a phrase before the splitter sees it. Five existing rung-0 tests that pinned the post-decline `sct is None` state now run with `rung0_pick_fallback=False`: what they were pinning — a decline is recorded and never upgraded to CONCEPT_LESS — is unchanged, and the arm is what moved.
- 2026-08-27 — **The incremental measurement (dev, 40 docs, 226 gold; plus a random 10-doc subset, seed 20260827, 62 gold — reported because it was asked for and NOT because it can separate anything).** Cumulative exact / overlap F1, one draw each, cache-warm where the prompts were unchanged. DEV: baseline 0.309/0.423 -> +chunked pick **0.331/0.436** -> +menu fallback **0.340/0.449** -> +coordination split (see next entry) -> +looser cut (see next entry). Detection is byte-identical across (1) and (2) at 0.449/0.745 — both are coding-only fixes, exactly as designed — and the movement is all in coding accuracy, 0.689 -> 0.738 -> 0.757 exact. TEN: baseline 0.343/0.463 -> chunked pick 0.388/0.507 -> fallback 0.388/0.507 (NO further movement: on those ten documents the chunking removed every `no_pick` gap, so the fallback had nothing left to fill — the two fixes are not additive on a small sample, and reading either one's value off this subset would be wrong in both directions) -> +split 0.440/0.482 -> +looser cut 0.426/0.482. **The 10-document numbers move 4-10 points per arm against a 2-3 point run-to-run noise floor measured on 40; they are recorded, and no ordering conclusion should be drawn from them.**
- 2026-08-27 — **The coordination splitter is the largest single win, and the looser cut is UNDETERMINED — the 10-doc subset and dev disagree in sign.** Splitter, mechanically: on the ten it fired 12 times over 5 quotes and produced 11 discontinuous records where the run had 0 — "severe muscle pain in hips, legs and ankles" became three mentions coded 49218002 |pain of hip region| / 10601006 |pain in leg| / 1264030002 |pain in muscle of ankle joint|, which is the shape gold uses. On DEV it is the biggest step of the four: exact 0.340 -> 0.373, paired-bootstrap step delta **+0.0341 [+0.0018, +0.0691]**, the only individual step whose interval clears zero; detection 0.449 -> 0.486 exact and 0.745 -> 0.763 overlap, n_pred 233 -> 251 — it buys recall with precision, the trade the 4-point discontinuity ceiling predicted. THE CUT: `rung0_cut_rate: 0.055` LOST on the ten (0.440 -> 0.426; coding up 0.795 -> 0.811, detection down 0.553 -> 0.525) and GAINED on dev (0.373 -> 0.386), where its paired step delta is **+0.0120 [-0.0146, +0.0386]** — spanning zero. Two subsets of the same split, opposite signs, neither separable: it is NOT rejected and NOT accepted, and it stays at the frozen 0.02 default because an arm that cannot be shown to help does not get switched on. This is the clearest demonstration in the repo of why the 10-document result was worth refusing: read alone it would have retired a change dev says is the second-largest of the four. CORRECTION: an interim report in this session called the cut rejected on the ten before dev had finished; that call was premature and is withdrawn here.
- 2026-08-27 — **Cumulative dev result, all four arms on, one draw: exact F1 0.309 -> 0.386, overlap 0.423 -> 0.453; paired bootstrap over documents +0.0764 [+0.0354, +0.1156] exact.** Step deltas, each vs the arm below it: chunked pick +0.0216 [0.0000, +0.0364], menu fallback +0.0088 [0.0000, +0.0161], coordination split +0.0341 [+0.0018, +0.0691], looser cut +0.0120 [-0.0146, +0.0386]. Only the split's own interval clears zero; the TOTAL clears it comfortably, which is the defensible claim — the individual steps sit at or inside the 2-3 point run-to-run noise floor measured the same day, and this is ONE draw of each arm, not three. Detection is unmoved by the two pick fixes (0.449/0.745 throughout) and moves only with the splitter, which is what a coding-only fix and a detection fix respectively should look like. Cost: 77 -> 95 api_calls on dev (+23%), the price of batching the pick. NOT PROMOTED: `manifest.json` is byte-unchanged, every arm is off or at its frozen value by default, and Phase F's numbers stand exactly as reported. Confirming any of this would need the held-out split, which is spent — so this is characterised headroom, not a new result for the ladder.
- 2026-08-28 — **MEASURED (user ran it, their key, their consent): `anthropic/claude-sonnet-5` as the rung 0 extractor buys DETECTION and buys the exact headline almost nothing.** Ten dev documents (seed 20260827), 62 scorable gold, same three arms on (chunked pick, menu fallback, coordination split; cut at the frozen 0.02), 26 calls, 174 s, ~$0.20 at the registered price. Result vs the same arms on `ollama/gpt-oss:20b`: **F1 exact 0.440 -> 0.449, overlap 0.482 -> 0.536**; paired bootstrap over the ten documents, exact **+0.0095 [-0.0249, +0.0505]** (spans zero — NOT separable) and overlap **+0.0542 [+0.0220, +0.0900]** (clears it). The decomposition is the finding: overlap detection recall 55/62 -> **61/62 (98.4%)**, i.e. Sonnet finds essentially every gold mention — and of those 61 matched only **38 are exact-span**, against 37 of 55 for the local model. One more exact span. Exact-correct answers: **31 for both models, identical.** Coding accuracy on matched spans did not improve and drifted down (exact 0.795 -> 0.756, overlap 0.618 -> 0.607), which is what a shared retriever predicts: both models pick from the same 20-candidate menu, so a better reader cannot beat the menu's 83% recall. CONCLUSION, and it is the cleanest isolation of rung 0's wall the repo has: **the extractor was never the binding constraint on the exact headline.** Detection was the one layer a frontier model fixes and it fixed it outright; the exact number is held by span-boundary convention and by retrieval, neither of which is a property of the model doing the reading. Caveats stated: TEN documents (the noise floor is 2-3 points on 40), one draw, all 62 gold in this subset are CADEC type ADR so the Disease/Finding compliance gap measured 2026-08-27 is NOT exercised here, and the 174 s vs 5 s wall-clock is not a latency comparison (the local arm was served from cache). Manifest unchanged: `model.extractor` stays local and this remains a per-run arm, exactly as decided 2026-08-26.
- 2026-08-28 — **The 0.70 target, priced against the Sonnet run: reachable only by fixing RETRIEVAL, and not by any extractor.** Cumulative oracles over the Sonnet records (10 docs, exact F1 0.449): a perfect picker over the existing menu gives 0.536; + perfect span boundaries 0.681; + perfect code drawn from all of SNOMED rather than the menu 0.884. So with the best available reader already finding 98.4% of mentions, **a perfect pick and perfect boundaries still land at 0.681 — below 0.70** — and the step that clears it is the one that replaces the menu, worth +0.203 on its own. The same shape held on the local model at 40 documents (perfect pick + perfect boundaries = 0.600). Reading: the 17% of mentions whose gold code never enters the 20-candidate shortlist is the hard ceiling on this design, and a second reader cannot see past a list it was not given. Hybrid retrieval and k=40 were both measured and both lost (2026-08-24), so this needs a different retrieval idea, not a tuning pass. NOT PURSUED: Phase F is spent, no held-out split remains, and every number in this entry is dev-side headroom.
- 2026-08-28 — **The retrieval ceiling is a RANKING problem, not a coverage one — 100% of dev gold codes are in the searchable index.** Every one of the 216 coded dev gold mentions has its code among the 127,515 distinct codes in `data/keywords.csv`, so nothing about the answer key is out of reach of the dense retriever by construction. What fails is depth. Full-index rank of the gold code, queried with the span rung 0 actually produced, over 174 matched mentions: **rank 0 52.3%, top-20 81.0%, top-50 83.9%, top-200 91.4%, top-2000 98.3%**, never surfaced 1.7% (3 mentions, all of them fragments the FIND step emitted — a one-word span cannot retrieve a concept). So k=20 leaves 19% of matched mentions with no gold code on the menu, and k=200 would leave 8.6%. This is the vise the design is in: **menu RECALL wants a big k and the PICK degrades with one** — measured 2026-08-24, k=40 made picks worse (pick-past-gold 34 -> 47) and the k=20 setting stands on that, not on a recall table. The unbuilt component both measurements point at is a RERANK stage between retrieve and pick: retrieve deep, rerank to ~15, then pick. Nothing in rung 0 does this today.
- 2026-08-28 — **MEASURED: gold's own span-boundary convention is only ~67% deterministic, which caps exact F1 below the 0.70 target for any system.** Over the POOL split (the learnable side, 12,287 left-edge and 12,715 right-edge boundary decisions at tokens seen >=5 times): **66.9% of decisions fall at tokens gold treats >=95% one way, 20% "leaning" (80-95%), and 13% genuinely mixed (20-80%)** — on both edges, near-identically. The contested tokens are not exotic: "terrible" is kept inside a span 52% of the time, "very" 70%, "joint" 73%, "in" 40%. A perfect learner of this convention gets roughly 0.92 per boundary decision and a span needs TWO, so the best attainable exact-span rate is ~0.85; observed today is 0.62 (Sonnet) to 0.67 (local). Composing that with the other measured ceilings — detection recall 0.98 (Sonnet, measured), menu recall @200 0.91 (measured), a strong reranker at ~0.90 — gives **F1 ~0.68 with every component at its best**, which lands on the directly measured figure: a perfect reranker over the top 200 PLUS perfect spans scores **0.667 exact** on dev. CONCLUSION: **exact F1 > 0.70 is not reachable on this task as defined**, and the binding constraint is the answer key's own boundary noise, not the model, the prompt or the retriever. The reachable version of the target is OVERLAP F1 — Sonnet already reaches 0.884 detection on the ten, and a rerank stage lifting coding accuracy from 0.607 toward 0.85 would put overlap F1 in the 0.70-0.75 band. That is a different claim about a different question and must be reported as overlap, never as the headline exact number.
- 2026-08-28 — **A SYSTEM MESSAGE more than halves rung 0's run-to-run nondeterminism — the first thing measured that touches the noise floor itself.** Rung 0 has never used the system slot: `ladder/llm.py` builds `[{"role": "user", ...}]` and both prompts open with a bare imperative ("Extract every adverse reaction..."), with zero occurrences of a persona anywhere in r0.py. Adding one (harness-only, patched into `LLMClient.chat`) over three draws of the same ten documents: pairwise span-set Jaccard across draws goes from **0.515-0.584 (no system message) to 0.926-1.000 for a bare identity** ("You are a clinical terminology specialist") and 0.717-0.837 for a longer identity-plus-scope message. Two of the three bare-identity draws are BYTE-IDENTICAL. The mechanism is not established here — a fixed prefix on every request is the obvious candidate — but the effect is large, consistent, and directly relevant: the 2-6 point draw-to-draw spread documented 2026-08-27 is what makes every rung 0 arm in this repo hard to separate, and this is the first intervention that shrinks it rather than fighting it. Worth confirming on dev before it is believed.
- 2026-08-28 — **THREE independent prompt interventions all trade exact F1 for overlap F1 — the pattern, not any one arm, is the finding.** Ten documents, three draws each, matched baselines, all harness-only: (a) added rules for lab values and single-word mentions, (b) a bare role system message, (c) role plus task-scope framing. Means against a base of exact 0.409 / overlap 0.501 / detection-exact 0.544 / detection-overlap 0.805 / coding 0.623: (a) 0.373 / 0.512 / 0.526 / 0.842 / 0.608; (b) 0.353 / 0.525 / 0.471 / 0.809 / 0.648; (c) 0.379 / 0.549 / 0.515 / 0.825 / 0.665. **Every one improves coding accuracy and overlap detection and degrades EXACT detection.** The consistent reading is that anything which makes the model reason harder about the clinical task makes it quote more interpretively — longer, more paraphrased spans that still overlap gold but no longer match it character for character. Since the headline metric is exact, all three are REJECTED as shipped changes; on an overlap headline (c) would be the best arm measured, +4.8 points overlap and +4.2 coding, consistent in direction across all three draws. Nothing was added to production for any of them — the variants exist only in the scratch harness, so there is nothing to remove.
- 2026-08-28 — **A prompt arm was run whose hypothesis the sample could not test — recorded as a method error.** The `role_scope` system message was written to attack the largest miss class (Disease/Finding/Symptom gold, 26 of 226 on dev, 40% of unproposed gold there) by reframing the task rather than adding another rule the model already ignores. It was measured on the ten-document subset, which contains **zero non-ADR gold mentions** — so the arm's actual hypothesis was untestable and the numbers it produced say nothing about it either way. The composition of an evaluation subset must be checked against the hypothesis BEFORE the run, not after; the ten documents were drawn at random for a general comparison and were never appropriate for a class-specific one. The determinism and exact-vs-overlap findings from the same runs stand, because those do not depend on the missing class.
- 2026-08-28 — **Restating an instruction the model already ignores does not make it comply — three for three.** The Disease/Finding scope clause (added Phase B, still missing 75-88% of that class), "Only the writer's own reactions count" (still emits a bystander's death as a reaction), and now an explicit lab-value rule ("Cholesterol, blood pressure, test results... are things measured, not things the writer felt") which changed the count of emitted lab-value spans by exactly ZERO across three draws (1.33 per run, both arms). By contrast the one NEW instruction in the same arm — that a reaction may be a single word — moved its target immediately, 14.7 to 16.7 of 19 single-word gold mentions found, up in two draws of three. The distinction worth carrying: prompt wording buys behaviour the model can do but has not been asked for, and buys nothing where the model has been asked and does not comply. The second case needs a different model, not different words.
- 2026-08-28 — **Three span filters shipped as arms, and they reproduce their offline measurement exactly through the production path.** `rung0_drop_ungrounded` (locate() could not find the quote — 11 records on dev, 0 of which matched any gold mention), `rung0_drop_fragments` (the span carries no content word: "because", "This", "No"; single CONTENT words are never touched, since 14 of 45 unproposed gold mentions on dev are one word) and `rung0_drop_duplicate_spans` (the same span key twice — gold is claimed one-to-one, so the second can only be a false positive). All three decide WITHOUT gold and can only decline to emit, never invent. `filter_spans` runs LAST, after locate() and after the trim, so it never judges a span the trimmer had not yet corrected. Stacked on dev over the four-fix arm: exact F1 0.386 -> **0.399**, overlap 0.453 -> **0.469**, paired bootstrap **+0.0133 [+0.0033, +0.0214]** — and the end-to-end run through `r0.extract_document` reproduced the offline simulation to three decimals (0.399/0.469, detection 0.516/0.785, coding 0.773/0.597), which is the check that the arm measures the code rather than the harness. Cumulative vs the frozen baseline: **+0.0900 [+0.0485, +0.1294]** exact. NOT a merge of OVERLAPPING spans — that was measured and LOST (-0.008), because the coordination splitter emits several records sharing a head span by design. 7 tests, TDD'd (`tests/test_r0_span_filters.py`).
- 2026-08-28 — **`clean.inconsistent_gold` — gold that contradicts its own coding of the same span text is excluded, by DOCUMENT, opt-in.** The user's rule: for every annotation text occurring more than once anywhere in the corpus, compare the code sets; occurrences sharing NO code disagree. A SUPERSET is explicitly not a disagreement — the gold rule already credits any code in the set, "joint pain" is 57676002 x78 and 57676002+68962001 x1, and treating that as contradiction cost 10 of 40 dev documents and 40% of dev's gold in the variant that did (measured, rejected). The unit is the document, not the mention. Measured: **17 disagreements, 94 of 1250 documents (pool 90, dev 2, test 2)**, carrying 837 mentions out with them corpus-wide; all 17 are real (leg cramps 449917004 x36 vs 449918009 x3; tendonitis 34840004 x9 vs 21545007 x1; high blood pressure three ways; sore, muscle damage, tingling hands, ringing in the ears...). Dev effect: gold 226 -> 214, exact 0.399 -> **0.413**, overlap 0.469 -> **0.480**. **That is NOT an accuracy gain** — it removes 12 mentions the system was mostly answering wrong, so the denominator moved and post-rule numbers are not comparable to anything reported earlier, Phase F's 0.204 included; it also removes 2 TEST documents, so quoting the two together requires saying so. OPT-IN behind `python -m ladder.clean --build --inconsistent-gold`, never on by default, and `data/exclusions.csv` was NOT regenerated in this worktree — it is a symlink shared with the main checkout. 8 tests, TDD'd.
- 2026-08-28 — **The reranker's job is not "get gold onto the menu", it is "get gold to RANK 0" — the vise, quantified on the deep pool.** Rebuilt the k=200 merged pool for all 235 dev predictions of `arm-filters-dev` (174 matched to coded gold) and reproduced the retrieval-depth table exactly: gold at rank 0 **52.3%**, @15 77.6%, @20 81.0%, @50 83.9%, @200 91.4%. Against it, the pick's conversion by gold's rank on the production k=20 menu: **rank 0 → 94.5% (86/91), rank 1-19 → 42.3% (22/52), absent → 0%**, total coding accuracy on matched mentions 62.1%. So the transfer function from a ranking change to an accuracy change is `0.945·P(rank0) + 0.423·P(rank 1..k-1)`, and moving a gold code from rank 7 to rank 1 is worth a fifth of moving it to rank 0. This is used as the offline metric for every rerank variant so a ranking idea can be rejected without paying for a pick call. Ceilings on the same set: a perfect pick over a perfect rerank@15 = 77.6%, @20 = 81.0%, @200 = 91.4%.
- 2026-08-28 — **Retrieving deeper WITHOUT a reranker is a mathematical no-op, and that is why k=40 lost in 2026-08-24.** Sorting a k=200 pool by the dense score and keeping the top 15 returns, by construction, the dense top 15 — measured identical to the production menu on rank-0 rate and recall (52.3% / 77.6%). Depth is only reachable through a *reordering* signal the cosine does not already encode. Stated because "retrieve 200, rerank to 15" reads like two independent levers and is one: the 91.4% recall@200 is inert until something re-scores it, and raising k alone can only move the menu's tail, where the pick converts at 42% and degrades (measured 2026-08-24, pick-past-gold 34 → 47).
- 2026-08-28 — **REJECTED: feature reranking on lexical overlap — it loses, confirming the 2026-08-24 hybrid-retrieval result in the rerank framing.** Best-synonym containment against `data/keywords.csv` (a 50/50 blend of span coverage and term precision over all of a concept's synonyms, not just the best-scoring one the retriever keeps), added to the cosine at weights 0.02-0.20 over the k=200 pool: rank-0 rate 52.3% → **52.3 / 51.1 / 50.0 / 51.7%** and recall@15 falls at every weight. Combined with polarity it also degrades that arm. The 2026-08-24 finding was that hybrid *retrieval* lost; this is the stronger version — lexical similarity is not a useful *reordering* signal even when the dense corpus filter is kept and only the order is at stake.
- 2026-08-28 — **A POLARITY feature is the only free reranking signal that moves anything, and it is too small to ship alone: +2.9 points rank-0.** The dominant inspectable failure class in the 68 mentions where gold is in the pool but not at rank 0 is **antonym inversion by the embedder** — "can't sleep" → |able to sleep| (gold: insomnia), "could'nt concentrate" → |able to concentrate| (gold: unable to concentrate), "could'nt walk" → |able to walk|, "barely remember anything" → |able to remember people|, "sever heat intolerance" → |tolerant of heat|. Cosine is close to blind to negation. A cue-based penalty (negative-polarity span × positive-ability label) at w=0.02 lifts rank-0 52.3% → **55.2%**, expected coding 60.1% → 61.4%. Real, cheap, deterministic — and **+1.3 expected points is inside the 2-3 point dev noise floor**, so it cannot be validated end to end on its own. Kept as a component, not proposed as the arm. The second-largest class is near-ties between genuinely synonymous distinct SNOMED codes ("upset stomach" 0.961 vs |stomach upset| 0.828 — different codes, both defensible; "absence of motivation" 0.943 vs |loss of motivation| 0.938), which no reranker fixes and which belong with the convention noise measured the same week.
- 2026-08-28 — **`checks.labels_proposed` is EMPTY on all 235 dev records: the label-query arm of `_merged_candidates` never fires in the shipped S2 path.** `FIND_PROMPT` ends "The concept name is chosen in a second step, so do not give one here", so S2's mentions carry no `sct_label` and the merged menu is span-only in every run this repo has measured. The function's own docstring cites the label path as worth 35 of 226 gold mentions on arm 3 (2026-08-25) — that measurement was S1-shaped and does not describe S2. Consequence for the reranker work: the pool it reranks is a *single-query* dense pool, and the docstring's example failure ("extremely sick" never surfacing |Generally unwell|) is unfixed for exactly that reason — it is still in the dev residue at rank 117. NOT changed here: adding a name back to the FIND schema is a retrieval change and a prompt change, and prompt arms lost three for three this week; recorded so the next reader does not mistake dead code for a measured path.
- 2026-08-28 — **The shipped rung 0 baseline is `rung0_cut_rate: 0.06`, which the handoff notes recorded as "undetermined, left off".** Reproducing `out/arm-filters-dev` in a fresh worktree gave exact 0.385 / overlap 0.463 against the recorded 0.399 / 0.469 — codes IDENTICAL wherever spans agreed (0 differences over 212 shared records), 36 spans differing, every one of them an interior clause cut that fired in the recorded arm and not in the replication ("lower back pain so severe" -> "lower back pain", "pain would subside" -> "pain"). Sweeping the one key that controls that cut: 0.03 -> 0.394, 0.05 -> 0.403, **0.06 -> 0.399 / 0.469, det 0.516/0.785, coding 0.773/0.597, 92/108 correct, 235 predictions — all seven figures exact**. So the arm was run at 0.06 and the note was wrong. Recorded because the trim rules are learned from POOL AT RUNTIME and the cut threshold is the only free parameter in that learning: an arm compared against a baseline built at a different threshold is comparing two trimmers, not the change under test. The reranker arms below are all measured against `{split, drop_ungrounded, drop_fragments, drop_duplicate_spans, cut_rate 0.06}`, reproduced in this worktree before any of them ran.
- 2026-08-28 — **MEASURED, and it is the first thing that moves rank 0: an LLM rerank pass over a deep menu lifts gold-at-rank-0 from 52.3% to 62.1%.** Offline probe over all 235 dev predictions (174 matched to coded gold), replaying the same k=50 dense pool the arm would retrieve: ask the extractor for a plausibility SHORTLIST — "up to 15 numbers per reaction, best first, this is not a choice" — batched five mentions to a call, then order the menu by the returned list with everything it omitted kept behind it in retrieval order. **rank 0 52.3% → 62.1% (+9.8), recall@15 77.6% → 81.6% (+4.0).** Under the conversion model measured the same day (0.945·P(rank0) + 0.423·P(rank 1..14)) that is expected coding accuracy 60.1% → **66.9%, +6.8 points** — outside the 2-3 point dev noise floor, and several times the +1.3 the best free feature reranker managed. THE COST IS REAL AND IS NOT NETTED OFF: 63 extra calls and 172,370 extra tokens for a dev split whose baseline is 95 calls and ~138k tokens, so the arm is roughly 1.7x the calls and 2.2x the tokens, 685 s of wall clock. Three measures, separately, as always. Two caveats on the probe itself: only **157 of 235** mentions came back reranked (3 unparsed replies plus reactions the model silently omitted from its list — the same `no_pick` under-coverage that made the pick batch 7), and an omitted mention falls through in retrieval order, so this is a floor rather than the arm's best form. Asking for RECALL over a deep menu rather than a CHOICE over a short one is the load-bearing difference: it is a strictly easier question, and the pick still runs afterwards, so nothing here decides a code.
- 2026-08-28 — **THE RERANKER WORKS AS A RANKER AND DOES NOT CONVERT, because the reranker and the pick are the same model making the same judgement.** End to end on dev, 40 documents, draw 0, against the reproduced baseline (exact 0.399 / overlap 0.469, gold at menu rank 0 52.3%, coding accuracy on matched mentions 62.1%): the `llm` arm at deep=50/keep=15 lifts gold to menu rank 0 **52.3% → 58.6%** and the pick still ratifies rank 0 at **96.1%** — the ranking change is real and it survives into production. **And the headline does not move: exact F1 0.399 → 0.399 (byte-identical, 92 correct, coding 0.773), overlap 0.469 → 0.477, for 166 calls against 95 and 1,106 s against 17.** The arithmetic of why: the rank-0 bucket gains 11 mentions and 12 correct answers (86/91 → 98/102), and the rank-1+ bucket loses 10 (22/52 → 12/41) because what the reranker promoted is exactly what the pick was already converting from below. Measured directly, on the 52 baseline mentions where gold sat at rank 1-19: the reranker promoted it to rank 0 for **50.0% (11/22) of the mentions the pick already coded correctly and 16.7% (5/30) of the ones it got wrong** — a threefold difference where independence would predict none. THIS IS THE GENERAL LESSON AND IT IS NOT ABOUT RERANKING: the 94.5%-at-rank-0 / 42.3%-at-rank-1-19 conversion table is NOT a transfer function you can push an arbitrary ranking through. It was measured over rankings the COSINE produced, and the pick's disagreement with ranks 1-19 is not inattention — it is the same disagreement the reranker has, because it is the same model. A stage placed in front of a model to feed it better inputs buys nothing to the extent that it shares the model's errors. The polarity arm shows the same shape from the free side: rank 0 52.3% → 55.2%, exact F1 unchanged at 0.399, overlap 0.469 → **0.490** at zero extra calls.
- 2026-08-28 — **The reranker's truncation gives back the depth it retrieves: `absent` did not move.** Gold is off the menu entirely for 31 of 174 matched mentions (17.8%) at baseline and for **31** in the deep=50/keep=15 `llm` arm — identical — because retrieving 50 (dense recall 83.9%) and then keeping 15 (77.6% if the order were unchanged) costs about as much menu recall as the extra depth bought. The polarity arm is worse on this axis, 37 absent, and its rank-0 gain does not pay for it. That bucket converts at **0%** by construction, so it is now the largest single loss in rung 0's coding layer and the only one a reranker could still address — which means the useful form of the stage is not "reorder the menu the pick already sees" but "use a deep pass to change WHAT IS ON a menu of unchanged length". Next arm: deep=200, keep=20 — a reranked 20 drawn from a 91.4%-recall pool is a different object from the dense-ordered 40 that lost in 2026-08-24, and it holds the menu at the length the pick was tuned for.
- 2026-08-28 — **REJECTED: deep=200 / keep=20. The reranker cannot recover the menu recall that is actually missing, and it costs 4.2x the run's entire token budget to fail.** Probe over the same 235 dev predictions with a 200-candidate menu (batch 3): rank 0 52.3% → 59.2% — no better than the 50-candidate version — and **menu recall@20 81.0% → 82.2%, +1.2 points against the 91.4% that is sitting in the pool**. Cost: 93 calls and **576,822 tokens** for a dev split whose whole baseline run is 95 calls and ~138k, 2,204 s of wall clock, and the model's own failure rate rises fivefold — **15 unparsed replies against 3** at deep=50, with only **133 of 235** mentions coming back reranked. This is the correlation result again in its sharpest form: the gold codes at ranks 20-199 are precisely the ones the model does not recognise as the answer, so a pass by the same model does not lift them, and asking it to hold 200 candidates in one judgement degrades the transport as well as the judgement. **The 91.4% recall@200 is not reachable by any reranker that shares the picker's model**, which is the honest closing statement on the +0.20 oracle: that ceiling prices a reranker with independent knowledge, and this repo has one model family for the extractor role.
- 2026-08-28 — **THREE DRAWS KILLED THE ONE ARM A SINGLE DRAW HAD CALLED SIGNIFICANT — and that is the most useful thing this session measured.** The free `polarity` reranker at deep=50/keep=15 scored, against a matched baseline at the same sample index, dev 40 documents: draw 0 overlap **+0.0217** (paired bootstrap +0.0215 [+0.0000, +0.0433] — a bootstrap CI that excludes zero), draw 1 **+0.0087** [-0.0084, +0.0274], draw 2 **-0.0089** [-0.0400, +0.0260]. Mean +0.0072 with the sign reversed on one draw of three; on exact, +0.0000 / +0.0087 / -0.0044, mean +0.0014. **The paired bootstrap over documents does not capture run-to-run model nondeterminism** — it resamples the documents of ONE draw, so it prices the corpus sample and nothing else, and a "CI excludes zero" from a single draw is exactly the evidence that produced this repo's earlier non-replicating gains. The two must be reported together or not at all. Baseline spread over the three draws for reference: exact 0.395-0.408 (1.3 points), overlap 0.469-0.475 (0.6 points) — TIGHTER than the 2-3 points the handoff notes assume for dev, and lumpy rather than smooth: draws 0 and 1 produced byte-identical detection (235 predictions, det 0.516/0.785) and differed only in the pick, while draw 2's FIND step differed outright (225 predictions, det 0.537/0.794). Nondeterminism enters at the FIND call and is all-or-nothing per run.
- 2026-08-28 — **DECIDED: the reranker SHIPS AS CODE AND AS ARMS, and NOTHING is turned on. `manifest.json` is byte-unchanged.** `ladder/rerank.py` + `rung0_rerank` (`None` | `"polarity"` | `"llm"`), `rung0_rerank_deep` (50), `rung0_rerank_k` (15), `rung0_rerank_batch` (5), `rung0_rerank_weight` (None -> 0.02), wired between `_step_pick`'s retrieval and `_decide`, 23 tests, TDD'd (`tests/test_rerank.py`), 649 tests green, preflight clean. Measured summary over dev, three draws each where an arm was run end to end:

      arm                        exact          overlap        extra calls   extra tokens
      baseline (mean of 3)       0.401          0.472              --             --
      polarity deep50 keep15     0.402 (+0.001) 0.479 (+0.007)      0              0
      polarity deep50 keep20     0.395 (-0.004) 0.482 (+0.013)      0              0    [draw 0 only]
      llm      deep50 keep15     0.399 ( 0.000) 0.477 (+0.008)     71        +172,370    [draw 0 only]
      llm      deep200 keep20    probe only: menu recall +1.2pt    ~93       +576,822

  Not shipped ON because nothing here separates from zero across draws, and an arm that cannot be told from noise must not become the configuration every rung above it is measured against. Kept as code because the component is now BUILT and its failure is DIAGNOSED rather than assumed — the next reader gets the measurement instead of the ceiling, and `rung0_rerank_deep` is the only place in rung 0 where retrieval depth and menu length are separable at all. THE HEADLINE FINDING IS NEGATIVE AND IT IS ABOUT ARCHITECTURE, NOT TUNING: a rerank stage in front of a picker buys only what it knows that the picker does not, and here they are one model.
- 2026-08-28 — **NOTHING measured this session is validatable.** The test split was spent in Phase F (2026-08-26) and this repo has no held-out data left. Every number in the four entries above is dev-side, on 40 documents and 226 gold mentions, against a baseline reproduced in this worktree rather than requoted. The reranker's rejection is therefore a rejection ON DEV; it is not evidence that no reranker helps, only that this one does not help here, and the standard of evidence that killed it (three draws, sign reversal) is the same standard nothing older in the repo was held to.
- 2026-08-28 — **REJECTED, and the premise was wrong too: a system message does NOT halve rung 0's nondeterminism on dev, because dev was never at 0.55.** The ten-document study (same date) measured pairwise span-set Jaccard across three draws at 0.515-0.584 without a system message and 0.926-1.000 with a bare identity, and was recorded as needing confirmation on dev. Confirmed on dev, 40 documents, three draws each, matched configuration: **no system message 1.000 / 0.764 / 0.764, mean 0.842; bare identity 0.859 / 1.000 / 0.859, mean 0.906.** The baseline is already at 0.842, so there is no 0.55 to halve, and the residual difference is not an effect — **both variants show the identical structure, two of the three draws BYTE-IDENTICAL and the third differing**, so with three draws the two means differ only in which pair happened to coincide. That is one coin flip, not a measurement. The accuracy cost, by contrast, is consistent and large enough to see: exact F1 **0.371 / 0.369 / 0.371 with the system message against 0.399 / 0.395 / 0.408 without** — the ranges do not overlap, roughly -3 points, at overlap 0.472-0.483 against 0.469-0.475. So the arm is the same exact-for-overlap trade the three prompt arms made, with no determinism benefit to justify it. Nothing added to production; the variant exists only in `out/harness/rolearm.py`. THE WIDER CORRECTION: **the ten-document subset overstates rung 0's nondeterminism badly**, the same way it was found on 2026-08-28 to overstate the baseline spread (6 points there against the 1.3 points of exact and 0.6 of overlap measured across these three dev draws). Ten-document arms were already on the do-not-repeat list for effect sizes; this extends it to variance estimates, which is the harder error to notice because a noisy subset makes a noise measurement look important.
- 2026-08-28 — **THE RUNGS 1-6 AUDIT, framing: the rung 0 baseline was reproduced EXACTLY in a fresh worktree before anything above it was touched.** `{split, drop_ungrounded, drop_fragments, drop_duplicate_spans, cut_rate 0.06}` over 40 dev docs: exact 0.399 / overlap 0.469, det 0.516/0.785, coding 0.773/0.597, 92/108 correct, 235 predictions — all seven figures identical to `out/arm-filters-dev`, served from the shared `.llm_cache` in 16s (`out/arm-auditbase-dev.*`). Every number below is measured against that one rung 0, for the same reason S2 was frozen: rungs 1-6 compared against different rung 0s are not comparable to each other. HARD CONSTRAINT, restated: Phase F spent the test split on 2026-08-26 and no held-out data remains, so everything in this audit is DEV-SIDE AND NOT VALIDATABLE, and Phase F's shipped numbers stand exactly as reported — a defect found here does not re-open them.
- 2026-08-28 — **FOUND, and it is the audit's central result: rung 0's accuracy work silently DISABLED the two rungs above it.** Rung 1 rejected 5.1% on the Phase F test run, all `schema_invalid`. On the current dev baseline it rejects **1 record of 248 (0.4%)**, because `rung0_drop_ungrounded` (shipped 2026-08-28) now removes the unlocated `(-1,-1)` spans that WERE that entire rejection class before rung 1 ever sees them. Rung 2's trigger is `r1_verdict == "REJECT"` with a reason in `correctable`; that set now holds **1 record of 248**. Rung 2 fired zero times in Phase F and would fire once here. Nothing is broken in rung 2 — its input was removed by a fix three rungs below it, and no test and no report could have shown that, because both rungs still pass every check they have. A ladder whose rungs are measured one at a time cannot see this; only replaying the whole stack against a changed lower rung can.
- 2026-08-28 — **FOUND: rung 1's code checks are STRUCTURALLY UNREACHABLE under the shipped rung 0, and the rung's own headline check is the dead one.** Over the 248 baseline records: `code_unknown` fires **0 times**, and `all_reasons` (which runs every check unmasked, so this is not the first-failure ordering bias) confirms 0. The reason is architectural, not lucky: S2 picks its code from a dense-retrieved menu built from `data/keywords.csv` and resolves names through `KeywordTable.resolve`, and both contain only real SNOMED codes — **the model can no longer emit a code that does not exist, so the check written to catch hallucinated codes cannot fire.** Two more are degenerate on the same records: `sct_active` is True for all 232 with a code and `sct_outdated` is False for all 235 (so `reject_inactive` and the `outdated` flag have nothing to act on), and `label_verified` is True for **232 of 232** — because S2's menu shows the code and its vocabulary label TOGETHER, so `sct_label` is the vocabulary's own label by construction and the check asks a question whose answer retrieval already guaranteed. Three vocabulary lookups per record that cannot vary. Rung 1 was tuned in 2026-08-22 against a rung 0 that recalled codes from weights; retrieval moved the failure mode out from under it.
- 2026-08-28 — **MEASURED: the one rung 1 check that survives carries MORE signal than the paid judge above it.** Rung 1's ACCEPT/BAND split is the lexical match between the span text and the vocabulary's words for the code. Coding accuracy on matched spans, dev baseline, overlap: **ACCEPT 83.7% (41/49) vs BAND 35.9% (47/131)** — a 2.3x separation, over 21% of predictions, at zero model calls. Rung 4's judge, one granite4:micro-h call per record, separates **21.0% (pass) vs 12.7% (fail)** on the dev full-ladder run (phaseD-r3-2) and **27.0% vs 21.9%** on the test run (phaseF-test-1) — 1.65x, falling to 1.23x where it counts. The free deterministic check outperforms the LLM judge on the only axis a judge is for, and it is the signal rung 5 already routes on.
- 2026-08-28 — **FOUND: nothing in `ladder/` reads `checks["r4_verdict"]`.** Rung 4's default is `route: false` on the sound argument that a judge that routes confounds every rung above it, and the docstring says "let rung 5 act" — but `r5.decide()` reads the zone, `r1_verdict` and `rec.confidence`, and has no branch for rung 4 at all. The only readers are `scripts/rejudge_r4.py` and its test. So rung 4 costs one model call per record, in every full run including Phase F's 314, and **cannot change any shipped number**. That is not a bug in rung 4's code; it is a rung whose output has no consumer, and it went unnoticed because "recorded, not routed" reads like a deliberate posture rather than a dead end.
- 2026-08-28 — **FOUND: rung 5's `tau` is a dead dial, because rung 0's confidence is a constant.** The confidence distribution over the dev baseline is `{1.0: 204, 0.99: 44}`; over phaseD-r3-2 `{1.0: 203, 0.99: 42}`; over the Phase F test run `{1.0: 236, 0.99: 45, 0.95: 21, 0.9: 12}`. There is no operating point: any tau <= 0.9 abstains nothing, and any tau above it abstains 80-98% of the set on a number the model emits as boilerplate. `R_LOW_CONFIDENCE`, one of rung 5's three declared abstention reasons, is unreachable — `tau: 0.0` was never a tuned choice but a forced one, and the risk-coverage curve `sweep()` produces has nothing to sweep. Recorded, not fixed: eliciting a calibrated confidence is a rung 0 prompt change, and prompt changes have been measured and rejected three times (2026-08-28).
- 2026-08-28 — **DEFECT, fixed TDD: rung 3 changed the code and left rung 1's verdict describing the code it replaced — and rung 5 routes on exactly that verdict.** `apply()` did `rec.sct = win` and stopped. Demonstrated on the shipped artifacts, not hypothesised: **phaseD-r3-2 changed 25 codes and phaseF-test-1 changed 30, and all 55 records carried a rung 1 verdict about a superseded code.** LIPITOR.739#0 is the one that reached a user: "Chronic pain" coded 82423001 |Chronic pain|, ACCEPTed by rung 1 on an exact lexical match, voted 3-0 to 762452003 |Chronic musculoskeletal pain|, and **SHIPPED as VERIFIED** — on a lexical match to a code it no longer had. Re-checked against the registry, `lexical_match("chronic pain", 762452003, exact)` is False, so the configured rung 1 would have banded it and rung 5 would have abstained it. The other 54 were already in BAND, so their zone did not change; the verdict was wrong on all of them. `ladder/rungs/r3.py:revalidate()` now owns adoption of the winning code: it ALWAYS records `checks["r3_r1_stale"]` so no rung above can mistake a verdict about a replaced code for a verdict about this one, and under the new arm `rungs.3.revalidate` (**default false**) it re-judges through `r1.zone` — the one measured rung 1, under `manifest.rungs.1` rather than r1.DEFAULTS — keeping the superseded verdict in `checks["r3_r1_before"]`. Off by default because re-banding MOVES COVERAGE to rung 6, which is a declared trade to be measured, not a correctness fix to be switched on. Four tests in `tests/test_rungs_audit.py`.
- 2026-08-28 — **DEFECT, fixed TDD: rung 2 re-validated its own repair under `r1.DEFAULTS`, not under the rung 1 the manifest configures.** `run_ladder` builds each rung's cfg from `manifest.rungs[n]` alone, so rung 2's cfg contains no rung 1 settings whatever; passing it straight to `r1.zone(cand, source, registry, cfg)` meant `{**r1.DEFAULTS, **cfg}` resolved every rung 1 key to its default, and the meddra table was not passed at all. A repair could therefore be counted `rescued` under a rule the rung 1 that rejected it does not use — the test makes the same repair a rescue under the defaults and a `still_failing` under `label_check: "reject"`. **No published number moves**: `manifest.rungs.1` currently equals `r1.DEFAULTS` on every key, and rung 2 attempted 0 corrections in Phase F, so this was a latent divergence that would have bitten silently on the first rung 1 setting anyone changed. `r2._r1_params()` now reads rung 1's settings from the manifest and the meddra table is threaded through. Two tests, one of which re-asserts the Phase C double-post contract from the audit side.
- 2026-08-28 — **NOT A DEFECT, contrary to the session handoff: rung 2's post-sent-TWICE bug was already fixed on 2026-08-26.** The handoff listed it as "flagged for its own session, never fixed". `correct()` passes `llm(prompt, "", "correct")` and `tests/test_contracts.py::test_rung_2_is_sent_the_post_exactly_once` has guarded it since. Re-verified here rather than taken on trust from either source, and re-asserted in the audit's own test file. Recorded because a handoff note and the decision log disagreed, and the code settled it.
- 2026-08-28 — **FOUND: "which configuration produced this number" has two answers again — `manifest.json` is 5.9 exact points below the baseline every recent measurement was compared against.** The 2026-08-27/28 rung 0 arms (coordination splitter, the three span filters, `cut_rate`) were shipped as CODE with `r0.DEFAULTS` off, and the manifest was never appended to. So `manifest.rungs.0` today is `{offsets search, step S2, k 20, dense, fewshot, trim}` with `rung0_split/drop_ungrounded/drop_fragments/drop_duplicate_spans` all false and `cut_rate` null. Measured this session on the same 40 dev docs from the same cache: **manifest as shipped = exact 0.340 / overlap 0.449, det 0.449/0.745, 233 predictions** (`out/arm-manifestdefault-dev.*`) against the audited baseline's **0.399 / 0.469, det 0.516/0.785, 235 predictions**. Both are real; only one is in the manifest, and it is not the one the decision log quotes. This is the same failure the `manifest.model` note was written for — a default in code and a declaration in configuration disagreeing — one layer down. NOT fixed unilaterally: turning the arms on changes every number the ladder produces, and the manifest is edited jointly. Flagged for the owner's call in the MR; every rungs 1-6 measurement below is stated against the configuration it actually ran on.
- 2026-08-28 — **MEASURED: the `rungs.3.revalidate` arm is correct bookkeeping that the headline metric cannot see.** Replayed EXACTLY over two finished full-ladder runs rather than re-run — legitimate because the arm changes nothing upstream of the vote (rung 3's samples, matching and winner are already fixed in the records, rung 4 does not route, rung 5 is deterministic given the verdict), so both arms are computed from the SAME votes and rung 3's own sampling noise leaves the comparison. The replay reproduces Phase F's shipped numbers exactly at `revalidate=false` — F1 exact 0.204 / overlap 0.215, 72 shipped, 242/314 routed to a person — which is what licenses the counterfactual. **Dev (phaseD-r3-2):** 25 changed records re-judged, flips `BAND->BAND` x24 and `ACCEPT->BAND` x1; F1 exact 0.153 and overlap 0.170 UNCHANGED, correct 35/39 unchanged, shipped 37 -> 36, routed 208 -> 209. **Test (phaseF-test-1, a labelled counterfactual — Phase F's reported numbers stand):** 30 changed, flips `BAND->BAND` x28, `ACCEPT->BAND` x1, `BAND->ACCEPT` x1; exact unchanged at 0.204, overlap 0.215 -> 0.218, shipped and routed both unchanged at 72 and 242. So the fix withdraws a false warrant — LIPITOR.739#0 stops shipping as VERIFIED on a lexical match to a code it no longer has — and moves precision, recall and F1 by nothing, because a record that was wrong scores the same whether it ships wrong or is withdrawn. **The ladder's own headline metric is blind to the defect the ladder exists to prevent.** That is the argument for the arm and it is not an accuracy argument.
- 2026-08-28 — **THE THROUGH-LINE, confirmed at rung 3: re-sampling the same model moves right answers about as readily as wrong ones, and the dev gain was a small-sample artefact.** Scored per changed record against gold, overlap, before and after the vote. **Dev (phaseD-r3-2, 18 scorable changes): fixed 5, broke 0, net +5** — and 3 of the 5 are `None`/`CONCEPT_LESS -> a correct code`, i.e. rung 3 filling a gap rather than correcting an error. **Test (phaseF-test-1, 21 scorable changes): fixed 2, BROKE 2, net 0** — it destroyed |Severe pain| 76948002 and 281245003 |chest muscle soreness|, both previously correct, while rescuing two others. This is the reranker's shape at a different rung (2026-08-28: the reranker promoted 50% of what the pick already had right and 16.7% of what it had wrong): a rung that asks the extractor to check itself has no independent information to add, so its edits are close to a coin flip, and a 5-0 on 18 trials is what a coin flip looks like sometimes. Rung 3 costs 2.6x rung 0's tokens for a net of zero correct records where it was measured out of sample. Rung 6 remains the only rung that adds information, and it adds it because it is a person.
- 2026-08-28 — **THE AUDIT'S STRUCTURAL FINDING: every deferral in the ladder terminates in a field nothing reads.** Rungs 2, 3 and 4 each decline to act on their own evidence, on the correct argument that a rung which routes confounds every rung above it, and each names rung 5 as the rung that will act: rung 2 writes `checks["r2_declined"]` ("rung 5 owns abstention and reads `checks['r2_declined']`"), rung 3 writes `checks["r3_unanimous_none"]` ("EVIDENCE for rung 5, not an action here"), rung 4 writes `checks["r4_verdict"]` ("record ... and let rung 5 act"). **`r5.decide()` reads none of the three.** It reads the zone, `r1_verdict`, `r1_reason` and `rec.confidence`, and its whole config is `{tau, abstain_zones, abstain_on_reject}`. A repo-wide search finds no consumer for any of the three outside `scripts/rejudge_r4.py` (a diagnostic) and the rungs that write them; `docs/wiki/content/r3.md` had already noticed one instance ("[[r5]] does not yet read `checks["r3_unanimous_none"]`; it fired 0 times, so nothing has exercised the channel") without the pattern being visible. So the ladder's paid rungs are, in composition, ONE rung: rung 5 routes on rung 1's free lexical verdict and on nothing else, and rungs 2, 3 and 4 can only affect a number by mutating `rec.sct` in passing — which only rung 3 does, and which is the defect fixed above. Each rung is individually careful and correct; the composition is empty. This is not a bug that a unit test could catch, because every rung does exactly what its own docstring promises. It is only visible from the outside, and only by asking what reads what.
- 2026-08-28 — **The audit's headline positive claim, put through the three-draw test that killed the reranker — and it holds.** Rung 1's ACCEPT/BAND separation was first measured on one draw (83.7% vs 35.9%), which by this repo's own 2026-08-28 rule is not yet a result. Re-measured over the three independent baseline draws `arm-rrbase-d0/d1/d2` (dev, 40 docs, coding accuracy on overlap-matched spans): **ACCEPT 83.7 / 83.0 / 87.2, BAND 35.9 / 36.8 / 36.2, ratio 2.33 / 2.26 / 2.41.** The BAND lane moves 0.9 points across draws and the ratio 0.15 — far tighter than the 1.3-point exact / 0.6-point overlap spread the headline F1 shows, because the quantity is conditional on a deterministic property of the record rather than on the run. It also holds under a DIFFERENT rung 0: on `arm-baseline-dev` (pre-span-filters) it is 85.7% vs 30.1%, a 2.8x separation. So "the free deterministic check out-separates the paid judge" is a three-draw, two-configuration result, and it is the only claim in the audit strong enough to be stated without a sampling caveat. Rung 4's separation, by contrast, is one draw per split and falls from 1.65x on dev to 1.23x on test — it is reported with the caveat attached.
- 2026-08-28 — **THE FULL LADDER, run end to end on dev under the audited configuration (`out/audit-full-dev-1.*`) — and every audit finding reproduces live.** All seven rungs, order [0,1,2,3,4,5,6], 40 dev documents, 248 records, one run id. Config is the run copy `out/manifest-audit.json`: the tracked manifest plus the five rung 0 arms it does not carry (the 5.9-point gap logged above), `rungs.3.revalidate` left FALSE because this is the ladder as it ships. Shipped **F1 exact 0.182 [0.124-0.260] / overlap 0.187**, detection 0.516/0.785, coding 0.353/0.238; outcomes exact 42 correct / 0 outdated / 76 abstained / 1 incorrect / 0 modernised. **52 records shipped VERIFIED at 0.808 answered-accuracy; 196 of 248 routed to a person (reviews_per_100 79.03).** Errors per 100 fall 63.3 -> 4.03, and every point of that fall is rung 1's free verdict acted on by rung 5. Per-rung answered-accuracy: rung 0 **0.371**, rung 1 0.371 (judges, does not route), rung 2 0.371, rung 3 **0.367**, rung 4 0.367, rung 5 ships 0.808 at 0.21 coverage. **Rung 3 is NET NEGATIVE on this draw and rung 4 moves nothing at all** — the latter by construction, since nothing reads its verdict. Three cost measures, never fused: **683,487 tokens / 618 calls** (rung 0 164,897; rung 2 548; rung 3 **425,355** = 2.6x rung 0; rung 4 92,687); **p95 latency rung 3 152.2 s, rung 4 1.5 s** — rung 0's and rung 2's p95 of ~1.0 s and 0.0 s are CACHE-SERVED and are not latency measurements, which is stated rather than quoted; **196 records routed to a person.** Rung 3's own breakdown: 185 voted-no-change, 29 changed, 7 withheld on a single sample, 27 not_resampled. Rung 4: pass 146 / fail 95 / 7 unjudged, and no downstream effect.
- 2026-08-28 — **The stale-verdict defect fired again on the fresh run, for the THIRD independent time, and the new stamp caught it in production.** `checks["r3_r1_stale"]` was written on all **29** records whose code rung 3 changed, and the breakdown by (rung 1 verdict, final zone) is `BAND->ESCALATE` x28 and **`ACCEPT->VERIFIED` x1**. The one that shipped is `LIPITOR.108#2`, span "stamina": rung 1 ACCEPTed 248276000 |Stamina| on an exact lexical match, rung 3 voted 2-1 to 248277009 |Lack of stamina|, and the record shipped VERIFIED carrying the verdict computed on the code it no longer had — `lexical_match("stamina", 248277009, exact)` is False. A different record from phaseD-r3-2's LIPITOR.739#0 and from phaseF-test-1's, and the same count each time: **25, 30 and 29 stale verdicts across three runs, with exactly one false VERIFIED warrant in each.** So this is not an artefact of one draw; it is what rung 3 does on every full run. (Whether the new code is *better* is beside the point here — 248277009 arguably reads the patient correctly — because the claim rung 1 made was about 248276000, and rung 5 shipped the record on that claim.) `revalidate` was off for this run, as declared, so `r3_revalidated` is 0.
- 2026-08-28 — **A model comparison measures YOUR HARNESS until you prove otherwise: two of five open models "failed" and both failures were ours.** Setting up the open-weight extractor comparison, `llama3.1:8b` and `qwen3:8b` both returned nothing parseable on the first dev document. Neither was a model failure. **qwen3:8b is a REASONING model** and no budget was ever registered for it, so it inherited `DEFAULT_MAX_TOKENS=2000`, spent the entire budget deliberating and returned `truncated=True` with EMPTY content on a 166-character post — the exact failure `models.yaml` exists to prevent, repeated because a new model was run without a registry entry. **`llama3.1:8b` produced CORRECT JSON with the RIGHT mentions** (`extreme rectal bleed`, `blood transfusion`, `extremely sick`, `might not survive`) and wrapped it in conversational prose: a lead-in sentence, a fence, then a trailing remark. `Caller._unfence` uses an ANCHORED match, so a fence preceded by prose is not a fence, and a good answer was counted as a parse failure. Publishing either as a model result would have measured **which model the harness was built around** — gpt-oss:20b emits bare JSON because these prompts were tuned against it — and would have put two false negatives into an article. Recorded as a method finding in its own right: before reporting that a model cannot do a task, read what it actually returned.
- 2026-08-28 — **`Caller._unwrap` — a THIRD counted repair, on the same terms as `_unfence` and `_reclose`.** Lifts the outermost JSON object or array out of surrounding prose. Fires only when the reply does not already parse; never fabricates (an empty parse result is refused, the same guard `_reclose` uses so `" {"` cannot become `{}`); applies identically to every model; and is COUNTED in `Caller.prosed`, so a model's chattiness is reported as a compliance cost instead of vanishing into a zero score. Call path is now `_reclose(_unwrap(_unfence(text)))`. TDD'd, 7 tests (`tests/test_llm_unwrap.py`), 662 green. This is a fairness fix, not a leniency one: it removes a harness advantage the incumbent model had, and the counter is what keeps the removal honest.
- 2026-08-28 — **Four models registered in `models.yaml` with measured budgets, and the budgets are the experiment's controls.** `qwen3:8b` at 8000/300s (matching gpt-oss:20b, the only budget in the file shown to let a reasoning model finish); `llama3.1:8b`, `mistral:7b-instruct` and `ibm/granite4:micro-h` at 2000/180s (non-reasoning, measured replies 180-400 completion tokens). `mistral:7b-instruct` is in the set as the CONTROL on the 2026-08-25 BioMistral result: we concluded "domain adaptation does not buy instruction-following" without ever running the un-adapted base family, so the claim had a hole. `granite4:micro-h` is in it because it FAILED the recall-based extraction path in 2026-08-24 (it answered `AFTERPROMPT`), and the retrieve-and-pick path turns coding into a multiple-choice question — the arm asks whether that is enough to rescue a 3.2B model. With budgets and `_unwrap` in place, **all five models produce parseable output on the smoke document**, and granite4:micro-h — which could not do this task at all under the old path — now returns three mentions and three codes.
- 2026-08-28 — **PRE-REGISTERED, before the sweep ran: four hypotheses for the open-weight extractor comparison.** Stated in advance because the article's central claim depends on one of them. **H1** detection F1 spreads >10pt across models. **H2** coding accuracy on matched spans stays within ~5pt for any usable model, because every model picks from the SAME 20-candidate dense-retrieved menu — the retriever is the ceiling, not the reader. **H3** the differentiator is format compliance, not medical knowledge: spread in usable-output rate exceeds spread in accuracy among usable outputs. **H4 — THE ONE THAT MATTERS — the ACCEPT/BAND separation is MODEL-INVARIANT**, holding at roughly 2-3x for every usable model, because it is conditional on the vocabulary and the record rather than on the run or the model. Falsification conditions, also fixed in advance: H2 dies if any model's coding accuracy moves >10pt at roughly constant detection; **H4 dies if any usable model's ACCEPT/BAND ratio drops below ~1.5x, and if it dies the article's central section narrows from a method to a single-model observation and gets rewritten.** Protocol: 3 draws per model (the three-draw rule), 40 dev docs, frozen rung 0 config, paired bootstrap over documents against the gpt-oss baseline, "usable" = parse-failure rate under 10%, detection and coding always reported apart, exact and overlap both, no model judged on one draw. Prior from the 2026-08-28 Sonnet arm: a frontier model fixed detection outright (55/62 -> 61/62) and produced IDENTICAL exact-correct answers (31 vs 31) with coding accuracy drifting down — which is what H2 predicts.
- 2026-08-28 — **The article moved to its own file, `docs/article.md`, and the angle changed on the user's call.** Away from "seven rungs, measured one at a time" — project jargon that means nothing outside the repo, and a numbering that has already been renumbered once — and towards the process: what we tried, in what order, and what each attempt taught. First draft was rejected by the user as "sounds like a bluff", and the diagnosis was accepted: every heading was a mic drop, the close was nine generalisations from single bugs dressed as laws, the title promised a method and delivered one correlation, and the confident voice was mismatched to a system that ships 21% of its answers. Worst of all, Part 3 was written as "the central result of the whole study" BEFORE the test that could falsify it had been run. Rewritten as a plain build report in InfoQ format (Key Takeaways box, scope up front, mermaid, practitioner voice), leading with the weak result and letting the failures carry the piece. `docs/article-iterations.md` stays as the raw material layer; `docs/article.md` is the piece.
- 2026-08-28 — **DEFECT, fixed TDD: an unexpected reply SHAPE crashed the whole run instead of costing one document.** `_step_pick` called `parsed.get("mentions", [])` on whatever `_parse` returned, so a reply that parsed to a JSON *list* raised `AttributeError` and killed a 40-document sweep on its third document. This repo's stated rule is the opposite — the call timeout was built to return an empty response rather than raise precisely so one runaway document costs ONE RECORD, not the run — and a reply shape nobody anticipated is the same case. `_parse` was never the problem: it correctly returned the parsed list. The crash was downstream, in code that assumed a dict. Found only because the open-weight comparison put a second model family through the path; three months of runs on `gpt-oss:20b`, which always emits the wrapper, never touched it. Now `_normalise_reply` decides shape, returns `None` for anything unreadable, and the caller counts a parse failure for that document and continues.
- 2026-08-28 — **`llama3.1:8b` answers with a bare JSON array where `{"mentions": [...]}` was asked for, and the bare array is ACCEPTED and COUNTED.** The content is unambiguously the requested content — a list of mention objects with the right fields — and only the wrapper is missing. Refusing it would score formatting luck rather than capability, and the incumbent emits the wrapper because these prompts were tuned against it. So `_normalise_reply` coerces a list of dicts to the wrapper and sets `meta["shape_coerced"]`, aggregated into the run's `shape_coerced` count, which puts the accommodation in the results table as a per-model compliance cost. **THE LINE, written down so the parser does not keep loosening until every model passes: a reply is accepted when its CONTENT is unambiguously what was asked for, and every accommodation is counted. Nothing is invented, guessed or repaired.** A list of mention objects qualifies; a list of bare strings is REFUSED, because "which field is this" would be a guess. 6 tests (`tests/test_r0_reply_shape.py`), 668 green.
- 2026-08-28 — **A test that used a real model name to test the no-entry fallback broke when that model got an entry.** `test_a_model_with_no_entry_gets_the_default` asserted on `ollama/ibm/granite4:micro-h`, unregistered at the time; registering it for the comparison failed the test for a reason with nothing to do with the mechanism under test. Repointed at `ollama/no-such-model:0b`, a name that cannot acquire an entry. Recorded because it is a small instance of a pattern this session hit twice: a test whose fixture is a moving part of the system tests the fixture.
- 2026-08-28 — **FOUND, and it reframes the whole study: the run-to-run nondeterminism this project spent months working around is a property of THE MODEL, not of language models. `llama3.1:8b` is BIT-REPRODUCIBLE across three independent draws.** The three llama draws produced record files with the **identical sha256** (`b57db22f3c28…`) and identical every figure: exact 0.336, overlap 0.395, detection 0.495/0.745, coding 0.679/0.530, 74/87 correct, 214 predictions. The same three draws of `gpt-oss:20b` under the identical config give **three different sha256s** and a 1.3-point exact spread. Verified as real rather than a cache artefact: the draws vary `sample_index`, which is part of the cache key, so they were three genuine sets of calls — 247 distinct cache entries were written during the llama window and the wall-clock times differ (756 s / 1056 s / 1011 s) — and they returned identical content. **The difference between the two models is architectural**: gpt-oss:20b is a Mixture-of-Experts REASONING model (its deliberation goes to a separate field and MoE routing is a known source of batch-dependent nondeterminism), while llama3.1:8b is a dense non-reasoning instruct model, deterministic at temperature 0. CONSEQUENCE FOR THE ARTICLE: "you cannot make the model repeatable" is FALSE as stated, and must become "we could not make THIS model repeatable, and the reason is a property we chose when we chose a reasoning MoE." The trade is now measurable in both currencies: llama is perfectly reproducible AND 6.5 exact points worse (0.336 vs 0.401). Reproducibility is a lever practitioners have, and it has a price.
- 2026-08-28 — **PRE-REGISTERED before the remaining draws landed, as a natural experiment on the architecture claim above.** The sweep's remaining models split cleanly on the proposed mechanism, so their determinism is a prediction rather than an observation: **`mistral:7b-instruct` (dense, non-reasoning) should be bit-reproducible; `qwen3:8b` (reasoning) should NOT be; `granite4:micro-h` (granite *hybrid*) is the interesting one and I am not predicting it.** If mistral varies or qwen3 does not, the architectural explanation is wrong and the finding degrades to "llama3.1:8b happens to be deterministic here", which is still worth reporting but is not a mechanism. Recorded now because the runs are already queued and the prediction is free to make and expensive to make honestly afterwards.
- 2026-08-28 — **PREDICTION CONFIRMED: `mistral:7b-instruct` (dense, non-reasoning) is ALSO bit-reproducible — identical sha256 (`d6d290e270116b69`) and identical every figure across three independent draws.** Two for two on the architectural explanation: both dense non-reasoning models are deterministic at temperature 0, the MoE reasoning model is not. `granite4:micro-h` (hybrid) and `qwen3:8b` (reasoning) still to land; qwen3 is the one that can still break it.
...
- 2026-08-29 — **DEFECT, fixed TDD: rung 4's prompt is the SEVENTH prompt constant and the FiNER port missed it — the judge was grading SEC filings by asking about adverse reactions.** The port rendered six rung 0 constants from `manifest.corpus.prompts` and left `r4.PROMPT` hard-coded, so on the FiNER arm a judge read a financial filing and was asked whether the span was *"really an adverse reaction the writer says they experienced"*, with *"the right SNOMED CT concept"*. The judge is the one rung whose entire job is to read the text, so a prompt about the wrong domain does not degrade its verdicts, it invalidates them — which is why `docs/cadec-vs-finer.md` records rung 4 as "not measured" on that arm. Fixed with the same slot mechanism as `r0.PROMPT_SLOTS`: `r4.judge_prompt(slots)`, fed from the `prompt_slots` that `run.py` already puts in every rung's cfg. **`judge_prompt(None)` is byte-identical to the old constant**, asserted against `git show HEAD:ladder/rungs/r4.py`, so no CADEC number moves. Two rounds of TDD, and the second is the point: the first fix passed a test that only checked "adverse reaction" and "SNOMED CT" were gone, and still rendered *"It read A PATIENT'S POST"* and *"the filer says they EXPERIENCED"* — a half-port wearing the new mechanism. The test now asserts that NONE of {adverse, SNOMED, patient, experienced, post, reaction} survives into the FiNER rendering, and it renders from `manifest.finer.json` rather than from slots invented in the test. Dependent slots (`vocabulary_concept`, `source_owner`, `claim_object`) are now DERIVED from the declared ones rather than defaulted, so a corpus that declares `vocabulary` cannot silently keep CADEC's "SNOMED CT concept"; `span_question`, `claim_verb` and `claim_object` are declared in `manifest.finer.json` because only the corpus knows whether its facts are reported, filed or observed — a filer does not *experience* a filed fact.
- 2026-08-29 — **FiNER-139 obtained and the adapter verified: the arm is reproducible from a clean checkout, which CADEC never was.** `finer139.zip` (98 MB) from `huggingface.co/datasets/nlpaueb/finer-139`, CC-BY-SA-4.0; the manifest's note that `load_dataset()` fails is correct and the zip is the route. The repo path in the manifest resolved to nothing in any worktree — the data had been lost with a pruned worktree — so this is the first time the arm has been rebuilt from its own instructions, and they worked. 1,121,256 sentences across train/validation/test; `corpus_finer.load_corpus` groups them into pseudo-documents at the natural tagged rate. Smoke over 5 test documents: 9 gold mentions, first three `('47.6', EffectiveIncomeTaxRateContinuingOperations)`, `('13.4', same)`, `('two', NumberOfOperatingSegments)` — the shape the adapter documents. `data/*` is gitignored so the 729 MB extraction is not tracked.
- 2026-08-29 — **DEFECT, fixed TDD: CADEC's exclusion list was applied to every corpus, including FiNER.** The first FiNER run printed `[run] gold exclusions applied: 414 (see data/exclusions.csv)` — 414 CADEC record ids, on a corpus whose ids all begin `FINER.`. `clean.load_exclusions()` defaults to `data/exclusions.csv` and all three call sites (`run.py` x2, `trim.py`) called it with no argument, so the corpus never entered the decision. Nothing was actually dropped, because no FINER id collides with an ARTHROTEC or LIPITOR one — but an exclusion list is a claim about ONE answer key ("these gold mentions cannot be answered, 3 carry only invalid codes and 4 quote text that is not at their offsets"), and inheriting another corpus's is a category error that one colliding id turns into silent gold loss. `clean.exclusions_for(man)` now resolves in declared order: an explicit `manifest.corpus.exclusions` path wins; an explicit `null` means nothing; absent falls back to CADEC's file ONLY when `corpus.adapter` is cadec, and to nothing otherwise. **A second corpus must OPT IN, because it cannot inherit a judgement made about someone else's gold.** Verified both ways: `exclusions_for` returns 414 for `manifest.json` and 0 for `manifest.finer.json`, the CADEC dev baseline reproduces unchanged (exact 0.399 / overlap 0.469, all seven figures), and the run banner no longer names a CADEC path on a non-CADEC run. Three tests.
- 2026-08-29 — **FiNER rung 0 runs clean now — the "5 of 10 documents failed json_decode" from the port session did NOT reproduce.** Smoke over 5 dev documents through the full runner: 51 records extracted, **zero** parse failures, truncations or timeouts, every record carrying a tag. 226.8 s for 5 documents (~45 s/doc) against CADEC's ~16 s for 40 from cache. The likely cause of the original failures is the `timeout_s` 300 -> 900 raise that main made for exactly this reason; recorded as "did not reproduce" rather than "fixed", because the failing artifacts are gone with the pruned worktree and no before/after was measured on the same machine state.
- 2026-08-29 — **PREDICTION FALSIFIED, and the mechanism is better for it: `qwen3:8b` is a REASONING model and it is BIT-REPRODUCIBLE.** Registered 2026-08-28: "mistral (dense) should be reproducible, qwen3 (reasoning) should NOT be." Mistral was right; qwen3 is wrong. Two independent draws of `qwen3:8b` produce the identical sha256 (`aa405fc6a1903d7a`) and identical figures (exact 0.141, overlap 0.177, det 0.212/0.318). So **"reasoning model" is NOT the variable**, and the explanation has to be narrower. The one that survives all four models is SPARSITY, not deliberation: `gpt-oss:20b` is a Mixture-of-Experts model (21B total, ~3.6B active) and is the ONLY nondeterministic one; `qwen3:8b`, `llama3.1:8b` and `mistral:7b-instruct` are all DENSE and all bit-reproducible, reasoning or not. That matches the known mechanism — expert routing depends on batch composition, so which experts fire varies with what else the server is batching, while a dense forward pass at temperature 0 does not. **Stated at its real strength: three dense models agree, and the MoE side has n=1.** One model is not enough to attribute a mechanism, and the honest form of the claim is "the only nondeterministic model here is also the only sparse one, which is consistent with routing and not established by it." Recording the falsification rather than quietly re-fitting the story to the data is the entire reason the prediction was written down first.
- 2026-08-29 — **THE BOUNDARY CONDITION ON H4, and it is a corpus boundary, not a model one: FiNER has NO ACCEPT LANE AT ALL.** H4 — the deterministic ACCEPT/BAND split identifies a subset of answers that are ~80-85% correct whatever model produced them — was registered and confirmed across three model families on CADEC (ACCEPT 84.6 / 80.4 / 83.3 against headline F1 ranging 0.401 to 0.206). It was never registered across CORPORA, and on FiNER it does not merely weaken, it **cannot fire**: over the 51 smoke records rung 1 returns **BAND 51 of 51, ACCEPT 0**, with `lexical_match` False on all 36 records carrying a tag (the other 15 answered CONCEPT_LESS, so no lookup ran). `sct_exists` is True for all 36 — the vocabulary check works fine; it is the LEXICAL check that has nothing to compare. The mechanism requires the source's words and the vocabulary's words to be the same kind of thing. CADEC: "chronic pain" against |Chronic pain|. FiNER: **"47.6" against |EffectiveIncomeTaxRateContinuingOperations|**, and `"200,000"` against a tag name — a number and a name share no tokens by construction, so the free check that carries the ENTIRE signal on CADEC has zero coverage here. **The honest scope of the article's central claim is therefore: deterministic evidence of this kind is available when the vocabulary's words and the source's words are drawn from the same language, and is unavailable when the span is a bare quantity.** That is not a weakness of the method so much as its precondition, and it was invisible for five phases because the project only ever had one corpus. What a FiNER-appropriate free check would be — magnitude, unit, sign, or scale agreement between the number and the tag's expected type — is real future work and was not built. Measured on the 5-document smoke (`out/finer/finer-smoke-1`); the 40-document run is in flight and will confirm the coverage figure rather than change the argument.
- 2026-08-29 — **DEFECT, fixed TDD: models were resolved lazily per rung, so a mistyped model name cost the ENTIRE run's compute before it surfaced.** The first full FiNER run spent **2,035 s in rung 0 and 5,947 s in rung 3 — 133 minutes — and then died at rung 4** with `openai.NotFoundError: model 'granite4:micro-h' not found`: `manifest.finer.json` named the judge `ollama/granite4:micro-h` where the installed model carries the namespace, `ollama/ibm/granite4:micro-h`. Nothing was wrong with the ladder. It learned the name was bad as late as it possibly could. **Only a second corpus could find this** — `manifest.json`'s judge name was always right, so five phases of CADEC runs never exercised the failure. `llm.check_models(manifest, rungs, available=None)` now resolves every model the run needs BEFORE rung 0 starts and `run_ladder` refuses with the available list and a "did you mean" hint. Three properties the tests pin, each of which is a way the check could have been worse than the bug: only the rungs ACTUALLY RUNNING are checked (`--rungs 0,1` must not fail on a judge it never calls); a rung with `enabled: false` is skipped (caught by the existing `test_a_disabled_rung_is_recorded_not_silently_skipped` the moment the preflight was wired in — a preflight that ignored a recorded run state would turn a deliberate configuration into a hard failure); and an unreachable provider returns `None`, not an empty list, so "the server did not answer" can never be read as "the server has no models" and stop a run that would have worked. Same rule as the timeout and reply-shape repairs: **one bad thing costs one thing, not the run.** Verified end to end — a deliberately mistyped judge now refuses in under a second, naming rung 4, the bad spec and the nine installed models.
- 2026-08-29 — **THE FULL LADDER ON A SECOND CORPUS SHIPS NOTHING. Coverage 0.0, 351 of 351 records routed to a person** (`out/finer/finer-full-2.*`, FiNER-139 dev, 40 documents, 351 records, all seven rungs, one run id). This is the 5-document smoke confirmed at full scale and it is not a degradation, it is a floor: rung 1 returns **ACCEPT 0 / BAND 350 / REJECT 1**, with `lexical_match` False on all 328 records carrying a tag and `sct_exists` True on all 328 — the vocabulary check works perfectly, the lexical check has nothing to compare. Rung 5 abstains BAND, as configured on CADEC and unchanged here, so it withdraws **every answer**, and rung 6 queues all 351. **The identical ladder that ships 21% of its answers on CADEC ships 0% on FiNER**, because its single load-bearing signal cannot fire when the span is `"47.6"` and the label is `|EffectiveIncomeTaxRateContinuingOperations|`. Per-rung answered-accuracy: rung 0 **0.1396**, rung 3 **0.1567** (net POSITIVE here, the opposite sign to CADEC's 0.371 -> 0.367 — one draw, and rung 3 numbers are samples, so it is a difference to note and not to claim), rung 4 no change as always. Cost, three measures apart: 2,532 tokens/record at rung 0, **6,813 at rung 3** (2.7x, the same ratio as CADEC), 873 at rung 4; p95 2.4 s at the judge; 351 records to a person. **A reliability system whose safety property is "abstain unless corroborated" degrades to "abstain always" the moment its corroboration mechanism is inapplicable — and it does so silently, reporting a perfect error rate over an empty output.** err_per_100 at rung 5 is 0.0 because nothing shipped. That number is the argument for reporting coverage beside every error rate, and it is the sharpest thing either corpus produced.
- 2026-08-29 — **The ported rung 4 judge ENGAGES with codes on FiNER, which it never did on CADEC — the first evidence that the judge's uselessness was a vocabulary problem, not a capability one.** With the prompt corrected (it previously asked an SEC filing about adverse reactions), granite4:micro-h returns fail 299 / pass 48 / 4 unjudged over 351 records, decomposing to (span_ok, code_ok) = (False,False) 192, (True,False) 102, (True,True) 48, (False,True) 5. So it judges the span wrong 197 times and the code wrong 294 times — it is answering BOTH questions. The 2026-08-23 CADEC finding was that the judge "engages with whether the reaction was really reported and says nothing about whether the code is a real SNOMED concept", which was read as a limit of a 3.2B model. On a 139-tag vocabulary the same model adjudicates codes, because the entire answer space fits in its context and it can actually check. **A judge cannot adjudicate a vocabulary it cannot see**, and that is a property of the task, not of the judge. It still changes no shipped number, because nothing reads `r4_verdict`.
- 2026-08-29 — **DEFECT, fixed TDD: the PICK call crashed the whole run on a reply shape the FIND call has accepted since it was hardened.** All three `granite4:micro-h` extractor draws died at `r0.py:1283` with `AttributeError: 'list' object has no attribute 'get'` — `_decide_batch` did `picked.get("picks", [])` and the model answers with a bare JSON array. `_as_mentions` has coerced exactly this shape on the FIND path for weeks; the PICK path was the one call site that never got the treatment, so the repo's own stated rule — **a reply shape nobody anticipated costs ONE DOCUMENT, not the run** — held on one half of rung 0 and not the other. `_as_picks(parsed, meta)` now mirrors `_as_mentions` exactly: a dict passes through, a non-empty list of dicts becomes `{"picks": [...]}` with `meta["shape_coerced"]` SET so the alternative shape is counted rather than hidden, and anything else returns None to be counted as a pick parse failure. Never raises. **The CADEC baseline is unmoved** — exact 0.399 / overlap 0.469, all seven figures — because gpt-oss:20b never used the other shape, which is precisely why five phases of CADEC runs could not find this. Two tests, one on the helper and one driving the real `_decide_batch` that actually crashed. Third defect this session whose only witness was a model or a corpus other than the one the project was built on.
- 2026-08-30 — **THE FIVE-MODEL TABLE IS COMPLETE, three draws each, and H4 is the strongest result the project has: the ACCEPT lane sits at 80-90% across five model families whose headline F1 spans a factor of 2.8.** Dev, 40 documents, identical frozen rung 0 config, `sample_index` 0/1/2, coding accuracy on overlap-matched spans.

  | model | exact F1 | overlap F1 | det ov | coding ov | **ACCEPT** | BAND | ratio | preds |
  |---|---|---|---|---|---|---|---|---|
  | gpt-oss:20b (MoE) | 0.401 ±0.007 | 0.472 ±0.003 | 0.788 | 0.599 | **84.6** | 35.9 | 2.36 | 232 |
  | llama3.1:8b | 0.336 ±0.000 | 0.395 ±0.000 | 0.745 | 0.530 | **80.4** | 28.8 | 2.79 | 214 |
  | mistral:7b-instruct | 0.206 ±0.000 | 0.264 ±0.000 | 0.685 | 0.386 | **83.3** | 14.6 | 5.70 | 259 |
  | granite4:micro-h (3.2B) | 0.185 ±0.000 | 0.220 ±0.000 | 0.637 | 0.345 | **89.3** | 14.6 | 6.12 | 292 |
  | qwen3:8b | 0.141 ±0.000 | 0.177 ±0.000 | 0.318 | 0.556 | **83.3** | 30.3 | 2.75 | 57 |

  Headline exact F1 ranges **0.141 to 0.401**; the ACCEPT lane ranges **80.4 to 89.3**, an 8.9-point band with no ordering relationship to model quality — the WORST model by F1 (granite, 0.185) has the HIGHEST ACCEPT lane (89.3%) and the highest ratio (6.12). The BAND lane, by contrast, tracks model quality as it did over three models. So the free check does not merely correlate with correctness: **it identifies a subset of answers that are ~85% correct no matter which of five open-weight families produced them, and it earns MORE the worse the model is.** That is the article's central claim, now measured across five families and fifteen runs rather than asserted from one.
- 2026-08-30 — **DETERMINISM, final: four of five models are BIT-REPRODUCIBLE and the only one that varies is the only Mixture-of-Experts model.** Distinct sha256 over three draws: llama3.1:8b **1**, mistral:7b-instruct **1**, qwen3:8b **1**, granite4:micro-h **1**, gpt-oss:20b **3**. qwen3 is a reasoning model and granite4:micro-h is a Mamba/transformer hybrid, so neither deliberation nor exotic architecture is the variable — sparsity is the one property that separates the four reproducible models from the one that is not. **Stated at its real strength for the last time: n=4 on the dense side, n=1 on the sparse side.** One MoE model cannot establish a mechanism, and the honest form remains "the only nondeterministic model here is also the only sparse one, which is consistent with expert routing varying by batch composition and is not established by it." The practical consequence is unchanged and does not depend on the mechanism: **run-to-run reproducibility is a model-selection choice with a measurable price** — gpt-oss:20b buys 6.5 exact points over llama3.1:8b and pays a 1.3-point run-to-run spread for them, and that spread is larger than every rung 0 improvement this project ever shipped.
- 2026-08-30 — **qwen3:8b answers less and is right more often, which is the case for never reporting a single F1.** It is LAST by headline exact F1 (0.141) and SECOND-BEST at coding (0.556 overlap, behind only gpt-oss's 0.599 and ahead of llama's 0.530 and mistral's 0.386), because it emits **57 predictions against gpt-oss's 232** — detection overlap 0.318 against 0.788. A single headline number ranks it below two models it beats at the part of the task everyone assumes is hard. It is also by far the most expensive: **7,493 / 7,375 / 5,550 seconds per draw**, roughly two hours each against llama's ~15 minutes, for a quarter of the output. Cost stays three separate measures and this is why.
- 2026-08-30 — **FIXED: the declared configuration is now the measured one. `manifest.json` reproduces the baseline exactly, and the workaround copy is gone.** The five rung 0 arms measured and accepted on 2026-08-27/28 (`rung0_split`, `rung0_drop_ungrounded`, `rung0_drop_fragments`, `rung0_drop_duplicate_spans`, `rung0_cut_rate: 0.06`) were appended to `manifest.rungs.0`, each with its own note carrying its measurement and its provenance. Rung 0 run from the TRACKED manifest with no cfg override now gives **exact 0.399 / overlap 0.469, det 0.516/0.785, coding 0.773/0.597, 92/108 correct, 235 predictions** — all seven figures, against 0.340 / 0.449 / 233 before. **No published number changes**, and that was verified rather than assumed: the full ladder re-run straight from `manifest.json` (`out/cadec-manifest-1`) produced a records file with the **identical sha256** (`f76de5186eb1ad9e1ec5`) to `cadec-verify-1`, the run behind every CADEC figure in `docs/article.md`. So the article was always reporting the arms-on configuration; only the tracked manifest disagreed. `out/manifest-audit.json` — the run copy that existed solely to work around the manifest being wrong — is DELETED from the working tree (the archived copy under `out/archive/` is left as the record of how `audit-full-dev-1` was produced), because leaving a run copy behind means the next session inherits a workaround nobody remembers the reason for, which is how the 5.9-point gap happened in the first place. Two tests pin it: one asserts every measured arm is declared EXPLICITLY in the manifest (a manifest that merely happens to agree with `r0.DEFAULTS` breaks the moment a default moves, and the default is what moved last time), and a second asserts the arms still DIFFER from the defaults, so the first cannot pass vacuously.
- 2026-08-30 — **DECIDED: the five arms are NOT ported to `manifest.finer.json`, and the refusal is the point.** Two of them are CADEC-specific by construction — `rung0_split` is a coordination splitter learned on clinical phrases, and `rung0_cut_rate` tunes a trimmer whose rules are LEARNED FROM POOL GOLD AT RUNTIME — and FiNER's spans are bare numbers ("0 of 407 begin with a currency symbol", per its own prompt rules). The three `drop_*` arms are generic and would probably transfer. Porting any of them unmeasured would declare as measured something that was never measured on this corpus, which is precisely the failure just fixed above, committed a second time in the same hour. FiNER keeps declaring its own deviations explicitly (`rung0_retrieval: full`, its own few-shot documents, its own prompt slots), which is the pattern that made the arm gap visible on CADEC. `finer-full-2` therefore stands as run: it already carries the ported rung 4 prompt and the corpus-scoped exclusions, and nothing in its configuration changed.
- 2026-08-30 — **FiNER detection improved +2.5 F1 at ZERO recall cost, and the fourth candidate filter was rejected for the usual reason.** Asked why FiNER's detection fails, the 238 false-positive spans decompose: **53.4% are numbers in a tagged document that are simply untagged here, and 23.5% are literals that ARE tagged elsewhere in the corpus** — 77% between them, and neither is a model error. FiNER gold tags a number only if the filer chose to XBRL-tag it AND that tag is among the 139 most frequent of thousands, so *"is this number an XBRL fact with one of 139 tags"* **is not decidable from the text**. The model marks `19.5 -> Revenues`; gold does not have it; both are defensible. That is the same shape as CADEC's near-miss blindness — the information needed is not in the source — and it is a ceiling, not a bug. The remaining 23% IS fixable and was priced against gold destroyed, the same discipline as the CADEC span filters: **duplicate span 39 predictions cut / 0 gold**, **bare year 22 / 0**, **clock time 4 / 0**, and **no-digit 14 / 7 GOLD LOST — REJECTED**, because FiNER spells small counts out (`"two" -> NumberOfOperatingSegments`). Free filters ship, the one with a false-rejection cost does not. Shipped as `rung0_drop_datelike` (new arm, off by default, FULLMATCH so `"2018 revenues"` survives) plus `rung0_drop_duplicate_spans`, which already existed and had simply never been declared for this corpus. Measured through the production path (`out/finer/finer-full-3`): **351 -> 292 predictions, precision 0.142 -> 0.171, recall UNCHANGED at 0.303, F1 exact 0.190 -> 0.214 / overlap 0.194 -> 0.219, and all 49/50 correct answers preserved.** CADEC is untouched — the arm is off there and its baseline reproduces unchanged. **The ceiling stands: even perfect false-positive removal caps FiNER at F1 ~0.465, because recall is 0.303 and precision filters cannot touch the 70% of gold the model never proposes.** Rung 5 still withdraws everything (292 of 292), because none of this touches the lexical check that has nothing to compare.
- 2026-08-30 — **Merged `origin/main` (the FiNER three-model arm) a second time. One conflict, and resolving it exposed a defect NEITHER branch had alone.** Both branches had added a transport normalisation to the same line: this branch's `_unwrap` (lift JSON out of surrounding prose, counted in `prosed`) and main's `_unwrap_array` (wrap a bare array in the expected object, counted in `unwrapped`). They are complementary and both are kept, ordered `_unfence -> _unwrap -> _reclose -> _unwrap_array`, because a model can do all four at once. **The defect is in the interaction:** `_unwrap_array` wrapped every bare array as `{"mentions": [...]}`, which is right for a FIND reply and wrong for a PICK one, where `_decide_batch` reads `"picks"` and would get `[]` — so main's fix silently converted this branch's granite crash into a **no-op that produces no picks and reports nothing**, which is strictly worse than the crash. It could not be fixed by reading `mode`, because the FIND and PICK calls of one step passed the SAME mode string (`"S2"`). The pick call now passes `f"{step}-pick"`: `mode` is NOT in the cache key (model, messages, temperature, sample_index, max_tokens, reasoning_effort), so this cost no cached call, nothing in the repo branches on the extraction mode string, and the ledger now says which of the two calls a row is. `_unwrap_array` also now refuses to wrap a list of non-dicts — wrapping `[1,2,3]` turns unparseable garbage into a parseable EMPTY result and deletes the parse-failure signal, the same rule the parse-layer coercions already follow. **A second, smaller collision:** main extended `_unfence` with a non-anchored fence search (`preambled`), measured on FiNER where llama3.1:8b put a sentence before the fence on 60 of 60 documents — which recovers the same reply this branch's prose repair was written for, earlier in the chain. `tests/test_llm_unwrap.py` was asserting the old path and is updated to the new one, plus a new test pinning the case only the prose repair handles (prose around UNFENCED JSON), so it cannot quietly become dead code. Its `_Bare` stub now DISCOVERS the repair counters from `ladder/llm.py` instead of listing them, because a hand-kept list is what broke on this merge. 690 tests green; the CADEC dev baseline reproduces unchanged at exact 0.399 / overlap 0.469 on all seven figures.
- 2026-08-30 — **THE SPINE ABLATION, and it settles the audit: dropping rungs 2, 3 and 4 costs ONE overlap-matched correct answer out of 43, and saves 76% of the token budget.** `manifest.spine.*` runs `rung_order: [0, 1, 5]` — the deterministic spine, with self-correction, voting and the LLM judge removed. **CADEC, dev, rung 0 held IDENTICAL on both sides:** full `[0..6]` F1 exact **0.182** / overlap **0.187**, 42 exact-correct, 43 overlap-correct, 52 shipped, 196 to a person, **683,488 tokens**, err/100 4.03; spine `[0,1,5]` F1 exact **0.182** / overlap **0.182**, 42 exact-correct, **42** overlap-correct, 52 shipped, 196 to a person, **164,898 tokens**, err/100 4.03. Every shipped number is identical except one overlap-matched answer, **bought for 518,590 tokens and a 152-second p95**. **FiNER:** both stacks ship nothing (coverage 0.0, 292 to a person); the spine costs 25% of 3,536,073 tokens. This is the audit's inference turned into a measurement: the per-rung deltas said the paid model rungs do not pay, and removing all three at once confirms it directly rather than by argument. **A CONFOUND WAS CAUGHT BEFORE THE RUNS, and it would have inverted the result's meaning:** the spine manifests were written before the five rung 0 arms were appended (2026-08-30), so running them as they stood would have compared a `[0,1,5]` stack on a rung 0 scoring **0.340** against a `[0..6]` stack on a rung 0 scoring **0.399**, and charged that 5.9-point difference to the three rungs being dropped — an ablation that is really two different rung 0s wearing an ablation's clothes. Rung 0 was synchronised across both manifests first, verified identical key-by-key, and the sync is recorded in each file's `_ablation_sync_note`. Runs: `out/spine-cadec/spine-cadec-1.*`, `out/spine-finer/spine-finer-1.*`.
- 2026-08-30 — **PRE-REGISTERED, with the two draws already seen stated rather than hidden: the decision rule for the three-draw debt on `rung0_split` and `rung0_cut_rate`.** `manifest.json` declares five rung 0 arms; three (`drop_ungrounded`, `drop_fragments`, `drop_duplicate_spans`) are deterministic filters whose effect is a count of dropped predictions and needs no draw, but `rung0_split` and `rung0_cut_rate: 0.06` were each accepted on **ONE DRAW plus a paired bootstrap** — which is exactly the protocol that certified the reranker's `+0.0215 [+0.0000, +0.0433]` before its sign reversed on the third run. Protocol: nine runs, `{base, base−split, base−cut} x sample_index {0,1,2}`, base being `manifest.json` as tracked, each arm removed LEAVE-ONE-OUT (the decision is "does this earn its place in the shipped config", not "does it help a stripped one"). `rung0_cut_rate: null` is the off side and it is NOT vacuous — it falls back to `trim.DEFAULT_CUT_MAX_RATE = 0.02`. **The rule, fixed before draw 2 of the split arm existed (draws 0 and 1 of both arms had already printed, and are quoted in the result entry):** an arm SURVIVES only if the leave-one-out delta has the SAME SIGN on all three draws on the layer it was sold on; a sign reversal removes it from the manifest, as it removed the reranker. The paired bootstrap is reported per draw and prices the corpus sample only — it is the secondary number, and by the 2026-08-27 finding it cannot see the run-to-run term at all. An arm that survives on exact but not on overlap is recorded as surviving on exact, with the overlap number printed beside it; neither layer is dropped.
- 2026-08-30 — **`ladder.score.paired_bootstrap` is now PRODUCTION code, TDD-first, because the scratch harness every rung 0 arm was measured with is not a bootstrap.** `out/harness/paired.py` resampled with `set(random.choices(ids, k=len(ids)))` — the `set()` collapses the duplicates a bootstrap draw is MADE of, leaving a ~63% subsample and a different, wider estimator wearing a bootstrap's name. The new function draws documents with replacement, keeps multiplicity, scores BOTH arms over the SAME resampled documents (an unpaired comparison of two intervals discards the pairing that makes a 40-document split usable), and reports `point / mean / lo / hi` for all three layers. Six tests, including one that pins the defect directly: on a corpus where one document carries all the signal, a with-replacement draw must put mass ABOVE the point estimate and a set-collapsed draw cannot. `bootstrap_ci` was refactored onto the same `_per_doc_stats` / `_layers` / `_pct` helpers and its output over `arm-debtbase-d0-dev` is **byte-identical before and after**, verified rather than assumed. 696 tests green.
- 2026-08-30 — **THE THREE-DRAW DEBT IS PAID: both one-draw arms SURVIVE, and the split's overlap claim does NOT.** Nine dev runs (`out/arm-debt{base,nocut,nosplit}-d{0,1,2}-dev.*`, driver `out/harness/debt.sh`, analysis `out/harness/debtpaired.py`), leave-one-out from `manifest.json` as tracked, three `sample_index` draws each, paired bootstrap over documents at 2,000 replicates. **`rung0_split`, F1 exact: +0.0389 / +0.0345 / +0.0579**, three draws for three, pooled mean **+0.0438 [+0.0012, +0.0937]** — the sign never reverses, the pooled interval excludes zero, and it is the largest rung 0 arm in the project. **`rung0_cut_rate: 0.06`, F1 exact: +0.0139 / +0.0183 / +0.0098**, pooled **+0.0140 [−0.0163, +0.0500]**; overlap +0.0054 / +0.0054 / +0.0011, pooled **+0.0039 [+0.0000, +0.0103]** — six sign-consistent comparisons out of six, but every per-draw interval and the pooled exact interval contain zero. By the pre-registered rule (sign consistency on the layer the arm was sold on) BOTH survive, so **neither is removed and no CADEC number is re-run** — but the two survive on different strengths and are recorded that way: the splitter is separated, the trimmer's threshold is *consistent and small*, sitting inside the corpus-sample band on its own layer. **The finding the one-draw measurement had wrong: `rung0_split` buys EXACT ONLY, and its overlap effect reverses sign** (+0.0000 / −0.0047 / +0.0229, pooled +0.0061 [−0.0267, +0.0395]). That is exactly what the mechanism predicts — cutting a coordinated quote into pieces converts one already-overlap-matched blob into several exactly-matched spans, so overlap has nothing to gain — and it is a claim the original entry did not make but the manifest note implied by quoting a single headline. Also measured incidentally: **the base itself moves 0.399 / 0.395 / 0.408 exact across the three draws**, a 1.3-point spread, so `rung0_cut_rate`'s whole effect is smaller than rung 0's own run-to-run noise. Both manifest notes are amended to carry the three-draw provenance rather than the one-draw provenance they declared.
- 2026-08-30 — **RUNG 4'S FATE, DECIDED BY THE OWNER AND THEN MEASURED: wire it to a reader as an off-by-default arm, run it, and leave it OFF — it buys 1 point of selective precision by destroying 26% of the correct answers.** The 2026-08-28 audit found rung 4's `checks["r4_verdict"]` had no reader anywhere in `ladder/`, so ~93k tokens per run could not move a shipped number by construction. Three options were put to the owner (wire-and-measure / disable / keep as a declared diagnostic) and the call was **wire it as an arm and measure whether it adds anything**. Shipped TDD-first: `R_JUDGE_FAIL` appended to `schemas`, `rungs.5.abstain_on_judge_fail` (default **false**, declared explicitly in `manifest.json`, one-key-diff copy in `manifest.judgearm.json` with a test asserting the diff is exactly that key), and `r5.decide()` now withdraws a record the free check would have SHIPPED when the second-family judge failed it. The arm may only SUBTRACT: a record already heading for ABSTAIN keeps `unresolved`, and a missing or unparsed verdict is never read as a failed one — otherwise disabling the judge would abstain the entire run. **Measured over three rung 0 draws** (`--rungs 1,4,5 --predictions out/arm-debtbase-d{0,1,2}-dev.records.jsonl`, six runs in `out/judgearm/`): coverage **0.210 / 0.202 / 0.215 → 0.153 / 0.149 / 0.156**; selective precision **0.808 / 0.800 / 0.824 → 0.816 / 0.811 / 0.838**; **yield (correct over ALL records) 0.169 / 0.161 / 0.177 → 0.125 / 0.121 / 0.131**; records routed to a person **196 / 198 / 186 → 210 / 211 / 200**. **The judge withdraws 14 / 13 / 14 shipped answers to remove 3 / 3 / 3 errors** — it destroys roughly **3.7 correct answers per error caught**, and its discrimination lift (P(wrong | judge withdrew it) / P(wrong | shipped at all)) is **1.11x / 1.15x / 1.21x**. On the SAME records the free lexical check separates **3.15x / 3.09x / 3.03x** at zero tokens. Precision rose, which is what abstaining always does; yield is the number that cannot be fooled by abstaining and it FELL 26%. **The arm stays off, and the question is now answered with a measurement instead of an inference.** Analysis `out/harness/judgearmcmp.py`; a defect in its first version is worth recording — scoring the post-rung-5 records directly reported the BAND lane as 0.000 correct, because rung 5 sets `rec.sct = None` and preserves the answer in `checks["withheld"]`, so any lane-level analysis over a finished run must restore the withheld answer first or it will measure rung 5 instead of the lane.
- 2026-08-30 — **The published artifact is no longer a build log. `docs/article.html` is the typeset ARTICLE and it now serves the existing artifact URL; the ladder-framed page is archived as v2.** The live artifact had been the build log (`docs/article-build-log.html`, last updated 2026-08-28) while the deliverable had moved to `docs/article.md` and its visibility-question framing on 2026-08-28 — so the one thing a reader outside the terminal could see was the superseded framing. Rewritten as ten numbered sections in the project's existing design system (the same tokens, faces and zone colours the build log used — honoured, not replaced, because the ACCEPT/BAND/REJECT colours carry meaning), republished IN PLACE so the link survives, and the outgoing page moved to `docs/versions/article-build-log-v2-2026-08-28.html` under the convention that already existed for this. The build log's `Reproducing` command block was carried across rather than dropped — the only content on the live page not already in the article. `docs/article-iterations.md` keeps the build-log prose and its header now says what the artifact serves. **`docs/article_v3.md` is the article revision this session produced**: `docs/article.md` plus four new results (the three-draw debt paid on our own two arms including the set-collapse bug in the bootstrap helper; the judge arm wired and measured; FiNER recall decomposed into detection 0.685 x coding 0.446; the refused document), with new material marked in the typeset version so a reader who has seen revision 2 can find what changed. One build defect worth recording because it is the class the design guidance names: the head was assembled by a `str.replace('</style>', ...)` against a slice that did not contain `</style>`, so the CSS silently swallowed the whole body and the page rendered blank — caught by opening it in a browser before publishing, which is the only way it could have been caught.
- 2026-08-30 — **FiNER RECALL, DIAGNOSED BEFORE ANYTHING WAS TRIED, and the premise in the handoff is wrong: the model does NOT fail to propose 70% of gold.** `out/harness/finerrecall.py` over `out/finer/finer-full-3`, 40 dev documents, 165 gold, 292 predictions. **Detection recall 0.685 (exact 0.679); coding accuracy on matched spans 0.446; 0.685 x 0.446 = the 0.303 headline.** The model REACHES more than two thirds of the gold spans and mis-codes most of what it reaches, and it proposes 292 spans against 165 gold (23.4% of the 1,247 numeric tokens in the dev text against gold's 13.2%) — so it is not under-extracting at all and the miss is WHICH numbers, not how many. On a corpus where the whole 139-tag vocabulary is in the prompt and the right tag is therefore ALWAYS on the menu, recall work is CODING work. Decomposing the 52 never-touched gold: **21 of them (40% of the entire detection gap, 12.7% of dev gold) are in ONE document**, the refusal above; 33 of 52 (63.5%) are literals the model proposed somewhere else in the corpus, which is the same "not decidable from the text" ceiling the false-positive decomposition found from the other side; by shape they are 75% decimals and 11.5% spelled-out numbers. Most-missed tags are the interest-rate and ownership-percentage families (`DebtInstrumentInterestRateStatedPercentage` 10 of 15). **The lesson for the article is the decomposition itself:** one recall number for a pipeline that both finds and classifies sends the effort to the wrong half.
- 2026-08-30 — **OFFLINE PROBE FIRST, and it is why the context arm was built at all: the 139-tag menu can be RANKED by the words around the number, at median rank 7.** `out/harness/finerctx.py`, all 165 dev gold mentions, granite-embedding:30m over de-camel-cased tag names against the 120 characters either side of the span. **recall@1 0.242, @3 0.394, @5 0.430, @10 0.558, @20 0.685, @40 0.800, @139 1.000 (by construction); median rank 7, mean 20.6.** Real signal — and NOWHERE NEAR enough to justify TRUNCATING the menu to 20, which would cap coding accuracy at 0.685 to fix a pick that currently runs at 0.446. So the arm reorders and drops NOTHING: `rung0_menu_order: "context"` (new third mode, `ladder/menuorder.py`, off by default in BOTH manifests, `manifest.finer.ctxmenu.json` is a verified one-key diff). Menu recall stays 1.000 and the arm tests exactly one thing — whether best-first order is worth anything when the query is CONTEXT rather than span. It is the direct application of two measured CADEC facts: alphabetising the pick menu cost 10-12pt of coding accuracy at identical detection because the pick anchors on early slots, and k=40 picked worse than k=20 — and FiNER's menu is 139 long in no order at all. The embedder is built in `r0.prepare` ONLY when the arm is on and is memoised, because 139 tag vectors recomputed per mention is ~40,000 embedding calls for one run to produce the same 139 vectors every time; a CADEC run never touches an embedding server it does not use.
- 2026-08-30 — **THE REFUSAL IS NOT THE DOCUMENT AND IT IS NOT THE MODEL — IT IS THE DRAW. Same model, same prompt, same document, temperature 0: one draw refuses and two answer.** `out/harness/finerrefusal.py` on `FINER.test.0013`, the document holding 21 of FiNER's 165 dev gold mentions. **`ollama/gpt-oss:20b` d0 -> "I'm sorry, but I can't provide that." (2,153 tokens, the run's own cached completion); d1 -> 33 mentions; d2 -> 33 mentions.** And every other family answers on its single draw: `llama3.1:8b` 38 mentions, `mistral:7b-instruct` 22, `granite4:micro-h` 9. One draw each for those three because all three are BIT-REPRODUCIBLE (2026-08-30) and three identical draws of a bit-reproducible model are three copies of one measurement. `qwen3:8b` is EXCLUDED and the exclusion is stated rather than hidden: it held the GPU for 45 minutes on this one document without returning, consistent with its measured ~2h-per-draw cost — a cost finding, not a refusal one. **What this does to the nondeterminism result is the point.** The 2026-08-28 measurement said gpt-oss:20b is the one non-bit-reproducible model in five and pays a 1.3-point run-to-run spread for 6.5 points of headline. That spread is an average over records and it UNDERSTATES the risk, because the variance is not spread evenly: it concentrates into whole-document, all-or-nothing outcomes, and one of them here is **12.7% of the corpus's gold and 40% of the entire detection gap, decided by which draw you got**. A bit-reproducible model cannot do this — it can be wrong, but it cannot answer on Tuesday and refuse on Wednesday. That is a second, sharper price on the same model-selection choice, and it is the one an operator would actually feel.
- 2026-08-30 — **REJECTED: the context-ordered FiNER menu. It carries real signal, the model FOLLOWS it, and following it is WORSE than the model's own unaided reading.** `manifest.finer.ctxmenu.json`, one draw (`out/finer/arm-finerctx-d0`, 4,660 s of compute), against the same-draw baseline `arm-finerbase-d0`. Detection is byte-identical by construction — 292 predictions, det F1 0.490, det recall 0.679 both sides, since nothing is dropped and only the order changes — so the whole difference is the pick: **coding accuracy 0.393 -> 0.304, F1 exact 0.193 -> 0.149, correct 44 -> 34.** The mechanism was then confirmed from the artifacts rather than inferred (`out/harness/finerctxdiag.py`): the pick lands on **menu slot 0 in 20.4% of records under the alphabetical baseline and 50.2% under the context ranking** (median picked slot 30 -> 0), so the model plainly follows the ranker. And the ranker IS informative — accuracy when slot 0 is picked rises 0.087 -> 0.373, and the offline probe's recall@1 was 0.242. **But the model's own judgement over a menu with no ranking in it scores 0.457** (base, records where it picked a slot other than 0). The arm replaced a 0.457-quality judgement with a 0.373-quality one by giving the model something to defer to. **A ranking can carry real signal, visibly move the model, and still make the system worse, because what it displaces was better than it.** That is the same shape as the 2026-08-28 reranker result ("works as a ranker, does not convert") reached from the opposite direction, and it completes the pair with CADEC's menu-order arm: destroying a GOOD order cost 10-12pt there, and imposing a MEDIOCRE order costs 8.9pt here. **Provenance stated, not smoothed: this is ONE DRAW and does not meet this project's three-draw standard.** It is reported as a rejection on an effect roughly seven times the CADEC run-to-run spread plus a mechanism verified on the artifacts, not as a separated measurement; FiNER's own run-to-run spread has never been measured. Making it a three-draw result needs FOUR more runs (base and arm at d1 and d2, since a paired comparison needs both sides at the same draw) at ~78 minutes each. The arm ships OFF in both manifests and nothing in `manifest.finer.json` changed.
- 2026-08-30 — **THE SLOT-0 ATTRACTOR: 19.5% of FiNER's predictions are the menu's FIRST ITEM, taken because it is first, and the decisive test is unambiguous.** Decomposing the other half of the recall gap (`out/harness/finermiscode.py`, the mirror of the false-positive decomposition): of 113 matched spans, **68 are mis-coded and ZERO are abstentions** — the model always names a tag here, unlike CADEC. The wrong tags are not near-misses: **79.4% share no leading word with gold and 64.7% have DISJOINT token sets**, and the confusion is a long tail (46 distinct gold->predicted pairs over 68 errors). But one tag dominates the over-predictions: **`AccrualForEnvironmentalLossContingencies` is predicted 57 times in 292 records and gold uses it TWICE** — and it is **menu slot 0**, because the FiNER menu is `sorted(set(tags))` and that tag is alphabetically first. **The position-vs-semantics discriminator was free, because the context-menu arm is exactly the experiment that moves it:** under alphabetical order it sits at slot 0 in every record and is predicted 57 times, **all 57 while at slot 0**; under context ranking its median slot is **92** and it is predicted **3 times, all 3 in the records where the ranking happened to put it first**. The model picks it IF AND ONLY IF it is first. That is position, not meaning, and it accounts exactly for the earlier diagnostic (slot-0 picks 20.4% ~= 57/279, slot-0 coding accuracy 0.087 because gold uses the tag twice). **This reframes the context-arm rejection into two competing effects rather than one failure.** The arm FIXED a real defect — it killed the attractor, 57 spurious predictions down to 3 — and it still lost overall, because it also AMPLIFIED the positional prior (slot-0 selection 20.4% -> 50.2%), moving mass off the model's own reading of the menu (0.457) and onto the ranker's top slot (0.373). Two real effects, opposite signs, net negative. **It also names the next intervention precisely, and it is not a better ranker: break the position prior rather than feeding it** — a slot 0 that is never a valid answer, or a per-mention permutation with a fixed seed — which is now the highest-value untried FiNER experiment. **And it composes with CADEC rather than contradicting it:** CADEC's menu is ordered by dense-retrieval score, so the same prior lands on the BEST candidate and is aligned with quality, which is precisely why alphabetising that menu cost 10-12 points. One prior, two corpora, opposite consequences, depending only on what the ordering puts first. Also found: the same literal is coded differently twice in one document in 4 documents, and 13 predictions are `CONCEPT_LESS` against 0 in gold.
- 2026-08-31 — **THE FiNER CONTEXT-MENU ARM IS REJECTED ON THREE DRAWS, and FiNER's first run-to-run spread turns out to be ONE REFUSED DOCUMENT.** Four runs finished (`out/finer/arm-finer{base,ctx}-d{0,1,2}.*`, driver `out/harness/finerdraws.sh`), base run first at each draw so the arm's find calls were cache hits. **Base: exact 0.193 / 0.205 / 0.205, coding 0.393 / 0.421 / 0.421, detection recall 0.679 / 0.691 / 0.691, correct 44 / 48 / 48. Context arm: exact 0.149 / 0.128 / 0.128, coding 0.304 / 0.263 / 0.263, correct 34 / 30 / 30.** Sign consistent three for three, and the effect is LARGER on the two clean draws (−0.077 exact, −0.158 coding) than on the draw that carried the refusal. Detection is byte-identical between the arms at every draw, as designed — the arm reorders and drops nothing, so it can only move coding. **The rejection stands and the arm stays off in `manifest.finer.json`.** THE SECOND RESULT IS THE MORE INTERESTING ONE: **d1 and d2 are BYTE-IDENTICAL** — same sha256 on the records file, for BOTH arms — and only d0 differs. So the whole of FiNER's 1.2-point exact spread is **one document**, the one `gpt-oss:20b` refused on draw 0 and answered on draws 1 and 2. This is the first run-to-run spread ever measured on this corpus, and it is not a distribution: it is a single all-or-nothing event worth 21 of 165 gold mentions. It also refines the determinism claim rather than contradicting it — `gpt-oss:20b` is the one non-reproducible model in the five-model table, and here it produced identical output on two of three draws, which is what batch-dependent MoE routing looks like when the batch happens to repeat. **Report FiNER's spread as "one refused document", never as "±1.2 points"** — the second phrasing implies a wobble spread across records and there is no such wobble here.
- 2026-08-31 — **CI RED ON A TEST THAT ASSERTED A BUILD ARTIFACT, and the failure was invisible on every developer machine.** MR !24's `tests` job failed on `test_cadec_still_gets_its_exclusions_by_default` — `assert len(got) == len(clean.load_exclusions()) > 0` — because `data/exclusions.csv` is written by `python -m ladder.clean --build` into a gitignored directory, so CI's clone has no such file and `load_exclusions()` correctly returns an empty set. The test passed locally, passed in a clean-dependency venv, and passed on the merged result; it fails ONLY where `data/` is absent, which is exactly where CI runs. The `> 0` clause was never the test's subject: the defect it names is corpus-scoping (a FiNER run must not inherit CADEC's exclusion list), and the CADEC half of that only needs "no `exclusions` key routes to the default path". Rewritten hermetically against `tmp_path` + `monkeypatch.chdir` with a two-line exclusions file, asserting the exact set — which also makes it STRONGER, since it now checks the content that comes back rather than a length. Confirmed discriminating by breaking `exclusions_for`'s cadec branch to `return set()` and watching the new test fail, then restoring. Verified under CI's own condition by moving `data/exclusions.csv` aside and running the suite in a fresh venv holding only `requirements.txt` + pytest: 692 passed, 14 skipped. **The general rule this is the second instance of: a test whose assertion depends on a gitignored build artifact is a test that cannot run where the guard is needed.** `tests/test_clean.py` already covers `load_exclusions` on an explicit path and on a missing file, so nothing was lost.
- 2026-08-31 — **REGISTERED BEFORE THE RUN: every prompt rung 0 will send BioMistral is above the EOS cliff, all 102 of them.** The 2026-08-25 Phase C rejection measured an instant-EOS threshold at ~430 prompt tokens (answers at median 324, EOSes at median 435, 500+ = 100% EOS) in the rung-4 JUDGE role. The extractor arm's prompt-token distribution is therefore registered in advance, exact counts from BioMistral's own tokenizer (`out/harness/biomistral_prompt_tokens.py`, results `out/biomistral-prompt-tokens.json`, sent straight to ollama at `max_tokens=1` so the probe cannot seed the run's cache): **S2 FIND, 40 dev documents — min 828, median 918, mean 936, p90 1050, max 1134. S2 PICK, 62 batched calls sized on gold spans — min 520, median 1404, mean 1479, p90 2112, max 2339. `over_430` is 40/40 and 62/62: 100.0% on both.** So the arm has a stated prior with no escape hatch: if the cliff is a property of the MODEL, this run returns nothing at all, on every document, and the shortest prompt in the whole split still sits at 1.9x the threshold. If instead the run produces mentions, the cliff was a property of the JUDGE HARNESS (rung 4 sent its post TWICE until the 2026-08-25 fix, measured at median 582 prompt tokens where once-only is ~360) and the 2026-08-25 claim narrows to one role. Registering the distribution first is what makes the null interpretable — a token distribution measured after an empty run is a distribution fitted to its own answer. Also changed, TDD'd: `models.yaml` resizes the entry from the judge-shaped 512/120 to **2000/180, matching `mistral:7b-instruct` exactly**, because at 512 a truncation and an instant EOS are the same observation and telling those two apart is the whole point of the arm.
- 2026-08-31 — **BioMistral-7B AS THE RUNG-0 EXTRACTOR: the run is a NULL, three draws, and the three draws are BYTE-IDENTICAL.** The registered TODO's protocol, run exactly as written: CADEC dev, 40 documents, frozen `manifest.json`, rungs 0-1, `--extractor ollama/biomistral:7b-q5_k_m` the only override, three draws each with its own COLD cache directory (`out/harness/bmdraws.sh`, runs `out/arm-biomistral-d{0,1,2}-dev.*`). Result: **3 predictions against 226 gold mentions, F1 exact 0.0087 / overlap 0.0087, detection recall 0.0088, 1 correct.** 36 of 40 documents came back `json_decode` with **zero** mentions; the whole split cost 39,611 prompt tokens and **621 completion tokens** — about 15 per document — in 27 seconds. The ACCEPT lane the five-model table is built on is **not measurable here**: rung 1 saw one ACCEPT and one BAND, so the arm contributes no row to H4 and must not be given one. All three draws have the **identical sha256** on the records file, which puts BioMistral with the four bit-reproducible dense models rather than with gpt-oss. **The failure reproduces the 2026-08-25 judge result exactly, down to the string**: the reply is `" {"` and then end-of-sequence. So the arm's registered prediction — "if the ~430-token cliff is real, rung 0 is where it bites HARDEST and the run will fail at step one" — is what happened.
- 2026-08-31 — **AND THE DIAGNOSIS IS THE RESULT, because the mechanism is NOT what was registered. It is not our harness, it is not truncation, it is not length as such, and at temperature 0.7 it goes away.** Before reporting this as a model result the 2026-08-28 rule was applied — a model comparison measures your harness until you prove otherwise — and all 40 FIND prompts were replayed through RAW httpx with no ladder code in the path (`out/harness/bmdiag.py`, `out/biomistral-diagnosis.json`). Four findings, in order of how much they change the claim. (1) **Not ours**: the raw replay answers 4 of 40, the same 4, so the ladder's transport is exonerated — and `finish_reason` is **`"stop"` on all 40**, never `"length"`, so this is a genuine EOS and not a budget the run could raise its way out of (the budget was already raised to 2000 for this arm). (2) **There IS a cliff and it is at ~856 prompt tokens, not ~430**: **0 of 31** prompts above 856 answered, against **4 of 9** at or below — and it is content-dependent right at the boundary, where ARTHROTEC.107 answers at 856 and ARTHROTEC.139 EOSes at 856. The Phase C judge role put the same wall at ~500. **A threshold that doubles when the role changes is not a token count**, so "BioMistral EOSes above N tokens" is the wrong shape of claim; what travels is "it EOSes on long prompts, at a length that is not fixed". (3) **Prompt shape does not rescue it**: stripping the few-shot block takes the prompt to 520 tokens and it still answers `" {"`, while a 77-token bare ask answers fine — so this is not the few-shot block, and it is not a prompt-engineering problem with a prompt-engineering fix. (4) **THE SHARPEST ONE: it is GREEDY DECODING.** On the identical failing prompt, temperature 0.0 EOSes, temperature 0.7 answered 2 of 3 samples, and temperature 1.0 answered **3 of 3**, with well-formed `mentions` JSON each time. The EOS token is merely the argmax and not by much. **The whole ladder runs at temperature 0 by design, so the protocol run measures an interaction between this model and that policy — not the model's ceiling.** Incidental finding from the same read, and it is the `manifest.model` defect one layer down: **`manifest.model.temperature` is read by NOTHING.** `Caller.__call__` defaults to `temperature=0.0` and no rung passes one except rung 3, which takes its own from `manifest.rungs.3.temperature`. So the key that looks like the answer to "what temperature produced this number" is decoration, and the real answer is a default in code — the same shape of failure as the pre-2026-08-24 model fallback. Not fixed here, because wiring it changes every rung 0 number in the project and the manifest is edited jointly; recorded, and it belongs in its own session.
- 2026-08-31 — **OFF-PROTOCOL ARM: raising the temperature escapes the EOS and does NOT rescue the model, which is what turns the null into an answer.** The protocol run measures a greedy-decoding failure, so on its own it cannot say whether BioMistral has domain skill underneath. Three further draws at **temperature 1.0**, everything else frozen, temperature injected by wrapping `Caller.__call__` in scratch so production is untouched (`out/harness/bmtemp.py` + `bmtempdraws.sh`, runs `out/arm-biomistral-t1-d{0,1,2}-dev.*`, scores `out/biomistral-arm-scores-t1.json`). **Documents answered rise 4/40 → 6, 8, 15 of 40. Predictions rise 3 → 10, 12, 18 against 226 gold. F1 exact 0.0087 → 0.0169 / 0.0504 / 0.0492.** So the escape is real and roughly triples the output — and lands three or four times below the WORST model in the five-model table (qwen3, 0.141) and a factor of **four** below the un-adapted `mistral:7b-instruct` at 0.206, which produced 259 predictions at temperature 0 on this identical split and configuration. 24-33 of 40 documents still fail to parse in every heated draw. **THE CONCLUSION, and it is the one the arm was registered to reach: "domain adaptation cost instruction-following" now holds in TWO roles, not one, and the extractor role is the harder test because its prompts are 2-3x the judge's.** The article's claim can be stated at full strength and the standing limitation ("we tested a domain-adapted model in one role only") is DISCHARGED. Three honest qualifications, none of which move the conclusion: the temperature arm is not comparable to the five-model table (every row there is temperature 0) and is labelled off-protocol wherever it is quoted; the ACCEPT lane is **not measurable** on any of these six runs (n = 1, 2, 2, 6 — the arm contributes no H4 row and must not be given one); and what the model DOES emit when it emits anything is not nonsense — draw d0's protocol run produced |Muddled thinking| on an exact lexical match and rung 1 ACCEPTed it, and the heated draws found "back pain", "dizziness" and "leg muscle soreness" alongside spans like `"word)"` and `"decrease"`. **The failure is transport, not knowledge, and that is precisely why it is fatal: a rung that cannot deliver a reply cannot be measured on the domain it knows.**
- 2026-08-31 — **`manifest.model.temperature` WAS DECLARED AND NEVER READ, and it is now the reader — plus two more instances of the same defect in the same layer.** Found while verifying the BioMistral rung-0 arm: `manifest.json` declared `model.temperature: 0`, `ladder/llm.py:Caller.__call__` carried `temperature: float = 0.0` as a hardcoded default, and no rung passed a value except rung 3, which reads its own `rungs.3.temperature`. So "which temperature produced this number" had a declared answer and a real answer that agreed **by accident** — the exact failure the `manifest.model` note itself was written for, one layer down, and the same class as the 5.9-point manifest/arms gap of 2026-08-28. **Owner's call: WIRE IT, not delete it** (deleting also breaks the append-only rule on `manifest.json`). `llm.temperature_for(manifest)` now resolves it, `for_rung` binds it to the `Caller` alongside the model — one place, not two — and `Caller.__call__`'s `temperature=None` means "the value I was bound at"; an explicit argument still wins, so rung 3's `sampler(0.7)` is untouched. **THE FLOAT CAST IS THE WHOLE NO-CHANGE GUARANTEE**: `temperature` is in the LLM cache key and the key is a `json.dumps`, so the manifest's int `0` and the code's `0.0` hash differently — an uncast wiring would have missed every one of the 7,247 cached calls and re-generated every published number from a cold cache while looking like a no-op. **TWO MORE DEAD DECLARATIONS, both fixed.** (1) `r4.DEFAULTS` carried `"temperature": 0` and rung 4 called `llm(prompt, "", "judge")` without it — REMOVED rather than wired, because a rung DEFAULT would override the manifest and put the code default back on top of the declaration. (2) The OUTPUT side, which is worse: `scripts/ladder_run.py` stamped `sampling={"temperature": 0.7, "k": 3}` and `scripts/full_run.py` stamped `{"temperature": 0.0}`, both LITERALS, and the first reported RUNG 3's sampler temperature as though it were the run's. Now `provenance.sampling_for(man)` reads the manifest and `gather` defaults to it; it carries `temperature_declared`, because `0` in the manifest and 0.0 from `llm.GREEDY_TEMPERATURE` are the same number and NOT the same fact, and after the wiring nothing else can tell them apart. An undeclared rung 3 is reported absent, never as a guessed `k=3` — a stamp that invents a vote nobody took. **VERIFICATION, and one finding in it: `out/cadec-manifest-1` NO LONGER EXISTS on this machine** — its worktree was pruned, nothing under `out/archive/` carries the name, and the sha `f76de5186eb1ad9e1ec5` survives only in this file — so the sha comparison the task specified could not be made against the artifact. Proven instead at the cache-key layer, which is deductive rather than sampled: the effective temperature is `0.0` before and after for every tracked manifest, so every payload dict is byte-identical, so every call returns the identical text, so every record is identical — asserted three ways (`isinstance` float, `_cache_path` equality, and a seeded pre-wiring cache entry that must still be a HIT with the network stubbed to raise). 20 new tests across `tests/test_llm_temperature.py` and `tests/test_provenance_sampling.py`, all TDD'd fail-first; 729 passed / 28 skipped. **The pin worth knowing about**: `test_every_tracked_manifest_resolves_to_the_published_temperature` fails if any `manifest*.json` moves off 0.0 — the wiring's own risk is that editing that key no longer relabels a run, it changes one. Confirmed discriminating by setting `manifest.json` to 0.7 and watching it fail, then restoring. **AND THE LAST GAP IS CLOSED, on the owner's approval the same day**: the FIVE manifests that declared no temperature (`manifest.finer{,.ctxmenu,.llama,.mistral}.json` and `manifest.spine.finer.json` — five files, not the four first reported) now declare `"temperature": 0` with its own note, appended textually so that a re-parse is equal to the original plus exactly those two keys. No FiNER number moves: every run in `out/finer/` was produced at an effective 0.0, and `manifest.finer.ctxmenu.json` remains the test-pinned one-key diff. All eight tracked manifests now answer the temperature question in configuration rather than from a code default.
- 2026-08-31 — **AUDIT: FOUR MORE DECLARED-AND-NEVER-READ SETTINGS, found by asking the temperature question of every manifest key.** Ran every non-`_note` key in `manifest.json` against a literal search of `ladder/` and `scripts/`; 15 have no reader, 10 of which are descriptive metadata that was never a switch (`task`, `gold_rule`, `corpus.n_docs_total`, `rungs.5.tau_tuned_on` …). **The other four are switches that cannot be switched, and they are NOT fixed here — each needs its own measurement, and all four are inert today, which is exactly how the temperature key looked.** (1) **`vocabulary.snomed_backend: "local-rf2"` — the highest-stakes one in the repo.** `run.py:85` is `return Registry(man["vocabulary"]["snomed_db"])`, hardwired; nothing consults `snomed_backend`. Setting it to `ols4` would change NOTHING while every results file claims ols4 — and CLAUDE.md's hard rule makes that exact string the citation for every rung 1 number, where the backends differ by 23.9% of CADEC gold. Declared and real agree today by construction, not by reading. (2) **`rungs.1.outdated_check: "flag"`** — `r1.DEFAULTS` does not even carry the key and `_record_history` flags unconditionally, so the note's documented `off` setting does not exist. (3) **`vocabulary.meddra_mode: "reference"`** — no reader, and rung 0 stopped reading MedDRA entirely on 2026-08-24, so the "declared choice that must go in the manifest" is currently a choice nothing can act on. (4) **`ladder/otel.py:run_meta` HAS NO CALLERS AT ALL** — the function whose docstring says "Temperature is here because rung 5's entire result is a function of it and it was never stamped" is itself never invoked, so it still is not stamped. Registered, and FIXED the same day — see the next entry.
- 2026-08-31 — **ALL FOUR ARE FIXED, plus a model name that does not exist. Every one was inert at its declared value, so no number moves — which is exactly how the temperature key looked.** All TDD'd fail-first in `tests/test_declarations_are_read.py` (16 tests); 746 passed, 28 skipped, preflight 0. (a) **`vocabulary.snomed_backend`**: `run.check_snomed_backend` is now read by `_vocab_for` AND by `scripts/ladder_run.py`, which duplicated the same hardwired `Registry(...)` and therefore the same blind spot. **`ols4` is REFUSED, not honoured** — `vocab.Ols4Vocabulary` serves rung 1's surface but has no `replacements` and none of rung 0's `shortlist`/`resolve`/`search_labelled`, so running the ladder on it is a measurement nobody has taken; refusing still closes the defect, because a results file can no longer claim a backend the run did not use, and the message carries the 23.9% / 7.5% retired / 16.4% AU-extension figures and points at `python -m ladder.vocab_crosscheck`. (b) **`vocabulary.meddra_mode`**: `answer_space` is refused, because it meant showing the model the MedDRA list — rung 0's S3, dropped 2026-08-24 for reasons that have not changed. Accepting it would let a manifest declare an experiment no code path implements. (c) **`rungs.1.outdated_check`**: added to `r1.DEFAULTS` as `"flag"` and honoured in `_record_history(..., mode)`. `off` records NOTHING rather than `sct_outdated: false` — absence is a claim about the run, `false` is a claim about the code, and `ladder/score.py` must not read "we did not look" as "not outdated". (d) **`ladder/otel.py` was not merely unread, it was UNIMPORTED**: nothing in `ladder/` or `scripts/` referenced it, so the `LADDER_OTEL=1 ... scripts/ladder_run.py` in its own docstring emitted no spans, and `run_meta` had no callers, so temperature still was not stamped. `run.ledger_for` is the wiring, in `run_ladder` and in the script the docstring names. **OFF it returns the SAME plain `Ledger`**, asserted with `type() is` rather than `isinstance`, so no published run can have been affected. Smoke-tested with the flag on: the row is written, `run_meta` carries temperature 0.0 / backend `local-rf2` / model / split / order, and the span is dropped with a printed reason — **`opentelemetry-sdk` is NOT in `requirements.txt`**, left out deliberately rather than added unpinned. (e) **Three tracked manifests named `ollama/granite4:micro-h`, which ollama does not have** (`manifest.finer.llama.json`, `manifest.finer.mistral.json`, `manifest.spine.finer.json`) — the name carries the `ibm/` namespace, confirmed against the live `provider_models('ollama')` list. That exact name spent 133 minutes in rungs 0 and 3 of the first full FiNER run before dying at rung 4; `manifest.finer.json` already carried a `_judge_name_note` about it and the other three never got the fix. Corrected, each with its own note, pinned by a test over every `manifest*.json`. No number moves: no run on those manifests ever reached rung 4 with the wrong name and produced a figure. **WHAT THIS PAIR OF SESSIONS IS ACTUALLY ABOUT**: five declarations and one model name, every one of which looked right and none of which were read. Stated as a rule, now the third instance in this file: a declaration nothing reads is not configuration, and a check nobody has seen fail is a check nobody has shown to work.
- 2026-08-31 — **`ladder/otel.py` IS DELETED, one day after being wired, and the wiring is what made the decision possible.** Asked plainly: does the project need it? The evidence said no. **One commit ever** (`44882a3`, 2026-08-23) touching exactly two files — the module and `runs/otel-smoke.jsonl` — so "phoenix transport verified" meant verified against a HAND-MADE row (`{"run_id": "smoke", "doc_id": "D1"}`), never a ladder run; yesterday's `bcffc4a` was the first commit in the repo's history to name `OtelLedger` outside the module. Five phases, two corpora, ~750 tests, and **zero `docs/decisions.md` entries depended on it**. Its spans were a strict copy of the ledger row — `denominator` and `evaluable` included — and the ledger is already JSONL on disk and already what `scripts/ladder_top.py`, `ladder/provenance.py` and every `out/harness/*.py` script read, so there was no question a span answered that `jq` over the ledger did not. It also needed `opentelemetry-sdk`, absent from a `requirements.txt` that is pinned by policy. **Removed**: the module, the smoke row, `run.ledger_for` (an indirection choosing between two ledgers, with one left it is a branch that cannot branch), the README block and architecture-line mention, the `docs/plan.html` feature row, and `provenance.gather`'s `env.ladder_otel` — a stamp naming an env var that switched on nothing describes a run nobody had. **Three tests keep it gone**, one of which greps the docs, because a feature the README advertises and no code implements is this week's defect pointed the other way; `docs/decisions.md` and `CLAUDE.md` are exempt, since a record naming a deleted thing is what a record is for. **THE TEST BUG WORTH REPORTING**: that doc grep first passed VACUOUSLY. It filtered on `path.parts`, and this repo is worked in worktrees under `.claude/`, so every absolute path contained an excluded component and the guard skipped every file while `README.md` still advertised the exporter. Fixed to filter on the RELATIVE path, then watched to fail on the real hits. A guard that cannot fail is the same thing as a declaration nothing reads. **Also corrected**: `ladder/provenance.py`'s opening example said `ollama/ibm/granite4:micro-h` "does not exist and 404s" — that has INVERTED since it was written, and the bare name is now the one ollama does not have. Dated note added rather than the history rewritten; the point it illustrates survives the flip intact.
- 2026-08-31 — **THE AUDIT RUN A THIRD TIME, over rung DEFAULTS and over dead code, and the first thing it caught was MY OWN TEST.** Three mechanical scans. **(1) The vacuous-guard scan** — build a collection by globbing, assert it is empty — found exactly TWO instances in the whole 756-test suite and **both were written in this session**, one of which had already bitten (the doc grep filtered on absolute `path.parts`, and this repo is worked in worktrees under `.claude/`, so every path was excluded and the guard read nothing while `README.md` still advertised the exporter). Both now count what they opened (`looked > 50` files, `seen >= 5` manifests) and were confirmed discriminating by pointing each walk at an extension that does not exist. **A guard that cannot fail is a declaration nothing reads, and the lesson is that writing the rule down does not exempt you from it.** **(2) Rung DEFAULTS, audited the way the manifest was**: three more declared and never read, all REMOVED rather than wired — `r2.max_attempts: 1` (its own comment says "more is a different experiment", and `_attempt` runs exactly once, so a manifest setting 3 changed nothing), `r2.allow_withdrawal: True` (withdrawal is unconditional and is Constraint 5, a design rule and not a switch), `r4.show_vocabulary_term: False` (from 39a94f0, never read since). The removal argument differs from `r4.temperature`'s: there is no manifest declaration to defer to here, only the question of whether the knob exists, and each comment already answered no. **(3) Dead public functions**, the shape `otel.run_meta` had: `ladder/vocab.py:lexical_overlap` (the OLS4-era "check 6"; `lexical_match` on both backends replaced it) and `ladder/rungs/r0.py:report_run` — the second **worse than unused**, since it reads `agg["documents"]` and `agg["tool_calls"]`, which the modern `apply()` aggregate does not carry, so calling it today would raise KeyError. Both deleted; `r0.report` is the live one and a test asserts it survives. **(4) A skip that was a category error, in the one test CLAUDE.md credits with catching two live bugs in `ladder_run.py`.** `ENVIRONMENT` listed `SystemExit` wholesale, so four scripts that do their whole job at import and **exit 0** — `port_prompts`, `port_prompt_constants`, `fix_tui_live`, `add_full_retrieval` — were reported as "environment, not code" and skipped. Exit 0 is success. `skip_reason()` now returns None for code 0 or None, names the code for a nonzero exit, and keeps everything else as it was; script-import skips fall 11 -> 7 and the remaining 7 are genuinely environmental (6 need the gitignored `snomed.sqlite`, 1 needs `rich`). **AND A BUG IN THAT FIX, caught by the suite**: the refactor replaced `except ENVIRONMENT` with `except Exception`, and **`SystemExit` is a BaseException, not an Exception** — it escaped the handler entirely and three scripts errored instead of passing. The old code worked only because `ENVIRONMENT` named `SystemExit` explicitly. Now `except (Exception, SystemExit)`, with the reason in a comment. 756 passed, 24 skipped (was 745/28: four honest passes, and the rest new tests).
- 2026-08-31 — **The local checkout was cleaned to main plus the unmerged work only: 31 merged branches and 9 worktrees deleted, and roughly 240 MB of `out/` artifacts went with them. This entry is the provenance those artifacts no longer carry.** Local `main` fast-forwarded to `origin/main` (GitLab, `cc0c875`); `git fetch --prune` run first. **Deleted, all fully merged into main:** in the bare checkout — `claude/article-full-ladder`, `biomistral-judge-reeval-6eb47e`, `centralise-llm-selection`, `engineering-wiki`, `gitlab-sync-step-review-9a3be1`, `jovial-easley-acefe2`, `ladder-data-selection-228062`, `meddra-codes-csv-extract-5c7c1a`, `phase-e-cost-measure-decision`, `phase-e-rung-6-human-loop-64ce8e`, `phase-f-test-split-cd5c99`, `phaseB-rung0-accuracy`, `plan-e2e-test-b5f28f`, `plan-progress-review-72b321`, `project-implementation-plan-3516ad`, `reliability-ladder-owner-a-8786e6`, `reverent-wiles-976e18`, `rung-0-review-plan-c6cd8d`, `rung-0-single-file-output-c0a3a5`, `rung0-enhancements-c6cd8d`, `rung0-prompt-engineering-8d1b2f`, `worktree-agent-a0b99ab8799068fa2`; and, after removing the worktree holding each — `audit-rungs-1-6-tdd-4a90df`, `dashboard-spec`, `determined-cannon-0d592f`, `infoq-article-placeholder-727379`, `phase-d-rung-3-repair-fafe4c`, `reliability-ladder-continue-fffbf7`, `rung-0-accuracy-4840ba`, `rung-0-reranker-5387bc`, `rungs-audit`. **KEPT because they are NOT merged and carry commits main does not have:** `claude/dashboard-m1` (ahead 6) and `claude/phaseC-models` (ahead 1). Nothing was force-deleted — every deletion went through `git branch -d`, which refuses an unmerged branch, so no commit reachable only from a deleted branch was lost. All nine worktrees were clean on tracked files before removal.
- 2026-08-31 — **What the cleanup destroyed, listed because `out/` is gitignored and therefore unrecoverable: the run artifacts behind every measurement in this file are GONE, while the measurements themselves stand.** This is the standing convention (`out/harness/README.md`) executed rather than merely stated — scratch is disposable, `docs/decisions.md` is the durable record — but the consequence is worth naming: **no number in this file or in `CLAUDE.md` can now be re-derived from disk, only re-run.** Per worktree, size and the run ids that lived there. `agitated-lewin-346b03` (88 MB, 183 files) — the five-model sweep `arm-m-{granite4-micro-h,llama3.1-8b,mistral-7b-instruct,qwen3-8b}-d{0,1,2}-dev` behind the five-model ACCEPT-lane table dated 2026-08-30, the reranker arms `arm-rr{base,llm,pol,pol20}-*`, the role arms, `spine-cadec`/`spine-finer`, `audit-full-dev-1`, and the FiNER data other worktrees symlinked to. `phase-e-rung-6-human-loop-64ce8e` (83 MB, 33 entries) — the three-draw debt arms `arm-debt{base,nocut,nosplit}-d{0,1,2}-dev`, `debt-paired`, `out/judgearm/` (the rung-4 wire-and-measure), `out/finer/arm-finer{base,ctx}-d{0,1,2}`, `phaseE-r6-{sim,oracle}` and its resolutions files, plus `out/harness/` — about 80 scratch scripts including `debtpaired.py`, `judgearmcmp.py`, `finerrecall.py`, `finerctxdiag.py`, `finermiscode.py`, `finerrefusal.py`, `examples.py`, `rungexamples.py`. `reliability-ladder-owner-a-8786e6` (32 MB, 112 files) — `phaseB-1`, `phaseB-2`, `phaseB-arm-alpha`, the rung-1 detection probes, `vocab_crosscheck`, the rerank-order arms. `plan-e2e-test-b5f28f` (16 MB, 50 files) — the gold-replay and ablation runs `gold_{dev,pool,control}`, `ablate_{dev,gold}`, `gate_mode`, the runs behind rung 1's 9.3% -> 0.13% error-floor result. `fervent-hellman-813240` (15 MB, 61 files) — the span-filter and inconsistent-gold arms, `noise-draw{1,2,3}`. `phase-d-rung-3-repair-fafe4c` (3.3 MB) — `phaseD-r3-1`, `phaseD-r3-2`, the run id every rung-3 number must cite. `phase-f-test-split-cd5c99` (2.2 MB) — **`phaseF-test-1`, the one held-out test run, ledger, records and manifest.** That last one is the sharpest loss: Phase F is spent by design and cannot be re-run, so the test-split numbers are now reported on the strength of this file alone. **A second reason the deletion was correct regardless: every `*.records.jsonl` carries CADEC span text, so none of it could ever have been committed** — the licence rule and the cleanup point the same way.
- 2026-08-31 — **One rescue, recorded so it is findable: four untracked deliverables in the infoq worktree were never in git and are not scratch.** `docs/article-infoq.docx` (189 KB), `docs/article-infoq.md` (29.7 KB), `docs/figure1.png`, `docs/figure2.png`, all dated 2026-08-22, no copy in any branch or in git history, and the `.md` DIFFERS from the tracked `docs/infoq-article-draft.md` (29,742 vs 33,516 bytes). Copied to `~/Documents/repo/reliability-ladder-archive/infoq-worktree-2026-08-22/` — outside the repo, so the checkout stays clean — before the worktree was removed. They are a 2026-08-22 export and predate the article rewrite; `docs/article.md` and `docs/article-v3.md` remain canonical per the naming rule.
- 2026-08-31 — **Dead-code sweep after the branch cleanup: `ladder/otel.py` deleted, and the reason is a provenance defect rather than tidiness.** The module was imported by NOTHING, yet its own docstring and `README.md` both documented `LADDER_OTEL=1 ... python3 scripts/ladder_run.py` as the way to enable it — and `ladder_run.py` never imported it either, so that command emitted no spans and no error. Meanwhile `ladder/provenance.py` wrote `env.ladder_otel` into every run stamp, so a run could record that tracing was on while producing none. **In a project whose every number is defended with its provenance, a provenance field that reports a capability the repo does not have is worse than no field at all** — it is the same failure this repo keeps finding elsewhere (a denominator that shifts silently, a could-not-run coerced into a pass), one level up. All three removed together: module, provenance field, README section. `tests/test_otel_removed.py` (3 tests, written failing first) keeps them gone, in the same idiom as the existing test asserting a filename is absent from `r0.py`. Anyone who wants OTel back should WIRE it and test it, not restore the file.
- 2026-08-31 — **Four applied one-shot codemods deleted from `scripts/` (803 lines), because a codemod that has already run is not a tool, it is a loaded gun.** `port_prompts.py` and `port_prompt_constants.py` (the 2026-08-28 FiNER prompt-slot port), `add_full_retrieval.py` and `fix_tui_live.py` all rewrite TRACKED SOURCE in place — `ladder/rungs/r0.py`, `ladder/ledger.py`, `manifest.json` — via `write_text`, and `port_prompt_constants.py` carries its own backup/restore path for when that goes wrong. Their edits were verified already applied before deletion (the `PROMPTS` slot table at `r0.py:402`, `Ledger.flush` at `ledger.py:107`), so a second run would apply a patch to source that no longer matches the anchor. They were also being kept permanently importable by `tests/test_scripts_import.py`, which globs `scripts/*.py` — the reason the suite went 710 -> 709 passed is exactly this (8 parametrised cases lost, 3 otel tests gained; 733 collected, and the arithmetic closes).
- 2026-08-31 — **`runs/` was a second `out/` that nobody had noticed, and the gitignore rule was half-written.** 29 output files (128 KB) were committed there — 24 provenance dumps from `full_run.py`/`dev_sweep.py`, 5 smoke jsonl — and NOTHING in the repo reads any of them back. `.gitignore` already excluded `runs/*.ledger.jsonl`, `runs/*.report.txt` and `runs/.r6-pool.json`, i.e. someone had drawn exactly this conclusion and stopped one glob short. Now `runs/*` with `!runs/.gitkeep`. **The directory stays TRACKED and that is deliberate**: `scripts/ladder_run.py:119` opens `runs/<run-id>.report.txt` with no `mkdir` (only `full_run.py` and `dev_sweep.py` create the directory, and `Ledger` creates it as a side effect of its own construction at line 124), so ignoring the directory outright would crash the runner on a fresh clone. Same convention as `out/` now, with the one difference recorded in the ignore rule itself.
- 2026-08-31 — **Documentation sweep: 11 unreferenced `.md` files removed, one misfiled draft archived, and the article family deliberately LEFT ALONE.** Deleted: the nine `MR-*.md` merge-request write-ups at the repo root (50 KB, zero inbound references, every branch merged and deleted the same day), `docs/HANDOFF-2026-08-23.md` (zero references; it names a Fedora checkout at `~/Documents/ai-reliability-ladder` and calls the article "not yet submitted" — superseded by CLAUDE.md's Current state and `docs/PLAN-next-sessions.md`) and `docs/article-v3-outline.md` (zero references, superseded by the draft it outlined). Moved: `docs/infoq-article-draft.md` -> `docs/versions/infoq-article-draft-2026-08-24.md`, because it was sitting in `docs/` as though current when it is the ancestor of `docs/article-v2.md`; indexed in `docs/versions/README.md` under that directory's stated convention, and README's link re-pointed. **NOT consolidated, on purpose: the six article documents and the 21-file `docs/wiki/`.** The wiki is gated by CI (`docs/wiki/build.py --check`, 23 pages, `.gitlab-ci.yml:60`). The article files LOOK redundant and are not — CLAUDE.md's naming rule assigns each a distinct job precisely so that "which article is this" has one answer, and merging them would re-open the question that rule was written to close. Reducing a file count is not a reason to undo a decision.
- 2026-08-31 — **TWO SESSIONS DELETED `ladder/otel.py` INDEPENDENTLY THE SAME DAY, and the pair of entries above is kept because the disagreement in them is only apparent.** The housekeeping sweep found it UNWIRED — nothing imported it, while its own docstring and `README.md` both published `LADDER_OTEL=1 ... scripts/ladder_run.py` as the way to switch it on, and `provenance` stamped `env.ladder_otel` on every run — and deleted it on that basis. The declarations audit WIRED it first, confirmed the wiring was correct, and deleted it because the feature was not wanted. Both readings are true in sequence, and the second is the stronger evidence: a thing removed after it demonstrably works is removed on its merits, not on a suspicion that it was broken. **What survives is the audit's guard, not the sweep's.** `tests/test_otel_removed.py` was deleted in this merge: it asserted the string `ladder_otel` appears nowhere in `provenance.py`, which forbids the explanatory comment the audit correctly left at the removal site, and `test_nothing_references_the_deleted_exporter` in `tests/test_declarations_are_read.py` already covers strictly more — it walks the whole repo, exempts the record files (`decisions.md`, `CLAUDE.md`, `MR-*`) that must be free to name a deleted thing, and asserts `looked > 50` so it cannot pass on an empty walk. Duplicate coverage where the weaker copy is also the wrong one is not redundancy, it is a trap.

- 2026-08-31 — **THE REPO HAD TWO SCORERS REPORTING UNDER THE SAME WORDS, and now it has one.** `ladder/score.py` (span-KEYED, `exact` or `overlap` declared per call, detection and coding as separate layers) produced every figure in this file. `bench/align.py` paired predictions with gold by **character IoU >= 0.5 under bipartite (Hungarian) assignment** — a third matching rule — and had exactly one caller ever, `scripts/full_run.py:67`. **Neither was wrong, and that is the point: "precision" meant two different things depending on which entry point produced it, with nothing on either number saying which.** This project's findings are largely about denominators and definitions moving silently — a rate over the wrong base, a could-not-run coerced into a pass — and a second matcher behind the same vocabulary is that same failure sitting in the reporting layer, waiting for someone to put both numbers in one table. `full_run.py` ported to `score_run`; `bench/` deleted; `tests/test_one_scorer.py` (3 tests) keeps it deleted, reusing the `looked > 50` guard-on-the-guard from `test_nothing_references_the_deleted_exporter` — and that guard immediately earned itself by failing on the very docstring written to explain the deletion, which named `bench.align` in import form. **One behaviour changed ON PURPOSE:** an unlocated prediction (spans `(-1, -1)`, the schema-invalid shape) was silently DROPPED by the old path and is a false positive to `score_run`. Counting it is the honest reading and is what `ladder/run.py` already did. **NO REPORTED NUMBER MOVES:** `full_run.py` runs `stub_llm`, never reached a model, and no figure in this file or the article came from it — it is a wiring smoke test over the r0 -> gold -> r2 path, which its docstring now says outright. It was NOT executed as part of this change (it needs the SNOMED index and the CADEC corpus, neither present in the cleanup worktree); the port is verified by `score_run`'s own tests, a source-level guard, and `py_compile`, and that limitation is stated here rather than glossed.
- 2026-08-31 — **The wiki was documenting the deleted scorer, and the same table called four rungs and `ladder/score.py` "not written".** `docs/wiki/` is CI-gated (`build.py --check`, 23 pages) which checks LINKS and orphans, not whether a claim is still true — so `architecture.md` had carried "`ladder/score.py` | shared scorer — not written" since 2026-08-23, through every phase that wrote them. Fixed both scorer entries and rewrote `measurement.md`'s pairing section around `score_run` (span keying, the two declared match modes, the detection/coding split, discontinuous fragments), keeping a short record of what the second scorer was and why it went. **The general point is registered and NOT fixed here: a docs gate that checks link integrity certifies nothing about accuracy**, and the rest of that wiki has not been audited against the code since the 2026-08-23 renumber.
- 2026-08-31 — **Archived rather than deleted: `docs/Group-The-AI-Reliability-Ladder-Measured-Rung-by-Rung.docx` (442 KB, the largest tracked file, zero inbound references) -> `docs/versions/article-v2-submitted-2026-08-30.docx`.** It is a BUILD OUTPUT of `docs/article-v2.md` and would normally be scratch, but it is the artifact that was actually submitted on 2026-08-30 — the source says what was written, the docx says what was sent — so it is kept under the `docs/versions/` convention and indexed there with a warning never to regenerate it in place, since a rebuild from a later `article-v2.md` would no longer be the submitted thing. Also removed: `MR-2026-08-31-declarations-are-read.md`, the eleventh merge-request write-up, same class as the ten before it.

- 2026-08-31 — **A1 SETTLED: the 0.70 ceiling claim is NOT wrong, it is CORROBORATED — and the article's own comparison was the defective part.** Read CONORM (medRxiv 10.1101/2023.09.26.23296150v3, Yazdani et al.) plus its published evaluation code (`github.com/ds4dh/CONORM`). Four facts and one inference. **(1) Their matching strategies are OURS.** `compare_normalizations.py` scores a prediction as a `(type, span, concept)` tuple under two strategies — `compare_exact` requires identical offsets, `compare_overlap` requires the spans to intersect — and **the concept code must match in both**. That is `ladder/score.py`'s `exact` / `overlap` with the code required, so the vocabularies differ but the metric does not. **(2) The 70.40% "exact F1" everyone quotes is NER ONLY.** It is Table 4, span detection with no code assigned: CADEC lenient 89.10, exact **70.40**. **(3) The end-to-end CADEC number is 72.45%** (Table 5) and the prose gives ONE figure per dataset with no matching label. **(4) THE INFERENCE, and it is arithmetic, not judgement: 72.45 CANNOT BE EXACT.** End-to-end requires span AND code, so it is bounded above by NER under the same strategy; 72.45 > 70.40 means the end-to-end figure is the LENIENT/overlap one. The macro-averages say it twice — end-to-end mean 73.77 against NER strict mean 73.17 and NER lenient mean 85.75. Table 5's column headers could not be read directly: medRxiv 403s automated fetch, the tables are not in the rendered DOM, and the highwire table endpoint is Cloudflare-gated. **SO THE DECISION IS THE OPPOSITE OF THE ONE PRE-REGISTERED: no published end-to-end SPAN-EXACT CADEC number above 0.70 exists in this paper, and the best supervised system's span-exact score on DETECTION ALONE is 0.704 — almost exactly where our ~67% boundary-determinism argument puts the cap.** A fully supervised BIO tagger trained on 875 CADEC files, before it assigns a single code, lands on the number we predicted as a ceiling. That is corroboration and it should be stated as such, not softened. **WHAT WAS ACTUALLY WRONG IS OUR COMPARISON**: §lit set "their 0.72 end-to-end" beside "our 0.399" — their LENIENT against our EXACT, and against a rung-0 extraction-only DEV number at that, while the shipped end-to-end held-out figure is 0.204 exact / 0.215 overlap. Fixed on both axes and on both splits. **THREE QUALIFICATIONS THAT ARE NOT EXCUSES AND MUST TRAVEL WITH THE NUMBER.** (a) They normalise to **MedDRA PT (English v16), not SNOMED CT** — CADEC carries both, and it is not the same answer space. (b) They are supervised on CADEC (875 train / 187 val / 188 test of the 1,250 files); their own out-of-distribution analysis on TAC shows what that buys — stratified by whether the gold concept was seen in training, precision falls to **47.1%** on unseen concepts against **85.8%** on seen ones. **A zero-shot system is 100% out-of-distribution by construction.** (c) INFERENCE, flagged not measured: CONORM-NER tags with a **BIOX** scheme, which is contiguous by construction, so the best supervised system on this corpus appears to carry our own discontinuous-span cap — relevant to B1, and a reason not to treat 17.3% of gold as a gap only we have.

- 2026-08-31 — **`scripts/preflight_rungs.py` VALIDATED against what each rung actually paid, and it makes the right call on every rung where a record exists.** The article's central recommendation — every layer has a precondition and all of them are measurable before the layer is built — rested on a tool that had never been shown to predict anything, which is the same position as the rungs it exists to catch. Answer key: this file. **(1) THE FREE WIRING CHECK IS EXACT ON THE ONLY CASE IT WAS BUILT FOR.** Run `check_readers` against `ladder/` as it stood at `9bb98c9^` — the commit before rung 4 got a reader — and it names **r2_declined, r3_unanimous_none and r4_verdict**, which is precisely the set the 2026-08-28 audit found by hand, and nothing else, while correctly leaving 55 diagnostic fields alone. Zero false positives, zero false negatives, no data, no model, under a second. On today's tree it reports 2, r4_verdict now having a reader — the correct present-tense answer. **(2) THE MEASURED PRECONDITIONS AGREE WITH THE RECORD ON CADEC.** `--static` on `manifest.json`, dev: ACCEPT lane **BUILD** (117 of 276 gold mentions match a term for their own code, 42.4%, mode `exact`) and the rejection lane **BUILD** (0 of 276 gold codes fail the free existence check, consistent with the 0.13% measured corpus-wide after the gold replay took it down from 9.3%). Both are the calls the record supports: the ACCEPT lane is the one thing on this ladder that demonstrably paid, and rung 1's post-repair gate is clean. **CAVEAT, and it matters for quoting: preflight reads `gold_records` directly and does NOT apply `data/exclusions.csv`** — 287 gold mentions on dev where every scored number is over 226 — so its rates share a corpus with the ladder's figures but not a denominator. **(3) THE FiNER CALL COULD NOT BE RE-RUN** — that corpus went with the branch cleanup and re-obtaining it is a 98 MB download — so it is validated as a mechanism instead: a bare-number span against a US-GAAP tag name shares no tokens *by construction*, and `check_accept_lane` returns DON'T with "cannot fire on a PERFECT answer set". That is the one-query call the article says would have saved the whole FiNER arm, and it is now pinned by a test that needs neither the corpus nor a model. **(4) 11 TESTS, and the mutation pass is the result, not the count.** Each of the six decision thresholds was mutated in turn and the suite was required to fail. **First pass caught 2 of 6**, and two of the misses were real weaknesses: a bare DON'T cannot distinguish "the lane cannot fire on a perfect answer set" (FiNER's finding) from "the lane fires but is too thin", and nothing exercised the resample check's UNKNOWN middle band at all. Both fixed by asserting on the branch's stated reason; all six mutations now caught. **(5) THE MUTATION PASS FOUND A DEFECT IN MY OWN TEST FILE FIRST, and it is the repo's own recurring one.** `spec_from_file_location` + `exec_module` writes `scripts/__pycache__/preflight_rungs.*.pyc` and serves it back, so three mutations "passed" against source that had not been re-read — a cache outliving the thing it keys on, exactly like the LLM cache key that survived a `reasoning_effort` change. The loader is now `compile` + `exec` over the source text and writes no bytecode. **NOT VALIDATED, stated so the claim does not overreach:** rung 3's resample and rung 5's tau are pinned as DECISION RULES against the recorded numbers (`not_resampled` 206 of 240; confidence `{1.0: 204, 0.99: 44}`), not re-measured — both need model calls and the artifacts are gone. The rung 4 check measures nothing by design; it points at `scripts/r4_gold_control.py`. **So the article may now say its predictions were checked against what each rung paid, with that scope attached — not that it was run on a fresh corpus and proved out.**

- 2026-08-31 — **`.gitignore` covered `ladder/cache/` and not `ladder/cache`, so the path every worktree SYMLINKS was untracked and unignored.** Found while setting a worktree up to validate `preflight_rungs`: the trailing slash matches a DIRECTORY, git does not treat a symlink as one, and the standing worktree convention creates a symlink to the main checkout's cache — so `ladder/cache` sat in `git status` one `git add -A` away from committing the embedding vectors. Fixed by adding the bare form, TDD'd fail-first in `tests/test_worktree_setup.py`, which asserts every path the convention symlinks (`ladder/cache`, `data/cadec`, `data/keywords.csv`, `data/exclusions.csv`, the SNOMED release) is ignored in SYMLINK form via `git check-ignore --no-index`. The corpus paths were already covered by `data/*`; only the cache was not. Same shape as the declared-and-never-read audit: the rule and the situation agreed for as long as nobody exercised the difference.

- 2026-08-31 — **CI RED on the gitignore guard, and the cause is that `python:3.12-slim` HAS NO GIT.** `tests/test_worktree_setup.py` shelled out to `git check-ignore` and was the ONLY test in 781 that ran a subprocess at all — so it passed on every developer machine and raised `FileNotFoundError: 'git'` in the pipeline, where the runner's helper image does the clone and the build image never needs the binary. Same class as the 2026-08-31 exclusions failure: a test whose precondition is satisfied everywhere except the one environment that gates the merge. **The fix splits the file by what each half needs.** The REGRESSION — a missing bare `ladder/cache` line — is now asserted against `.gitignore`'s own text and needs nothing but the file, so it runs in CI. The broader check, every symlinked path evaluated by git's real matcher, is `skipif` on `shutil.which("git")`. Written that way round deliberately: **the half that goes quiet in CI is the half that is not the guard**, which is the opposite of the usual mistake. The git-dependent test also opens with a CONTROL — `README.md` must NOT be ignored — because a `check-ignore` that answered "ignored" to everything would have made the rest of it vacuous, and this repo has already caught two guards that passed on an empty walk. Verified by removing the `ladder/cache` line and running with an empty PATH: the surviving test fails, which is the combination CI actually runs.

- 2026-08-31 — **B2, THE OFFLINE MENU-RECALL PROBE: a domain-adapted encoder DOES put gold on the menu more often (+1.4pt at k=20, separated), and the ceiling it moves is still not the binding constraint.** `ladder/menurecall.py` + `tests/test_menu_recall.py`, 16 tests, TDD-first, and **the control reproduced the recorded deduped baseline to every decimal before any arm was read** — granite-embedding:30m over `data/keywords.csv`, 6,595 scorable coded gold reaction mentions (exclusions applied), 3,560 distinct query strings: **63.7 / 77.7 / 83.2 / 87.0 / 91.1 at k=1/5/10/20/50**, exactly the row recorded on 2026-08-25. SapBERT (`cambridgeltl/SapBERT-from-PubMedBERT-fulltext`, 110M, [CLS] pooling, same corpus, same row order, same k, same answer key — the ONLY thing that varies is the encoder): **66.1 / 80.7 / 84.5 / 88.4 / 91.6**. Paired bootstrap over the 1,144 documents, 2,000 draws: **k=1 +2.30pt [+1.36, +3.29], k=5 +2.99 [+1.95, +4.00], k=10 +1.24 [+0.29, +2.18], k=20 +1.44 [+0.51, +2.37], k=50 +0.53 [−0.26, +1.26]**. Separated at every k the menu actually uses. **So the stop condition is NOT met and part of the retrieval ceiling is ours, not the task's.**
  - **THE CHURN IS EIGHT TIMES THE NET.** At k=20 SapBERT rescues **435** mentions granite could not reach and breaks **340** it could — net **+95 of 6,595**. Same shape as the rejected query-rewriting arm (rescued 26, broke 62), net positive this time.
  - **AND IT GAINS WHERE THE PICK CONVERTS WORST.** The 435 rescued arrive at **median rank 4, only 18.2% at rank 0**; the 340 broken were sitting at **median rank 2, 28.5% at rank 0**. Priced through the conversion table measured on the 2026-08-28 reranker arm (**94.5% of gold at rank 0 is picked, 42.3% at ranks 1–19**): gained 225 answers, lost 194, **net +31 of 6,595 = +0.47pt of coding recall, ×0.516 detection = +0.24pt of F1 exact.** Rung 0's own run-to-run spread on the same split is **1.3pt** (0.399 / 0.395 / 0.408). **The predicted effect is a fifth of the noise floor, so a three-draw arm cannot resolve it and a null from one would mean nothing.**
  - **THE PRECEDENT IS DIRECT AND IT ALREADY RAN.** The LLM reranker moved dev rank-0 recall **52.3% → 58.6% (+6.3pt)** and F1 exact was **byte-identical** (0.399 → 0.399). SapBERT moves corpus-wide rank-0 recall **63.7% → 66.1% (+2.3pt)** — a third of a change that converted nothing. The mechanisms differ (the reranker reorders a fixed pool and shares the pick's errors; SapBERT makes 435 mentions reachable that were not), which is why the probe was run rather than the precedent quoted, but the size comparison stands.
  - **THE CEILING ON ALL ENCODER WORK HERE, and it is the number §9 should carry.** An ORACLE choosing the better of the two encoders per mention — unbuildable, it needs the answer to choose — reaches **93.6% at k=20 against granite's 87.0%**. Converted at the measured rates that is **+1.76pt of F1 exact**, barely above the 1.3pt run-to-run spread. So the *entire* headroom in this corpus's retrieval, exploited perfectly, is about one run's worth of noise. Meanwhile the pick converts **42.3% of the gold the menu already carries below rank 0**. The menu is not where the gap is; the pick is, by roughly a factor of two.
  - **PROTOCOL NOTES.** Both encoders are deterministic, so the retrieval comparison needs no draws — the +1.4pt is exact, not a sample. The probe DEDUPES QUERIES (6,595 mentions carry 3,560 distinct strings) but never the (query, gold) pair, and the tests pin that. Mutation-checked seven ways; **two mutations survived the first test set** — resampling mentions instead of documents, and an off-by-one at the k cut on the paired path — and both are now caught by tests written specifically for them. SapBERT is NOT wired into the repo and torch/transformers were installed in a scratch venv only: it is an unbuilt arm, not an off-by-default one.

- 2026-09-01 — **B2 REJECTED: the domain-adapted retriever puts gold on the menu more often across the corpus and makes the shipped system WORSE, three draws for three.** `manifest.sapbertarm.json`, one key (`rungs.0.rung0_encoder` granite -> sapbert), pinned by a test, off in `manifest.json`. Six runs (`out/arm-sap{base,bert}-d{0,1,2}`), dev, 40 documents, rungs 0-1, cold cache per draw, **base first at each draw so the arm's find calls are cache hits — detection is byte-identical between the arms at every draw (219/226/219 predictions, det recall 0.540/0.558/0.549, matched 122/126/124), so the whole difference is the menu and the pick over it.**
  - **THE RESULT. F1 exact 0.4135 / 0.4336 / 0.4225 → 0.4045 / 0.3938 / 0.3910** (paired bootstrap over documents: −0.0090 [−0.0510, +0.0308], −0.0398 [−0.0821, +0.0000], −0.0315 [−0.0705, +0.0114], pooled mean **−0.0268**). **Coding accuracy 0.754 / 0.778 / 0.758 → 0.738 / 0.706 / 0.702**, pooled mean **−0.0481**. Overlap agrees in sign and size (pooled F1 −0.0342, coding −0.0419). Sign-consistent six for six across both span-match rules. Rejected on **consistency plus an effect twice the base's own spread** (the base is 0.4135/0.4336/0.4225 exact, a 2.0pt spread; the coding-accuracy loss averages 4.8pt against a 2.4pt base spread) — **not on separated intervals**, none of which exclude zero. Same standard `rung0_cut_rate` was kept under, applied in the other direction.
  - **THE OFFLINE PROBE SAID GO, AND THE PROBE WAS RUN ON THE WRONG POPULATION. This is the session's real finding.** The stop-condition probe measured recall over **all 6,595 gold mentions across 1,144 documents** and separated: +2.30pt at k=1 [+1.36, +3.29], +1.44pt at k=20 [+0.51, +2.37]. Re-run over the population the arm actually sees, the sign flips: **dev gold (216 mentions, 38 documents) −2.78pt at k=1 [−7.04, +1.79] and −0.46pt at k=20 [−5.03, +4.81]; dev gold the model MATCHED (120 mentions, 32 documents) +0.00pt at k=1 and −1.67pt at k=20**. The dev intervals contain the corpus-wide point estimate, so this is not a contradiction — it is that **38 documents cannot resolve a 1.4-point retrieval difference, and the probe's separation came from 1,144.** An offline probe used as a go/no-go must be run on the denominator the arm will be scored on. Ours was not, and it is the reason a null was predicted and a loss was measured.
  - **IT IS THE SUBSET, NOT THE QUERY.** Production searches the model's proposed and trimmed span text and merges label queries, where the probe searched gold text — so the two could have disagreed for that reason. They do not: the same probe over gold text on the three nested denominators (all / dev / dev-matched) already carries the whole reversal, **87.0→88.4, 88.9→88.4, 94.2→92.5 at k=20**. The mentions the model successfully detects are an easier population than the corpus, and SapBERT's corpus-wide gain lives in the hard tail it never reaches (its 435 corpus rescues arrive at **median rank 4**; the 340 it breaks were sitting at **median rank 2**).
  - **AND THE SLOT-0 ATTRACTOR AGAIN, ON THE THIRD CORPUS-CONFIGURATION IN A ROW.** On the matched records the arm pushes the pick further onto menu line one — **slot-0 selection 76.2 / 78.6 / 76.6% → 79.5 / 83.3 / 79.0%** — while slot-0 accuracy **falls, 0.849 / 0.848 / 0.853 → 0.794 / 0.752 / 0.776**, and tail conversion falls with it (0.419 / 0.452 / 0.433 → 0.400 / 0.290 / 0.367). Roughly 60% of the coding loss is menu recall on this subset (−3.0pt) and the rest is the pick behaving worse over a menu it trusts more. Same shape as the FiNER context menu and the reranker: **a retrieval change can be right about ranking and still lose, because what it displaces was better than it.**
  - **COST IS FLAT AND THE LATENCY NUMBER IS NOT COMPARABLE.** Tokens 155,649 / 155,889 / 156,916 → 154,119 / 155,029 / 155,642; calls 93 / 95 / 94, identical. Rung 0 p95 reads 78.5 / 74.0 / 92.4s → 31.2 / 20.6 / 19.9s and **that is a cache artefact, not a saving**: the arm ran second at each draw so its find calls were served from disk. Reported here so nobody quotes it as a speed-up.
  - **WHAT SURVIVES FOR THE ARTICLE. §9's claim stands and is now better supported from a second direction:** a measurably better encoder, chosen from this task's own literature, did not beat the menu-and-pick it was dropped into. **But the ceiling itself is partly ours and the article must say so:** corpus-wide, SapBERT reaches 88.4% at k=20 against granite's 87.0%, and an oracle choosing the better encoder per mention reaches **93.6%** — so 87% is not the task's ceiling, it is this encoder's. What B2 shows is that moving that ceiling is not the same as moving the result.
  - **The arm SHIPS OFF and the code stays.** `rung0_encoder` is declared explicitly in `manifest.json` as `granite`, `ladder/embed.py:ENCODERS` raises on an unregistered name, each encoder has its own index at `<embed_prefix><suffix>` (hyphen, because `Path.with_suffix` eats a dotted tail and would silently load the other encoder's matrix), and torch/transformers are late imports behind the default path. **The production path reproduced the offline probe to every decimal on both encoders before a single run** — 63.7/87.0 and 66.1/88.4 through `r0._retriever` — which is the check that the arm measures the code and not the harness.

- 2026-09-01 — **A regression the B2 wiring introduced and a test now pins: `python -m ladder.embed --query` searched the index under a DIFFERENT encoder from the one that built it.** Adding `--encoder` demoted `--model` to an override defaulting to `None`, and the query branch still passed `a.model` straight to `ollama_embedder` and `a.prefix` straight to `EmbeddingIndex` — so it asked the server to embed with model `None` (404) and, had that succeeded, would have ranked queries from one encoder against another's matrix. **The build path was correct and the read path was not**, which is the shape that produces confident nonsense rather than an error: the matrix carries no labels of its own. Found by smoke-running the CLI after the suite was green, because 808 passing tests did not touch `main()`. Fixed through the same registry the build uses, TDD'd fail-first and mutation-checked. Same class as the `ladder/cache/` vs `ladder/cache` gitignore defect the day before: the tested path and the used path were not the same path.
...
- 2026-09-01 — **THE GAZETTEER SUBSET WAS DECIDED IN WRITING BEFORE ANY MEASUREMENT, AND ALL THREE OPTIONS WERE PRICED.** GeoNames holds 13.4M entries; a vocabulary built from the 575 names this corpus happens to mention would make `exists()` vacuously true and reduce retrieval to a choice among 575 — the same class of error as the 20-tag menu that silently capped the FiNER run. Gold coverage of the 575 distinct canonical names: **`cities15000` 37.4%**, **plus `countryInfo`/`admin1`/`admin2` 55.5%**, **full `allCountries` 98.1%**. No curated cut reaches usable coverage because the gold contains oceans, mountain ranges, stadiums, airports and buildings alongside cities — `Atlantic Ocean`, `Appalachian Mountains`, `Anfield`, `Bank of England`, `Amsterdam Airport Schiphol`. Chose the full 13,463,738-row dump: **97.8% of mentions resolve** (2,346 of 2,399). The size is also the point — 13.4M against SNOMED's 129,675 makes retrieval two orders of magnitude harder, so a lane that survives it is better evidence than one that does not have to. **MAIN NAME ONLY**, not the alternate-names table: the alternates hold the demonyms and exonyms that are 30% of gold, so including them would lift the ACCEPT rate substantially and is a **separate declared arm**, not a default. Also measured and worth watching: **1,117 of 2,399 mentions (47%) have a canonical name held by more than one GeoNames entry** — three Parises. The pick step here is disambiguation among identically-named candidates, a different failure mode from CADEC's retrieval from a large space.

- 2026-09-01 — **`ladder/registry.py` TURNED OUT TO BE CORPUS-AGNOSTIC, SO THERE IS NO `vocab_geo.py`.** Its two tables — `concept(id, active, is_finding, is_finding_hist)` and `description(concept_id, term, norm, fsn)` — carry nothing SNOMED-specific. A GeoNames index built into the same shape by `scripts/build_geo_index.py` (13.4M rows, 1,656 MB, 55s) is read by the **same `Registry` class** with the same `exists`, `terms`, `preferred`, `lexical_match`, `search`, `codes_for_term` and `shortlist`. That is not merely convenient: the third corpus is checked by the **same code** as the first, so the comparison is between vocabularies rather than between two implementations of a check. Two of rung 1's three free checks are **vacuous** here — GeoNames has no retirement and no hierarchy, so `is_active` and `is_finding` are true by construction, exactly as on FiNER, and only `exists` and `lexical_match` do real work.

- 2026-09-01 — **THE VOCABULARY GATE ASSERTED A SNOMED CONSTANT AGAINST EVERY CORPUS, AND HAD NEVER BEEN GENERALISED FOR FiNER EITHER.** `cmd_init` ran `assert reg.exists("162031009") and not reg.exists("999999999")`. The first is SNOMED's |Abdominal pain| and means nothing to a gazetteer or to a tag set; the gate could only ever pass on CADEC. Now `vocabulary.gate.{real,fake}`, manifest-declared, defaulting to the CADEC values so nothing frozen moves. **Same class as CADEC's exclusion list being applied to every corpus** (found 2026-08-30) — a single-corpus constant living in corpus-agnostic code, invisible until a second corpus reaches it. Two found now; worth grepping for a third. Related: `.gitignore`'s `!data/splits/` exception was also CADEC-shaped and silently ignored `data/*/splits/`, so **frozen split ids for the non-CADEC arms were never committed** — frozen ids that are not committed are not frozen. Generalised to `!data/*/splits/`.

- 2026-09-01 — **`scripts/preflight_rungs.py` CAUGHT A BUG IN THE ADAPTER IT WAS POINTED AT, IN SECONDS, BEFORE ANY GPU TIME WAS BOOKED.** The first `corpus_geo.load_corpus` left `sct` holding the canonical **name** and resolved names to GeoNames ids in a helper only `__main__` called. Every other caller therefore saw records whose code was `"Republic of France"`, and the preflight reported **452 of 452 gold codes failing the free existence check — 100% of a perfect answer set**. That is impossible on its face, which is what made it visible; a loader that returns records the vocabulary cannot read is not loaded, and resolution now happens inside `load_corpus`. The tool exists to say *don't build this rung* before a rung is built, and its first real use said *your loader is broken* before a run was booked. Recorded because it is the strongest available argument for the tool being a product rather than a script — and because it was found on the free side, which is the whole design.
- 2026-09-01 — **B4, PRE-REGISTERED BEFORE THE FIRST RUN: break FiNER's slot-0 position prior with a per-mention menu permutation. `rung0_menu_order: "shuffle"`, `manifest.finer.shufflemenu.json`, off in both shipped manifests, 22 tests, twelve mutations killed.** Written down first because a null is only readable if the prediction was.
  - **THE MECHANISM CHOSEN, AND WHY IT BEAT THE OTHER ONE.** Two candidates were registered: a sentinel slot 0 that is never a valid answer, and a per-mention permutation under a fixed seed. **The permutation ships**, because it changes ORDER AND NOTHING ELSE: the answer set is untouched, menu recall stays 1.000 by construction, and no option exists that cannot be picked. A sentinel ADDS an option that is not an answer, so a pick of it has to be mapped to a null code — mixing a position effect with an abstention effect, two changes in one arm, and a null that could not be attributed. It is also the option-order-bias literature's own mitigation. It is explicitly **not a better ranker**: a better ranker was measured on 2026-08-30 and lost, because what it displaced was better than it.
  - **CONSTRUCTION.** Permutation is a pure function of `(manifest.seed, record_id)` through blake2b — NOT `hash()`, which is salted per process and would make two runs of one configuration incomparable and every paired draw a different experiment. Per MENTION, not per run: one shuffle for a whole run only moves the attractor to a different tag and leaves it there. Each candidate carries `shuffled_from`, its pre-permutation slot, so "the menu actually moved" is checkable on the artifacts. The seed is the manifest's EXISTING top-level `seed`, so the arm stays a one-key diff and does not smuggle in a second declaration.
  - **THE PREDICTION.** (1) `AccrualForEnvironmentalLossContingencies` falls from **57 predictions in 292 toward gold's 2** — it now reaches slot 0 in ~1/139 of records instead of 100%. (2) **Slot-0 selection falls from 20.4%** toward the rate at which slot 0 happens to hold the right tag. (3) Detection is byte-identical by construction — the arm touches only the pick — so **the whole delta is coding accuracy**, and detection moving at all is a bug, not a result.
  - **WHAT WOULD REFUTE THE MECHANISM.** The prior is a POSITION prior, so: (a) slot-0 selection **staying near 20.4%** under a permuted menu would mean the model is anchoring on the slot rather than on any tag, and the attractor was a coincidence of which tag sat there — the arm then trades one arbitrary tag for a uniformly random one and coding accuracy should FALL, not rise; (b) the attractor's count staying near 57 would mean it is SEMANTIC after all and the context arm's evidence was misread; (c) coding accuracy moving while the attractor count does not is the arm doing something other than what it claims.
  - **AND THE PREDICTION THAT IS NOT A WIN.** Killing the attractor removes 55 spurious predictions, which raises precision on the false-positive side. It does not follow that F1 rises: the 55 records must then be coded by the model's own unranked reading, which the context arm measured at **0.457** against slot-0's 0.087 — better, but not a certainty, and the permutation also destroys nothing valuable only if alphabetical order carries no signal. On FiNER it plainly carries none. **On CADEC the same key would be actively harmful** and the arm is off there, because that menu is retrieval-score-ordered and the same prior lands on the BEST candidate — which is exactly why alphabetising it cost 10-12pt.
  - **PROTOCOL.** Three draws, base first at each draw on a shared cache so the arm's find calls are hits and detection is held fixed; paired bootstrap over documents; **detection and coding reported SEPARATELY**. Because the base runs first at each draw, its p95 latency includes cold find calls and the arm's does not — **the two p95 figures are not comparable and will not be compared**. `shuffled_from` plus a menu diff is the no-op check, since a held-fixed layer and a dead arm both print Δ = 0.000. There is no go/no-go probe: FiNER's menu recall is 1.000 by construction, so there is nothing to probe and no stop condition — which also means B2's denominator trap cannot be walked into here.

- 2026-09-01 — **B4 DRAW 0, AND IT REFUTES THE FINDING IT WAS BUILT TO FIX: THE SLOT-0 ATTRACTOR IS OUR OWN FALLBACK RULE, NOT THE MODEL.** `arm-finerbase-d0` vs `arm-finershuffle-d0`, dev, 40 docs, 165 gold, rungs 0-1, cold cache, base first, 313 predictions both sides. The pre-registered prediction was half right in a way that inverts the conclusion.
  - **THE ATTRACTOR COUNT FELL EXACTLY AS PREDICTED — 77 → 2, gold's own count — AND THAT IS THE PROBLEM.** Decomposing which lane produced those 77: **74 of them are `pick_fallback`, and 3 are the model.** `r0._fill_from_menu` (landed 2026-08-28, two days BEFORE the attractor was reported) fills any record the pick left uncoded with **menu position 0**. On FiNER the menu is `sorted(set(tags))`, so position 0 is `AccrualForEnvironmentalLossContingencies` in every record, and the fallback writes it 74 times. **The model's own slot-0 selection rate is 3/231 = 1.3%, against a 1/139 = 0.72% chance rate.** There is no position prior at slot 0 to break.
  - **SO THE 2026-08-30 ENTRY IS WRONG WHERE IT MATTERS.** "19.5% of all predictions are the list's first line" is right as arithmetic and wrong as attribution: it is 19.5% of predictions written by rung 0's own fallback, not chosen by the model. And the discriminator that looked decisive — under the context arm the tag is predicted 3 times, "all 3 in the records where the ranking happened to put it first" — is the SAME artefact seen from the other side: the fallback always writes slot 0, so every fallback-written prediction is at slot 0 by construction, whatever the ordering. **The evidence was consistent with the position hypothesis and was also fully explained by a line of our own code.** The 2026-08-30 entry stands as a description of the artifacts and is superseded as an explanation of them. The original run's artifacts are gone (2026-08-31 cleanup), so the 57/292 split cannot be re-decomposed directly — but `_fill_from_menu` was in the code, on by default, and the structure reproduces here at 74/77.
  - **THE ARM IS REJECTED, AND NOT NARROWLY.** Detection byte-identical by construction (F1 0.502 both sides, 313 predictions, no-op check: all 313 menus moved, 0 contents changed). **Coding accuracy 0.425 → 0.058. F1 exact 0.213 → 0.029, paired delta −0.1841 [−0.2531, −0.1094] over documents.** Restricted to the model's OWN picks, excluding the fallback lane: **0.476 → 0.083**, and the model answers less (103 → 84 of 120 matched spans). `no_pick` 6 → 38.
  - **AND THE MECHANISM IS AN INDEX-ALIASING ARTEFACT THE ARM CREATED, NOT THE PERMUTATION'S POINT.** The pick is BATCHED — `rung0_pick_batch: 7` — and with `rung0_retrieval: full` every mention in a call sees the SAME 139-line menu, so "choice 42" means one tag throughout the call. A PER-MENTION permutation puts seven DIFFERENT orderings in one call. Measured on the arm's own artifacts: in **20 of 77** model mis-codes the chosen index is one at which the CORRECT tag sits in a SIBLING mention's menu in the same call — **26.0% against a 3.7% null** (2,000 reps, the chosen index replaced by a random one; observed 20, null max 9; **p = 0.0005**). The model reaches the right concept and reads it off the wrong permutation.
  - **REJECTED AS AN EXPLANATION: alphabetical prefix clustering.** The obvious alternative was that camel-cased XBRL sorts `DebtInstrument*` together and the model navigates by prefix. It does not: base mis-codes sit at median alphabetical distance **44** from gold against ~46 for uniform, only 17.0% land within five slots, and the ARM's mis-codes are alphabetically CLOSER (median 29), which is the wrong sign for that story.
  - **WHAT THIS MEANS FOR THE FALLBACK, WHICH IS THE LIVE DEFECT.** `_fill_from_menu` was measured on CADEC, where position 0 is the top DENSE-RETRIEVAL hit and the note's "+0.015 exact, it cannot cost a correct answer" is sound. On FiNER position 0 is an ALPHABETICAL accident, and the rule writes one arbitrary tag onto 74 of 313 records — 24% of the run's answers — at an accuracy of 2/17 on matched spans. It cannot lower F1 under this scorer, so it is invisible in the headline; it is highly visible in every artifact, and it manufactured a finding. **A default tuned on one corpus's ordering semantics became a claim about a model on another's.**

- 2026-09-01 — **B4 FINAL, THREE DRAWS: the shuffle arm is REJECTED, the three draws produced ONE distinct outcome per side, and the fallback result above is confirmed on all of them.** `out/finer/arm-finer{base,shuffle}-d{0,1,2}.*`, driver `out/finer/draws.sh`, analysis `out/harness/slot0.py`, report `out/finer/slot0-report.json`.
  - **ALL THREE BASE DRAWS ARE BYTE-IDENTICAL (sha256 `f479bd0f…`) AND ALL THREE ARM DRAWS ARE BYTE-IDENTICAL (`b0d591f0…`).** Every reported figure is therefore the same at d0, d1 and d2: base F1 exact 0.2134 / overlap 0.2176, detection F1 0.5021, coding 0.4250, 313 predictions, 51 correct; arm F1 exact 0.0294, detection F1 0.5021, coding 0.0583, 313 predictions; **paired delta −0.1841 [−0.2531, −0.1094]** exact over documents. **This is the first FiNER measurement with NO run-to-run variation at all**, and it corroborates the 2026-08-31 result rather than adding to it: the spread there was one refused document, and no document was refused on any of these six runs (`FINER.test.0013`, which held 21 of 165 gold and was refused on the old d0, was answered every time). Six zero-mention documents all have zero gold — the model correctly found nothing in each, which is the capability `natural_rate` sampling exists to test. **Report this as "three draws, one outcome", never as a tight confidence interval: the interval is the paired bootstrap over documents, and the draws contribute nothing to it.**
  - **The no-op check passes on every draw** — 313 of 313 menus moved, 0 menu contents changed, 0 records only in one arm — so the Δ = 0.000 on detection is a held-fixed layer and not a dead arm, which is the distinction this project has to make explicitly.
  - **COST, THREE SEPARATE MEASURES AND NOT FUSED.** Tokens 927,418 (base) vs 931,047 (arm), 108 calls each. Rung 0 p95 131.4 s vs 93.2 s — **and these two are NOT comparable and are not compared**: the arm ran second on the base's cache, so its find calls are hits. Records routed to a person is NOT REPORTED, because rungs 5 and 6 did not run: this arm is `--rungs 0,1` on purpose, since rung 3 resamples rung 0 and would put sampling noise between the two sides. Absent, not zero.
  - **THE FALLBACK, PRICED.** `arm-finernofb-d2` (`manifest.finer.nofallback.json`, one-key diff, test-pinned) ran in **2.7 seconds** — every call a cache hit, because the rule fires after the pick reply. Answered records **313 → 239**. **The fallback lane is 74 records carrying ONE distinct code**, `AccrualForEnvironmentalLossContingencies`, 74 times. Turning it off costs **F1 exact 0.2134 → 0.2050, −0.0084 [−0.0279, +0.0000]**, correct 51 → 49 at unchanged detection: on FiNER the rule converts **2 of 74 writes**. So it is not, in fact, free-and-harmless here — it is nearly free and it is what made 24% of a run's answers a constant. **The scorer cannot see this**, since a filled record and an empty one are both not-CORRECT and both already in `n_pred`, which is exactly why it survived a corpus port, a five-model sweep and two arms before anything noticed.
  - **THE ARM STAYS OFF AND SO DOES ITS SUCCESSOR IDEA.** `rung0_menu_order: "shuffle"` ships off in both manifests. It is not rejected as "randomisation does not help" — it is rejected because **there was no position prior to mitigate**, and because a per-mention permutation is unsafe under a BATCHED pick: seven different orderings in one call, and 26.0% of the arm's mis-codes read the right concept off a sibling's permutation (p = 0.0005). If anyone revisits option-order randomisation on this ladder, the batch is the constraint — permute per CALL, not per mention — and they should first check whether the effect they are mitigating is their own code.
  - **THE REUSABLE LESSON, and it is the third one this project has had of this shape.** A go/no-go probe must use the arm's own denominator (B2). A declared setting must be read (temperature). **And now: a metric computed over a rung's OUTPUT must be decomposed by which LANE produced each row, because a rung's own defaults are in that output and they do not look different from the model's answers.** `checks.pick_fallback` was on every record the whole time; nobody grouped by it. The 2026-08-30 entry stands as a description of the artifacts and is superseded as an explanation of them.

- 2026-09-01 — **B4 fallout for the CONTEXT-MENU arm: its REJECTION stands, its MECHANISM does not, and it cannot be re-checked.** The 2026-08-30 diagnostic reported "the pick lands on menu slot 0 in 20.4% of records under the alphabetical baseline and 50.2% under the context ranking" and read it as the model following the ranker. **Both rows mix two lanes**: `_fill_from_menu` writes menu position 0 on every record the pick left uncoded — 74 of 313 on the reproduced base, 24% — and it does so under BOTH orderings, so a share of "the pick lands on slot 0" is not a pick. The same contamination reaches "accuracy when slot 0 is picked, 0.087 → 0.373": under the context arm the fallback is writing the RANKER's top hit, so part of that rise is our rule inheriting a better default, not the model deferring to a ranking. **The rejection is unaffected** — coding accuracy 0.393 → 0.304 is measured over the same records with the same lanes on both sides — but the sentence "the model plainly follows the ranker" is not supported by a lane-decomposed number and is now caveated in both article files. **It cannot be repaired from disk**: `out/finer/arm-finer{base,ctx}-d{0,1,2}` went in the 2026-08-31 cleanup, so re-deriving it means four runs at ~40 min. Not run this session; recorded as owed.
- 2026-09-01 — **The article files are corrected, not softened.** `docs/article-v3.md` (the live draft) and `docs/article.md` (the longform build report) both led with the slot-0 attractor as a model-behaviour finding. Both now carry the refutation in place of the claim, with the pre-registration, the lane table, the batched-pick artefact and the fallback's price — and both end the section on the transferable lesson, which is **not** position bias: *a metric over a component's output must be decomposed by which lane produced each row.* The related-work paragraph is rewritten too: it previously claimed option-order randomisation as a mitigation we arrived at independently, which is true and now reads as what it was — we matched a documented pattern to our numbers before checking whether our own code had produced them. The 19.5% figure is kept where it appears, as arithmetic over the artifacts, and never as a claim about the model.
- 2026-09-02 — **THE ACCEPT LANE INVERTS ON A GAZETTEER: BAND OUTSCORES ACCEPT IN ALL THREE MODELS.** GeoWebNews test split, 60 documents, three models at three draws each, scored against the answer key's own coordinates at the conventional 10 km cut. ACCEPT vs BAND correctness: `gpt-oss:20b` **0.206 / 0.250**, `llama3.1:8b` **0.134 / 0.330**, `mistral:7b-instruct` **0.214 / 0.370**. On CADEC the ACCEPT lane runs 80–89% and BAND 15–36%; here the ordering reverses, consistently, on every model. **The mechanism is name uniqueness, and it is a precondition nobody stated.** Rung 1's ACCEPT means *the vocabulary uses these very words*. On SNOMED that is evidence, because a clinical term is distinctive — very little other than |Chronic pain| is called "chronic pain". On GeoNames it is nearly worthless: "London" matching an entry named "London" says nothing about WHICH London, and 1,117 of 2,399 gold mentions carry a name more than one entry holds. **The free check fires most confidently exactly where ambiguity is highest**, which is worse than not firing — it is the endorsement machine of the `contained` setting, arriving from a different direction. The ~85% ACCEPT lane therefore needs TWO preconditions, not one: the span and the code must be lexically comparable (which FiNER lacks), AND the vocabulary's names must be discriminative (which GeoNames lacks). Only the first is in the article.

- 2026-09-02 — **CORRECTNESS FALLS MONOTONICALLY WITH LEXICAL OVERLAP, ON THREE INDEPENDENT MODELS, WITHIN ONE CORPUS.** The arm's primary hypothesis, and it holds. Correctness within 10 km by overlap stratum, computed from gold before any run and carried on `GoldMention.cadec_type`:

  | stratum | share of gold | gpt-oss:20b | llama3.1:8b | mistral:7b |
  |---|---|---|---|---|
  | identical | 40.5% | **0.305** | **0.274** | **0.309** |
  | subset | 24.8% | 0.158 | 0.204 | 0.140 |
  | partial | 4.9% | 0.143 | 0.182 | 0.143 |
  | none | 29.9% | **0.070** | **0.079** | 0.150 |

  Monotonic in two of three models and directionally right in the third, with the identical stratum roughly **4× the no-overlap stratum**. This is the mechanism CADEC and FiNER could only bracket: those two arms are one point each on the axis, so the axis was confounded with task, vocabulary, prompt and model. Here it is a **controlled variable** — same corpus, same prompt, same check, same model, four regimes — and the gradient survives all three models. The zero-overlap stratum is not noise but four systematic phenomena: demonyms (`French` → Republic of France), abbreviations (`US`, `EU`, `DPRK`), exonyms (`Moscow` → Moskva), and metonymy (`Congress` → United States Capitol, the annotators resolving the building an institution sits in).

- 2026-09-02 — **THE MENU HELD THE RIGHT PLACE 42.5% OF THE TIME, SO THE HEADLINE NUMBERS PRICE OUR RETRIEVER AND NOT THE LADDER.** recall@20 measured the way the predictions are scored — a candidate counts if it is within 10 km of the annotators' coordinates — over 200 test mentions: **85 of 200 = 42.5%**, against CADEC's 86.1%. The models score 0.218 / 0.220 / 0.240, so they are converting roughly **half of what they are actually offered**, which is not poor picking against that ceiling. **The cause is the deviation declared before the run:** `manifest.geo.json` uses `rung0_retrieval: "lexical"` because dense retrieval is cosine over an embedded keyword table derived from SNOMED, and no such table exists for GeoNames — building one means embedding 13.4M place names. On CADEC that same substitution was measured to cost **21.0 points of recall@20** (65.1% vs 86.1%). So no absolute score from this arm is comparable to CADEC's, and **the arm's findings are the two above — both of which are about SHAPE and survive the ceiling — not any F1**. Reported here rather than buried, because a lower geo score has an obvious wrong reading ("the ladder does not travel") and a correct one ("we gave it a worse retriever").

- 2026-09-02 — **ALL THREE MODELS ARE BIT-REPRODUCIBLE ON GEOWEBNEWS, INCLUDING THE ONE THAT IS NOT ON CADEC.** Three draws each, `sample_index` varied so the disk cache cannot serve an old answer, temperature 0: every draw produced an identical record count and identical scores — `gpt-oss:20b` 411 records ×3, `llama3.1:8b` 412 ×3, `mistral:7b-instruct` 212 ×3. **`gpt-oss:20b` is the Mixture-of-Experts model that gave three DIFFERENT files and a 1.3-point exact-F1 spread on CADEC**, and the article's reproducibility finding rests on that. It does not vary here. Not explained, and recorded as an open question rather than reconciled: the corpus is shorter per document, the retrieval is lexical rather than dense, and the answer space is different, so at least three things changed at once. It does narrow the claim — "this model is not run-to-run reproducible" is false as stated and should read "was not, on that corpus, under that configuration".

- 2026-09-02 — **THE ANSWER KEY GAVE COORDINATES AND THE ADAPTER THREW THEM AWAY, WHICH INVENTED A GOLD LABEL.** `gwn_full.txt` carries a canonical NAME plus a LATITUDE and LONGITUDE per mention, and no geonameid. `corpus_geo.resolve_ids` filled the gap with `codes_for_term(name)[0]` — the first GeoNames row with a matching name. That is arbitrary where a name is ambiguous, and 1,117 of 2,399 mentions are: the id it chose for `Brooklyn` is a Brooklyn in **South Africa**. Scored against those invented ids, **97 of the first run's 240 "wrong" answers carried a label identical to gold's** — the system was being marked down for choosing a different row with the same name. `scripts/score_geo.py` ignores the invented ids and scores against the coordinates the annotators actually recorded, which is the standard geoparsing metric. **The lesson is not about geography.** A loader that supplies a gold label the answer key did not provide is generating evidence, and the resulting number looks exactly like a measurement. It was caught only because a 3,000 km median error is absurd on its face; a subtler substitution would not have announced itself.
- 2026-09-02 — **FiNER'S RUNG 1 WAS NOT DEAD BECAUSE THE CORPUS HAD NO SIGNAL. IT WAS DEAD BECAUSE WE ONLY WROTE ONE KIND OF CHECK.** The lexical check compares the extracted span's words against the code's own name, and on FiNER that is a **structural** zero rather than a low score: the spans are numerals (`47.6`) and the tags are English phrases (`EffectiveIncomeTaxRateContinuingOperations`), so the token sets are disjoint by construction and the intersection is empty for every possible run, model and prompt. ACCEPT 0 of 704, rung 1 rejected 1 record in 704, and rungs 2 and 5 went quiet with it — four rungs silent because one check had no signal.

  **A second deterministic relation was available the whole time, and both sides carry it.** Tag names encode a type (`…Percentage`, `…Amount`, `…Shares`, `…Date`, `…Term`) and so does a span's immediate context (`1.50 %`, `$ 19.4 million`, `1,350,000 shares`, `January 9 , 2023`, `6.8 years`). A percentage tag on a span followed by "shares" is **provably wrong** in exactly the sense rung 1 exists for — deterministically, at zero model cost, without knowing the right answer.

  Measured on the FiNER test split's 187 gold mentions, `scripts/finer_type_check.py`, no model calls:

  | | lexical check | type-compatibility check |
  |---|---|---|
...
- 2026-09-02 — **The 30 run outputs deleted by the cleanup are back, in `runs/archive/`, tracked.** 29 went with `7201dc5` (dead-code sweep) and `ladder.ledger.jsonl` with `1a3437a`; all 30 restored byte-identical from their parent commits and verified by sha256. The `runs/*` ignore rule STANDS — the runners' write directory is still scratch — and `runs/archive/` is a single negated exception beside it, so the two never interact and a future sweep cannot take these again. They contain no corpus text (doc ids, codes, token counts, latencies), and preflight passes on all 30. Nothing reads them; they are the raw material behind the entries dated before 2026-08-25, which is the one thing the 2026-08-31 deletions cost.

- 2026-09-02 — **The `ibm/` judge prefix was reverted by `829a762` and it took CI red; restored in four manifests.** `8eed4e7` ("A mistyped model name must not cost 133 minutes of compute") fixed `manifest.finer.json`'s judge to `ollama/ibm/granite4:micro-h`; the rung-7 commit flipped it back to `ollama/granite4:micro-h` and added a `_judge_note` asserting the prefixed name "is not a name ollama has" — which contradicts the `_judge_name_note` sitting directly above it and is false: `ollama list` has `ibm/granite4:micro-h` and no bare `granite4:micro-h`. The wrong name had spread to `manifest.finer.typecheck.json`, `manifest.geo.json` and `manifest.geo.smoke.json` too. All four now carry the prefix and the contradictory note is gone.

  **Three tests were already red for this and nobody had run them** — `test_no_tracked_manifest_names_a_model_ollama_does_not_have` globs every `manifest*.json` and its docstring names the exact defect, so the guard for this was written before the regression landed and still did not stop it. The third failure is the informative one: `test_the_ctxmenu_manifest_differs_by_exactly_the_menu_order` went red because the arm manifest kept the correct name while its base lost it, so a one-key diff became a three-key diff. **An arm-pinning test is also a regression detector on the base** — it fails when the base moves under the arm, which is how a manifest edit that touches no arm still gets caught.
- 2026-09-02 — **THE MANIFEST IS THE ONE PLACE A MODEL IS NAMED, AND THE NAME THAT RESOLVES DEPENDS ON WHICH MACHINE PULLED IT.** Owner A's machine has `granite4:micro-h`; Owner B's has `ibm/granite4:micro-h`. **Both are digest `076afb3855dc`, 1.9 GB — the same blob under two names.** Each of us then "fixed" the manifests to match our own box: `829a762` removed the `ibm/` prefix from four manifests after the availability guard refused to start, and `df531c1` restored it with three red tests behind it. Both changes were correct locally and neither was correct globally.

  **Owner B's restore is the one that stands**, and the reasoning is recorded there: `8eed4e7` had already fixed that name after it burned 133 minutes of rung 0 and rung 3 compute before dying at rung 4, the rung 7 commit flipped it back, and the `_judge_note` added then **directly contradicted a `_judge_name_note` sitting immediately above it** — a note written without reading the note it disagreed with. Three tests were red for this on main and were not run before the revert.

  **The finding underneath is worse than the config churn.** `ladder/llm.py` says *manifest.model is the ONE place a model is named*, and that claim cannot hold while the resolvable name is a property of the machine. The provenance stamp records the **requested** name, not which name resolved and not the digest that ran — so two runs can record different models and be the same model, or record the same model and be different ones. **Provenance that records a request rather than a resolution is not provenance.** The fix is to stamp the digest; recorded rather than done, because every existing run's stamp would then be incomparable with every new one and that is a decision for the article, not for a Wednesday.

  Resolved for now by pulling both names locally, which is presumably what Owner B's machine already had and why the divergence went unnoticed for eleven days.

  **The informative failure is the test that caught it**, and it is worth stating on its own: `manifest.finer.ctxmenu.json` is pinned by a test asserting it differs from its base by exactly one key. The arm kept the correct model name while the base lost it, so the pinned one-key diff became three. **An arm-pinning test doubles as a regression detector on the base** — a property nobody designed and everybody should want.

- 2026-09-02 — **THE COST ESTIMATOR WAS WRONG THREE TIMES, AND THE ARITHMETIC WAS RIGHT EVERY TIME.** `scripts/cost_estimate.py` projects a full run from a smoke run's own timings, because every GPU session in this project has mis-estimated itself — the geo arm was projected at 3–4 hours by hand, re-projected at 15 after the first measurement, and ran in 7. Built against a known answer: six runs of the type-check arm, which took **52 minutes**.

  | projection | why it was wrong |
  |---|---|
  | **51 hours** | projected a LAPTOP smoke run onto rented-GPU work. A 4 GB card with the model half on CPU is ~18x slower; the tool cannot see which machine produced a ledger and did not say so |
  | **171 min** | multiplied by six runs as though each paid full price |
  | **132 min** | modelled the cache per-RUNG — "rung 0 is free after the first draw" |
  | **57 min** | modelled it per-RUN, which is what actually happens |

  **The measurement that settled it:** across three base runs of one arm, run 1 cost 1,706 seconds and runs 2 and 3 cost **exactly zero** — not mostly cached, entirely cached, every rung. The cache key is `(model, messages, temperature, sample_index)`, so a run repeating another's configuration pays nothing at all. The question is therefore not which rungs cache; it is **how many runs are new**.

  57 against 52 is within 10% on a duration already known, which is the only test an estimator can pass.

  **The lesson is the project's own, arriving from the inside.** Every one of the three wrong numbers was arithmetically correct. What was wrong each time was the **model of how the runs work** — the same failure this repository documents in rung 4's agreement figure, in the pooled drug/reaction ratios, and in every rate computed over an unnamed set. A number can be computed perfectly and describe nothing, and there is no way to notice except by checking it against something you already know.

- 2026-09-02 — **`scripts/manifest_diff.py`: THE ARM-PINNING CHECK, MECHANISED — AND IT IMMEDIATELY FOUND A SECOND VARIABLE IN AN ARM ALREADY RUN.** An arm is a claim of the form *one thing changed, and here is what it bought*, and nothing in this repository checked that one thing changed. The spine ablation (2026-08-31) differed in `rung_order` **and** in rung 0's configuration, which would have charged a 5.9-point rung-0 gap to the rungs it removed; it was caught by hand, key by key, because someone thought to look.

  Run against `manifest.finer.json` → `manifest.finer.typecheck.json`, declaring `rung_order`: **one declared difference and five undeclared ones** — the whole `corpus.r2_prompts` block, added because rung 7 woke a rung 2 whose prompt named SNOMED CT at a model reading SEC filings. That block was necessary and it is still a second variable, and the rung 7 result was reported without it being named.

  It ignores documentation keys (`_`-prefixed and `*_note`, of which this repo has hundreds, one of them several hundred words), ignores `output.dir` and run ids, separates the declared variable from the rest, and exits non-zero on an undeclared difference so it can gate a run. Two hours of work against a confound that has already landed once and nearly landed twice.
- 2026-09-02 — **RUNG 7 REJECTED: A CHECK TUNED ON GOLD AND VALIDATED ON GOLD IS 29× WORSE ON MODEL OUTPUT.** The type-compatibility check built this morning was run as a proper arm — FiNER dev split, 40 documents, base `[0,1,2..6]` against `[0,1,7,2..6]`, three draws each, rung 0 identical at each `sample_index` so the two arms share byte-identical extraction. **It fails on both axes and is not enabled.**

  **It rejects correct answers.** Pooled over three draws, rung 7 rejected 42 records with gold to check against: **27 were wrong (correct rejections) and 15 were right (FALSE rejections)**. A false-rejection rate of **35.71%** on model output, against **1.22% measured on gold** before the rung was written, and against rung 1's own 0.13% on CADEC. A check that discards a correct answer once in three is worse than no check.

  **And it changed nothing.** Answered accuracy **0.508 in both arms, all three draws, zero spread.** Rung 7's 44 rejections per run never reached the output because rung 5 abstains on every record on this corpus regardless — the rejections were absorbed by a rung already withdrawing everything.

  **WHY GOLD LIED, and this is the transferable part.** The check reads a span's type from the characters either side of it: `%` after, `$` before, `million`, `shares`, `per share`. On gold every span sits exactly where the annotator put it, so that context is always the right context. **The model's spans drift** — a character or two off, a different token boundary, a longer or shorter quote — and the type rules then read the wrong window and reject *confidently* on a misreading. The rules were tuned on gold and measured on gold, so of course they looked good: **the measurement set was the tuning set.**

  This is the reranker's lesson (2026-08-31) arriving from a different direction. There the offline probe separated over 1,144 documents and the arm ran on 38, and the sign flipped on the arm's own denominator; the recorded rule was *a go/no-go probe has to be run on the denominator the arm will be scored on*. **Here the denominator was right and the INPUT was wrong** — gold spans against model spans — which is the same failure with a different name. The rule generalises: **a check must be validated on the kind of input it will actually see.**

  A single-draw replay over a finished run gave 11 of 11 correct rejections and was read as encouraging. Three draws and 40 documents gave 27 of 42. The 11 was a small-sample artefact and was not flagged hard enough at the time.

  **WHAT SURVIVES.** The finding recorded earlier today — *FiNER's rung 1 was dead because we wrote one kind of check, not because the corpus had no signal* — **still stands and is unaffected.** 87.7% of gold mentions carry a readable type on both sides; the relation exists. What fails is this particular implementation of it against model output. The article's precondition should still be widened from *the span and the code must share a language* to *there must be SOME deterministic relation*, and this arm is the evidence that finding one is harder than noticing one.

  **AMENDED after `scripts/manifest_diff.py` was written, the same day:** this
  arm declared `rung_order` and changed **six** keys. The other five are the
  whole `corpus.r2_prompts` block, added because rung 7 woke a rung 2 whose
  prompt named SNOMED CT at a model reading SEC filings. The block was necessary
  — the arm could not run without it — and it is still a second variable, and
  the result above was reported without it being named. It is unlikely to
  explain a 35.7% false-rejection rate, since rung 7's verdict is computed
  before rung 2 is reached. But "unlikely" is an argument, not a measurement,
  and this arm's claim to have tested one thing is weaker than it was written.

  Kept in the tree, not enabled: `ladder/rungs/r7.py`, `manifest.finer.typecheck.json`, `scripts/finer_type_check.py` and 25 tests. `rung_order` in `manifest.finer.json` is untouched, and `--rungs` defaults to `0-6`, so the rung is unreachable without asking for it twice.

- 2026-09-02 — **RUNG 2 FIRED 918 TIMES ON FiNER AND RESCUED NOTHING — the CADEC null, now on a real trigger set.** Rung 7 gave rung 2 the trigger it has never had on this corpus: 44 rejections per run instead of CADEC's one in 248. Outcomes pooled over three arm draws: **789 unchanged, 129 declined, 3 parse-failed, 0 rescued.** The model was told a specific, stated fact — *the text "10.85" names a percent, but DebtInstrumentFaceAmount names a money amount* — and returned the same answer 86% of the time and refused 14%.

  **This is the stronger form of a result the project already had.** CADEC's rung 2 rescued nothing, and that null could always be waved away: it fired once in 248 records, so the trigger set was too small to conclude from. Here it fired on hundreds and still rescued nothing. Whatever else is true, **stating a correct fact back to the model does not make it produce a different answer** — which is the same shape as the 2026-08-28 finding that restating an instruction the model already ignores buys nothing, three for three.

- 2026-09-02 — **FOUR DEFECTS IN CODE THAT HAD NEVER RUN, FOUND IN ONE AFTERNOON BY MAKING A DEAD RUNG FIRE.** Rung 2 has fired once in 248 records on CADEC and never on FiNER. Giving it a trigger surfaced, in order: **(1)** its correction prompt opens *"checked against SNOMED CT and the source post"* and closes *"if no SNOMED CT code is right for this reaction"* — sent, as written, to a model reading SEC filings; **(2)** `build_fact` formats a template with a fixed set of keys, so a new reason whose fact names other variables raises `KeyError`; **(3)** `correct()` does `float(m.get("confidence"))`, and the model answers `"high"` and `"low"`; **(4)** `RUNG_NAMES` is a hardcoded dict that raised `KeyError` **after** `apply()` had run and the ledger row was written, so the work happened and only the console line failed.
...
- 2026-09-07 — **THE CONSOLIDATED RE-RUN'S CORPUS-FREE FILES ARE IN THE REPO, SO THE ARTICLE'S NUMBERS HAVE A SOURCE ANYONE CAN OPEN.** Asked for: every script and file that generates a number in the article, tracked. Audit of the chain: run (`scripts/consolidated_rerun.sh`, `rerun_spine.sh`, and three drivers that lived only in the archive — now `scripts/rerun_all.sh`, `rerun_steps.sh` for the S0/S1 draws, `rerun_typecheck.sh` for the FiNER type-check arm) → `out/<run>.*` → `scripts/rerun_analysis.py` / `ladder/analysis.py` / `python -m ladder.probe` → `out/rerun/cadec.{md,json}` and the probes → the article. The scripts were tracked except the three drivers; the run and the reports were not, and both articles cited `out/rerun/cadec.md`, a gitignored path nobody else can open.

  What went in, under `runs/archive/consolidated-2026-09-03/` (the one tracked exception to the scratch rule, extended rather than a second one invented): for each of the **36 runs** — 15 CADEC (3 base, 9 arms, 3 spine), 6 S0/S1, 12 FiNER, 3 FiNER type-check — the four files that carry no corpus text: `.aggregates.json`, `.ledger.jsonl`, `.results.csv`, `.manifest.json` (19 MB; ledger `extra` keys enumerated — denominators, lanes, menu flags, minutes — the longest string value is a FiNER tag). Plus `rerun/cadec.{md,json}` in the **2026-09-04 regeneration** (the version with the gold-lane occupancy section that `article-v3-CADEC.md` was audited against; the archive's own copy is the 09-03 one without it, and a test pins the difference), the four `cadec-probe-*.json`, `cadec-s{0,1}.*`, `finer.*`, and the drivers' logs. The preflight scan over the whole directory is clean; the `text` values in the report JSON are extracted spans, which is the licence rule for examples.

  What did NOT go in, and why: records, state rows and call traces (corpus text — the archive stays under `out/`); the six matrix cells' run files (they exist only on pushpdeep's machine; `matrix.csv` is the tracked result, and the same four-file layout is the drop-in for them); the multi-corpus InfoQ draft's `analysis.shipping_rules` and figure script, uncommitted in another worktree on `claude/infoq-cadec-data-expansion-745bd8` and not touched from here. **A reader can open every number's source from git; recomputing a report still needs the raw run and the corpus.**

  Guard, TDD'd (`tests/test_runs_archive.py`, 130 tests, seen failing 127/130 before the files existed): the 36 run ids pinned, every run has its four files, aggregates name their run and the dev split, no records/state/calls file kind anywhere under `runs/archive/`, no ledger row carries a text-shaped key, the cited reports exist, the CADEC report is the audited version, the preflight scan passes, the three drivers parse under `bash -n`, and the two articles cite the tracked path.

- 2026-09-07 — **THE INFOQ FIGURES ARE DRAWN FROM THE TRACKED REPORT, AND THE ARTICLE'S SHIPPING-RULES TABLE REPRODUCES TO THE THIRD DECIMAL.** Asked how Figure 2 (`infoq-fig5-shipped.png`) was generated: by `docs/figures/make_infoq_figs.py` from seven hard-coded tuples whose comment cited a scratch file, `bars2.py`, that existed nowhere. The dial (`infoq-fig2-dial.png`) was the same. Nothing on any machine could regenerate either, and nothing could say whether the figure and the article's table agreed.

  **Recovered, then made production.** The definitions were re-derived against `rerun-cadec-d0`'s state rows: "ship everything" is the record set AFTER rung 3 (88 right code on the exact span, 28 exact span wrong code, 23 right code boundary off, 91 neither; F1 0.397 = `score_run` over the shipped subset); ACCEPT and the loose check read the final zone with correctness at rung 4, the last rung that changes a code, which is what makes "177 to a person, 49 correct" computable; the vote rules read `checks.r3_votes`, the judge rules `checks.r4_verdict` in the base and in the `-judgemenu` arm. **One definition was not what the sibling code on the multi-corpus branch assumed**: "all three samples agree" is three matching samples with none missing (the article's caption: a split or a missing sample goes to a person), 108 records on d0 — not "one code with at least two votes", which gives 146. `ladder.analysis.shipping_rules` (+ `shipping_rules_mean`) now computes every row: four-way split, to a person and how many of those were right, accuracy, yield over one denominator, F1 over the shipped subset, extra tokens of the rung read. TDD'd in `tests/test_analysis.py`; wired into `scripts/rerun_analysis.py` as a section of the report and a `shipping_rules_mean` block in the JSON.

  **Every row of the article's table reproduces**: everything 232.7 / 0 / 90.3 / 0.388 / 0.388 / 0.405; ACCEPT 52.3 / 180.3 / 0.771 / 0.173 / 0.292; loose 90.7 / 142.0 / 0.607 / 0.236 / 0.353; unanimous 115.7 / 117.0 / 0.495 / 0.246 / 0.345; two agree 186.3 / 46.3 / 0.426 / 0.341 / 0.396; blind judge 138.3 / 94.3 / 0.467 / 0.278 / 0.363; menu judge 141.3 / 91.3 / 0.545 / 0.331 / 0.430; extra tokens 424,832 (voting) and 85,436 (judge) against the article's "~420,000" and "~84,000". The four tracked reports were regenerated with the new section; every previously existing line is byte-identical. `make_infoq_figs.py` now reads `rerun/cadec.json` and **`infoq-fig5-shipped.png` regenerated byte-identical to the published PNG**; the dial moved by the rounding of its means (the "all 3 agree" point at 49.7% / 0.495 where 50 / 0.50 had been typed) and its token labels now say 425k / 85k. `tests/test_infoq_figs.py` pins the first draw to the published figure and refuses hand-typed counts in the script.

  **Documentation, asked for as "so the report can be regenerated from any machine", including the rungs.** `docs/REPRODUCE.md`: three layers — re-score from the tracked files (seconds, no corpus), re-derive the reports from the raw run (the exact four commands; the CADEC report came back byte-identical), re-run the rungs (models, what a run writes, the draw-and-arm protocol with its five scripts, determinism, and the path for new data through gatecheck, crosscheck, an adapter and a manifest). `docs/figures/README.md` maps every PNG to its `.dot` or script and its data; three older scripts (`fig0.py`, `fig2.py`, `build_geo_figures.py`) still carry literals and are named as such. README and CLAUDE.md point at both.

- 2026-09-07 — **`vocabulary.gate` copied into the thirteen arm manifests; the pipeline was red on main.** The gate codes added to the base manifests earlier today (`4d5a21c`) were not added to the arms, so every "differs from its base by exactly one key" test saw two keys — eight failures on a clean `origin/main`, verified. Each arm now carries its base's `gate` block verbatim (CADEC arms from `manifest.json`, FiNER arms from `manifest.finer.json`; the six-corpus manifests have their own). `tests/test_manifest_gate.py` names the rule those tests enforced by accident — an arm inherits its base's gate — and refuses a new CADEC or FiNER manifest that is not listed with its base. 13 failing before the copy, 1136 passing after in the CI venv.

- 2026-09-08 — **The InfoQ CADEC article checked against InfoQ's ten submission criteria; four fixed in the files.** (1) Word file renamed to the required `Group 3 – <title>.docx` (the old `article-infoq-CADEC.docx` removed; regenerate with the recorded pandoc command into the new name). (2) Authors named under the title — Wejdan Bagais and Pushpdeep Mishra — and in "About the authors"; the two bios are still to be supplied. (3) Key takeaways rewritten from 252 words to 129 (limit 130), five full sentences, same five claims. (4) Word count 3,543 → 2,998 excluding code (2,909 excluding the reference list too; limit 3,000): the division-of-labour table at the end folded into one paragraph (it restated evidence given above), the 4-in-6,492 planted-corruption figure and the "which 80 percent" line kept, and light trims in a dozen paragraphs — no number changed. (5) Every caption now ends "Image: the authors", and Figure 1 (`fig7-pipeline-cadec.png`) is rendered at 200 dpi by `make_infoq_figs.py` (752 → 1,566 px wide) instead of a one-off `dot` call. Not fixable from the files: the generative-AI disclosure (the article was drafted with Claude Code and must say so on submission), editor access on the uploaded document, and the medRxiv DOI, which answers 403 to every automated fetch and should be opened once in a browser. codespell over the markdown: zero findings.

- 2026-09-08 — **The InfoQ article's takeaways rewritten as five lessons, the body aligned with them, and the submission rules made a test.** Takeaways (130 words): judge layers on correct over all inputs; start with the free check; the model reads, never remembers; repeat runs as a validation step with an acceptance level the reader sets; study the data, not medicine. Two rounds of the owner's review removed every number a cold reader could not place (the 83 / 28 tiers, "one of 53") and the refusals point as a standalone lesson. **One conflict caught on the read-through**: a draft said the free check sorted answers "better than three paid layers" — the menu-shown judge separates more sharply (3.4–4.2× vs 2.7–2.8×) and is merely unread, so the takeaway now says "sorted them by trustworthiness" and `docs/INFOQ-SUBMISSION.md` forbids the stronger wording. Body changes: the third run's agreement with the byte-identical pair (70 percent of mentions, 84 percent of codes on shared spans) added to "We nearly published", since a takeaway cited it and the body did not; the Monday floor practice now says "decide what difference you will accept". `tests/test_infoq_article.py` enforces the six mechanical InfoQ rules — word budget, five takeaways ≤ 130 words as sentences, byline, captions with source, no tracking links, and a `Group <N> – <title>.docx` whose text contains the title, all takeaways and all captions (a title-only check passed on a stale export and was strengthened; the previous export fails the new check). `docs/INFOQ-SUBMISSION.md` carries the rules, the pandoc command and the four items only a person can do; CLAUDE.md points at it. Article at 2,989 words excluding code.

- 2026-09-07 — **The InfoQ article's dial section put on ONE basis, the first run.** Figure 2 was the first run (230 records), the table beneath it three-run means (233 records, F1 0.405 against the figure's 0.397), and the two paragraphs after it — the 88/116 ceiling lines and "128 fewer errors, 49 fewer correct answers, 177 more records" — were the first run again; a reader comparing the figure's row to the table's found different numbers for the same rule. The figure's per-row `ships N · F1 x` annotation, the part that duplicated the table, is gone; it keeps only the withheld count and how many of those were correct, and its title says "first run". The table now carries the first run's rows (`rerun-cadec-d0`'s `shipping_rules`), which moved two prose figures: shipping everything is 0.38 not 0.39, and strict voting gives up eight points of F1 not six (d0's 0.315 against 0.397); "within six points" replaces "about five" for the loose check (5.7). Chosen over means because every count in the article is per-run (the layer table gives triples) and a figure of integer bars cannot show a mean. `tests/test_infoq_figs.py` now reads the article's table against the report's first draw, so the two cannot drift apart again, and forbids the annotation. Word export regenerated.

- 2026-09-08 — **Wejdan's author bio added to the InfoQ article; the word budget stops at `## About the authors`.** The body was at 2,985 words, so two 75-word bios could never fit a count that included them. InfoQ's budget is for the article and shows bios as author metadata beside it, so the budget test now cuts at the section heading (a second test asserts the cut is real), and a bio-length test (50–100 words each, two bios) is `xfail(strict)` until Pushpdeep's bio arrives. Pronouns she/her. The bio names USP as the affiliation — confirm that is wanted before submission.

- 2026-09-08 — **The InfoQ CADEC article rewritten as one story, on the reviewers' 27 comments.** The Word file came back from both authors with the same verdict — hard to follow, re-read sections to understand them, repeats itself, not one smooth story — and 27 anchored comments. Every one is addressed in `docs/article-infoq-CADEC.md`, same numbers throughout (no number changed; three were added from tracked sources, below). **Structure now**: why we tested (the layers as a validation step; CADEC chosen because it is already annotated, so verdicts can be scored) → the system under test → first measure the noise → what each layer bought → one dial → where the model loses → does it hold beyond CADEC → why no test saw it → Monday → open questions. **Title changed** (comment 0) to *Testing Six LLM Reliability Layers: What Each Bought and What It Cost*; the Word file is renamed with it. **Tables**: the dial table is DELETED (two comments: "very hard to read, delete it, highlight what matters in the text") — Figure 2 stays and the prose under it carries the first run's rows, now pinned to the report by `test_the_article_dial_prose_is_the_first_run_like_the_figure_above_it` (replacing the table test); the per-layer table and the per-layer paragraphs, flagged as duplicates of each other, are one bulleted list. **Figures** (fonts too small, add dark colours for print): the two Graphviz tables run 2.5 pt larger with `#5a6b73 → #3a464c` and `#946c00 → #7a5800`; the chart 2–3 pt larger, wider canvas. Figure 3 was read by a reviewer as a comparison between models ("add the model name, add the original model"); it is one model's funnel and now says so in its header (`gpt-oss:20b`, pinned by `test_the_funnel_figure_names_the_model_it_measures`) with a `stage` column head. **Added from tracked sources**: the three-run agreement table from `rerun/cadec.json`'s `consensus` block (233 mentions: 164 all agree, 22 same span different code, 1, 10, 15, 21) answering "how many of the 226 agreed"; the five-model determinism result (four families byte-identical over three draws, gpt-oss:20b the only MoE and the only one that varied, 2026-08-30) answering "why are the multi-model results nowhere"; the reviewers' pasted README section "Five more corpora" rewritten as the section "Does it hold beyond CADEC?" from the README's matrix summary and the 2026-09-08 PsyTAR full-ladder entry (in prose, no table). The reviewers' second pasted section, "The three checks", is one sentence with the stagecheck link. **Cut**: the reranker/`set()` bootstrap story (comment: confusing, not needed), the long sample post, the "not shippable" paragraph (comment: do not dwell on low accuracy, it is not the article's focus), the division-of-labour paragraph (the closing paragraph already says it), one open question (the synonym scoring). The reviewer's lesson on disagreement — a mention coded differently on identical input marks where the task is underspecified for the model — is in the noise section, with the honest note that we did not follow it up. **The question "should F1 be a percentage?" is left for the owner**: kept as 0–1 because CONORM's 0.70 and the literature report it that way; the table row now says "(0 to 1)". 2,989 words excluding code (limit 3,000), takeaways 129 (limit 130); `tests/test_infoq_article.py` and `tests/test_infoq_figs.py` green; `docs/INFOQ-SUBMISSION.md` carries the new pandoc command, the new heading map and three rules from this round.
- 2026-09-08 — **Section 1 of the InfoQ article rewritten on the owner's review, and the held-out tier table was WRONG: it counted 22 unanswered records as correct.** The owner asked (a) for a hook and a stated theory, (b) whether the CONORM comparison's "our 0.20" was right ("we have more than 0.2?"), (c) that the layers be framed as evaluating and flagging output rather than as ship / do-not-ship decisions, and (d) that any comparison with the supervised paper use the bare model, everything it returned. **(b) was right and found a defect.** The 0.20 was the held-out F1 of what ships AFTER abstention (0.204, 242 of 314 withheld), set against CONORM's end-to-end 0.7245, which the 2026-09-03 verification had already shown to be LENIENT matching — so the comparison was wrong on both axes. Replaced with the bare model's own numbers from the tracked report, `rerun/cadec.json` `rung0`: overlap 0.474 / 0.474 / 0.504, exact 0.393 / 0.393 / 0.434, stated as development-split. **The tier table (314 / 127 / 0.40; 72 / 60 / 0.83; 242 / 67 / 0.28) is corrected and moved.** Its 127 was derived from the Phase F ledger's `err_per_100` 59.6 (187 errors over 314 records), but the extractor answered only 292 of the 314 (coverage 0.930 after rung 3), so 314 − 187 counted the 22 unanswered records as correct. The Phase F entry's own figures are 105 exact-correct on the stack, 60 of them in the 72 ACCEPT records and 45 in the 242 withheld — 60 + 45 = 105. So the withheld tier held 45 right answers, not 67, and "two-in-five odds into five in six and two in seven" is now "one in three into five in six and one in five". The paragraph lives at the end of "One dial", worded as flagged / not flagged, with the ACCEPT tier's accuracy unchanged at 0.83. Section 1 is now "The theory we set out to test": the hook (80 percent right and cannot say which), the theory (each layer catches what the ones below missed, so accuracy rises per layer), the result in one sentence, CADEC as the instrument rather than the problem, and the honest comparison. 2992 words excluding code; tests green; Word file regenerated.
- 2026-09-08 — **Unproven field claims cut from the InfoQ article's opening, on the owner's review.** "Nobody had run all of them", "the common answer is", "the theory is rarely stated", "most teams have no 875 annotated files", "most of what an organisation knows is prose" and "withholding always raises accuracy" were assertions about the field or about arithmetic that the article neither cites nor measures (the last is false in general: withholding at random leaves accuracy where it was). Each is now a statement about what we did or measured: "we tested six layers", "the theory we set out to test is the one that justifies stacking them", "we ran all six on the same records", "it needs those 875 annotated files", and the hook is our own measurement — 40 percent right, full confidence on two thirds of answers (rung 0's 1.0 on 66 percent). Rule for the file: a sentence about what practitioners generally do, or what nobody has done, needs a citation or goes.
- 2026-09-08 — **Figure 1 of the InfoQ article is a flowchart now, and it replaces the worked example.** Owner asked whether a table was the best form for the pipeline, then whether the flowchart could replace the walkthrough prose (the sample post, the five-line menu, the pick and the lookup). It can: the flowchart (`docs/figures/infoq-fig6-pipeline-flow.dot`, new; `fig7-pipeline-cadec.dot` untouched because `article-v3-CADEC.md` still uses it) carries the same example end to end with the payload on each arrow — the whole post, a quote with no code, a 20-line menu of names with no codes, a line number, and the code on the last arrow only — which shows the claim the table's footer had to state. Four facts the figure does not carry stay as one short paragraph: denied reactions are extracted, the retriever's 227,554 rows, the `null` option, and the bar notation. The two code blocks are gone (they never counted toward the budget); body 2936 words. Rendered by `make_infoq_figs.py` alongside the others; README table updated.
- 2026-09-08 — **Figure 1 redrawn horizontally: the flow and the example in one picture.** The owner found the vertical flowchart right in content but wanted the flow across the page with the example shown at every step, and rejected an icon-heavy generated graphic as "fun but crowded and not professional". The figure is now five cards left to right (INPUT, 1·FIND, RETRIEVE, 2·PICK, RESOLVE), each with a header (who acts, what it does) and a body showing the example's payload at that step: the post excerpt; three quotes including the denied one; the menu's lines 0–4 and 19 for "bit drowsy"; the pick, 0; the code 271782001 |Drowsy| with "the first time a code exists". Built as one Graphviz HTML table so the card widths are fixed in points (130 / 120 / 170 / 85 / 125) and the whole figure is 2,118 px at 200 dpi, which is what keeps the type legible once Word scales it to the page — the first attempt at 380-pt cards came out 5,877 px wide and unreadable. Same file name, `infoq-fig6-pipeline-flow.dot`, so nothing else moved; caption updated to say the cards carry the example.
- 2026-09-08 — **The three-variant table (recall the code / name the concept / pick from a menu) is a chart now, Figure 2, read from the tracked reports.** Owner: "also hard to read" — three numbers per cell, metrics as rows. `make_infoq_figs.py` draws `infoq-fig7-variants.png`: one dot per cold run of rung-0 span-exact F1 from `rerun/cadec-s0.json`, `cadec-s1.json` and `cadec.json`, a median bar and the range beside each group, and beneath each variant its tokens per run (`tokens_in + tokens_out` of the rung-0 aggregate) and unparseable replies (`parse_failed`). TDD: `test_the_variants_figure_reads_the_three_step_reports` written first and seen failing; it pins 0.031/0.030/0.024, 0.252/0.253/0.276, 0.393/0.393/0.434, the 3-of-40 failures every S0 run, and the token bands, and refuses the table's return. Later figures renumbered (shipped set → Figure 3, funnel → Figure 4). The paragraph under it is unchanged. The article now has four figures and two tables (the agreement table and none other of numbers per cell). Body 2,920 words.
- 2026-09-08 — **Three-run figures in the InfoQ article's prose are ranges now, not triples and not averages.** Owner asked whether to average the three runs. Recommended against: two runs are byte-identical, so a mean (0.393 / 0.393 / 0.434 → 0.406) is a number no run produced and buries the spread the noise section exists to show. Triples were the raw data in prose and hard to read. Applied as ranges: ACCEPT 76 to 82 percent, BAND 27 to 30, self-correction two or three firings, voting net −1 to +1 destroying 2 to 5, 177 to 187 to a person, 230 to 238 records. Per-run values remain where a run is the subject (the diverging third run, the one changed answer on the first run) and as dots in Figure 2. Rule recorded in docs/INFOQ-SUBMISSION.md.
- 2026-09-08 — **Voting and the judge reframed as diagnostics, on the owner's suggestion.** Owner: maybe they help identify where the agent is inconsistent, for the developer to fix. The evidence supports it — unanimous samples ship at 0.47 against 0.38 for everything, the menu-shown judge separates 3.4–4.2× and its `best: null` verdict is the one signal that tells a failed menu from a failed pick — so a paragraph after the deletion replay says: diagnosis, not repair; a split vote marks an answer the model cannot reproduce, "not on this list" marks a failed menu; read offline both are lists of where to fix the prompt or the retriever, like the noise section's disagreements; we did not use them that way and should have. No number changed; 2,99x words.
- 2026-09-08 — **Refusal is described as a guard, the person bullet carries no count, and the deletion replay is one measured sentence.** Owner's three notes on the layers section. (1) Refusal's job is protection, so its bullet says so: only ACCEPT passes, the rest is held back for a person; the 21–23 percent / 0.74–0.82 setting stays as what was measured. (2) The person bullet no longer gives 177–187 records: the article is not asking the reader to pick a setting, and the reviewer's decisions are also training data for a later model — the count stays in the dial section, where it is cost accounting. (3) The replay sentence was "not very accurate, it is more complicated": the |Pain| → |Increased pain| mechanism is dropped and only the measurement remains — with the paid layers removed and refusal replayed over the same records, the passing set differed by one record on the first run and none on the others (the spine arm). Kept rather than deleted because takeaway 2 rests on it. Body 2993 words.
- 2026-09-08 — **The dial section's prose carries three relationships instead of restating Figure 3's rows.** Owner: "focus on what we learned and the relationships, not the numbers from the figure". Rewritten as (1) any verdict raises accuracy because withholding is a layer's only lever and no rule ships more than the extractor found, so accuracy flatters any guard and yield is reported beside it; (2) the price differs by verdict — ACCEPT is the most precise and the most expensive (128 errors for 49 right answers and 177 reviews, because it can endorse only the 32 percent of mentions whose wording matches the vocabulary), the menu-shown judge is informed rather than lexical and the one rule beating ship-everything on F1; (3) verdicts do not stack, which is why removing the paid layers changed one record. Held-out result reduced to the odds. `test_the_article_dial_prose_is_the_first_run_like_the_figure_above_it` now pins the five numbers that anchor those lessons (88, 128, 49, 177, 0.420 vs 0.397) instead of nine row values; changed first, seen failing. Duplicated sentences cut elsewhere to stay at 2999 words.
- 2026-09-08 — **"The verdicts nest rather than add" — the third relationship in the dial section, from a three-set overlap the owner asked to see as a Venn diagram first.** Computed over `rerun-cadec-d0` (230 records; ACCEPT from the final zone, unanimous vote from `r3_votes` with all k samples, judge pass from the `-judgemenu` arm's `r4_verdict`; correctness at rung 4, span-exact): ACCEPT 53 (39 right), unanimous 108 (51), judge pass 136 (74). Regions — all three 29 (25 right); ACCEPT+judge only 23 (14); vote+judge only 49 (21); judge only 35 (14); vote only 30 (5); ACCEPT only 1 (0); ACCEPT+vote only 0; none 63 (9). So 52 of the 53 ACCEPT records are also judge passes: the free check nests inside the judge, agreement of all three is the strongest region (six in seven right) and "vote only" is worse than "none". The paragraph states the lesson without the counts at the owner's request; the counts live here. **Scratch computation** (`out/` archive on the main checkout, not the tracked report): before submission the overlap should be added to `ladder.analysis` and the tracked report with a test, like `shipping_rules`. Body 2997 words.
- 2026-09-08 — **"Where the model loses" halved, kept.** Owner asked whether the section matters; it carries takeaway 5 and the ladder's ceiling, so it stays, but it walked Figure 4's rows and listed both specialist-model results in full. Now: once found, retrieval and pick succeed (93 / 81 percent); the loss is at finding; the reasons are not medical (three examples); two domain-adapted models did not help, a worked example and the denied-reactions rule did; the ceiling is detection, 0.521 on held-out. The encoder's 87.0 → 88.4 menu recall and the generator's 4 of 40 documents are in the 2026-08-31 / 2026-09-01 entries and no longer in the article. Body 2909 words.
- 2026-09-08 — **Figure 4 is a funnel chart drawn from the report, and the hand-written table it replaces carried a stale split.** Owner: "figure 4 is not clear". `make_infoq_figs.py` now draws `infoq-fig8-funnel.png` from `draws["rerun-cadec-d0"].budget`: four bars (226 annotated → 116 quoted on the exact span → 108 with the right concept on the menu → 87 right line picked), the loss between each pair labelled, bars coloured by who acts. The finding loss's split is derived from the same block — never touched = `overlap.missed`, wrong boundary = `overlap.matched − exact.matched` — and comes out **48 / 62**, where `infoq-fig3-funnel.dot` (and the article prose until this morning) said **43 / 67**, typed in from an earlier analysis under a different pairing. The report's numbers are the ones that reproduce; the dot file stays only for the README's record. TDD: `test_the_funnel_chart_reads_the_error_budget_from_the_report` written first, seen failing; it pins 226/116/108/87, 110 = 48 + 62, and refuses the old file in the article. Body 2910 words.
- 2026-09-08 — **Future work stated in the article: a second reader to lift the detection ceiling.** Owner's suggestion, asking the second model to identify what the extractor missed. Added after the ceiling paragraph with the guard that it is a second reader, not a judge: a different family shown the post and the first model's quotes, its additions proposals needing the same measurement as the extractor's. Not built. Evidence it is worth trying is indirect — on FiNER, three other families answered the document gpt-oss refused (2026-08-30) — and union recall across families on CADEC has never been measured. Body 2999 words.
- 2026-09-08 — **"Does it hold beyond CADEC?" is a scatter chart and four sentences, and the matrix has FOUR model families, not five.** Owner: wordy, too many numbers, could a figure make it comparable. `make_infoq_figs.py` now draws `infoq-fig9-matrix.png` from the tracked `matrix.csv` (28 cells: bc5cdr, finer, geo, lgl, linnaeus, psytar, trnews × gpt-oss:20b, granite4:micro-h, llama3.1:8b, mistral:7b-instruct): x = how often the ACCEPT lane fires (`occupancy`), y = how often it is right (`correctness`), coloured by vocabulary family, CADEC's three draws hollow from `rerun/cadec.json` `lanes` (ACCEPT n / final n = 21–23 percent, correct 75.5–82.4). The picture is the finding: clinical vocabularies cluster top-left (small lane, right), gazetteers bottom-right (large lane, wrong), seven cells at the origin (FiNER on every model, LINNAEUS on three of four) where the lane never fired. **Discrepancy found and fixed in the article**: the README's five-corpora table and the article said the ranges were across five families including qwen3:8b; `matrix.csv` and `scripts/score_matrix.py`'s own notes say qwen3 produced no scorable cell, so the section and the chart say four. The README still says five and should be corrected in its own pass. The section's number ranges (7–31 / 80–100, 20–74 / 10–26, 0.6–0.9× vs 2.0–3.8×) are gone from the prose; the chart carries them. TDD: `test_the_matrix_chart_reads_the_tracked_matrix_and_the_cadec_report` first, seen failing. Body 2974 words.
- 2026-09-08 — **"Does it hold beyond CADEC?" now answers its own question, lesson by lesson.** Owner: the figure alone did not answer it; the goal is whether the findings and lessons are the same on different data. Rewritten as five bullets, one per takeaway: (1) layers sort, do not fix — held, PsyTAR full ladder ×3, zero changes; (2) the free check — held only where the vocabulary's names are evidence, Figure 5; (3) read, never remember — not re-tested, every corpus ran the menu design; (4) repeat runs — held, variation is the model's: four families byte-identical, FiNER's one instability the refused document on one run of three; (5) study the data — held but where the model loses moved: FiNER found two thirds of mentions and mis-coded them (detection 0.685 × coding 0.446, 2026-08-30), largest error the slot-0 habit (19.5 percent of answers). Lead: four of five held; where the model loses did not. The open question on the loose vocabulary match is cut — nothing in the article introduces it any more; it stays in the 2026-09-03 entries. Body 2984 words.
- 2026-09-08 — **"Does it hold beyond CADEC?" is about agreement only.** Owner: "I don't care about their performance numbers, I care whether they agree or disagree with our lessons — what held and what changed." Every number is out of the bullets; each says agrees / not re-tested / agrees with a condition / agrees with a change, and what the change is (the free check reverses on gazetteers and is silent on tag names; where the model loses moved from reading on CADEC to coding on FiNER, with a positional habit as its worst error). Figure 5 stays under the bullets as the evidence for the free-check lesson. Body 2996 words.
- 2026-09-08 — **Figure 5 removed from the article.** Owner's call: the section is about whether each lesson agrees on other data, not performance, so the matrix chart goes. It stays in `make_infoq_figs.py` and the README as the repository's picture of `matrix.csv`; the test now asserts the article does not carry it. Four figures in the article. Body 2952 words.
- 2026-09-08 — **"Does it hold beyond CADEC?" checks two findings beyond the takeaways, and states where the evidence stops.** Owner asked whether only the takeaways should be compared. Added: self-correction's idleness, sharpened on PsyTAR (the check wrongly rejects right gold codes there, so rung 2 had work, and still fired zero times because rung 1 never REJECTs model output — 2026-09-08 PsyTAR entry); and the "recorded once" lesson, repeated by the matrix (twelve cells with the wrong prompt, five with the wrong model, caught by the copied manifest — 2026-09-07 entries). One sentence marks the boundary: the judge, verdict nesting and the diagnostic use of split votes were measured on CADEC only. Body 2999 words.
- 2026-09-08 — **"Why none of it showed up in a test" cut to its two unique claims and retitled "What the tests could not see".** Owner asked whether it repeated. Its first paragraph (unread verdicts) was said twice already and is now one clause; the metric defect (0.204 → 0.204) and the "recorded once" lesson stay, since the Monday practices and the cross-corpus section rest on them. Body 2973 words.
- 2026-09-08 — **"What we could not settle" folded away.** Its two bullets were not repeats but orphans: the unexplained divergence now closes the noise section's model paragraph, and the caveat on the small judge and CADEC's likely presence in training data joins the caveats in "The theory we set out to test". The closing paragraph follows "What to do on Monday" directly. Body 2963 words.
- 2026-09-08 — **"What to do on Monday" sharpened to six practices, each with the measurement that earned it.** Owner asked whether it needed sharpening. The first practice had lost its body in an earlier cut; two lessons the article argues for had no practice — never ask the model for an identifier (S0's 13–18 percent invented codes, ten times worse) and treat disagreement as a to-do list (split votes, the judge's "not on this list", repeat-run disagreements) — and the readers-of-fields and metric-blindness practices are merged into one. Sentences the practices now duplicate were cut from the noise, losses and cross-corpus sections. Body 2998 words.
- 2026-09-08 — **One host in the article.** Owner asked whether to cite GitLab or GitHub, and whether the checks sentence should carry a path. The article cites one address, github.com/wbagais/reliability-ladder (public, in sync with the GitLab `origin` at da84389 today); the stagecheck link is reached from the README. The checks sentence names gatecheck, crosscheck and stagecheck and points at "the repository below", no path and no second host. Upload checklist gains: push the final commit to both remotes. Body 2999 words.
- 2026-09-08 — **Final read-through of the InfoQ article.** Rules (budget, takeaways, captions, ranges, one link, no field claims) all met. One unsupported number found and fixed: the opening's "40 percent right" was the held-out table's corrected-away row (127 of 314); it now says "about one answer in three", the held-out 105 of 314 and within a rounding of the dev 88 of 230. Joins smoothed: Figure 2's three alternatives are named in the sentence that introduces it; the retriever sentence under Figure 1 reads as a sentence; the divergence clause in the noise section no longer says "since"; the cross-corpus free-check bullet no longer says "agrees" three times; the training-data caveat is two clean sentences. Cut to pay: "an abstention option reduces fabrication", "the question the layers exist to answer", the 73-of-226 count. A first attempt committed at 3,008 words with the budget test red because the shell chain did not stop on the failed count; amended. Body 2999 words, takeaways 130.
- 2026-09-08 — **Pushpdeep's bio added; the bios test is live.** Supplied by the owner in the first person; rewritten in the third person to the house pattern (role first, then scope, then values), 84 words, without pronouns since none were given. `test_two_author_bios_of_about_75_words` loses its strict xfail and passes on both bios. The Word file is regenerated. The upload checklist's open items are now the generative-AI disclosure and the DOI check only.
- 2026-09-08 — **Pushpdeep's bio takes he/him**, on his word via the owner. 85 words, same content.
- 2026-09-08 — **Title: "Reliability Layers Don't Make an LLM Right. They Tell You Which Answers to Trust."** Owner wanted a more interesting title than the descriptive one; chose this from four offered. It is takeaway 1 as a headline — a claim about what the layers do, not (as the first title the owner rejected) what they fail at. Word file renamed with it, the trailing full stop dropped from the file name.
- 2026-09-08 — **Three trims for headroom, at the owner's request after a whole-article read.** The CONORM paragraph to two sentences; the layers section's bold lead to one sentence (it restated its bullets); the two orphan sentences under Figure 1 merged into one. No claim or number lost. Body 2978 words, leaving room for the editor.
- 2026-09-08 — **Figure 1 in two rows of three cards**, at the owner's request for readability at page width: INPUT → FIND → RETRIEVE, a bent arrow carrying the menu down, PICK → RESOLVE → OUTPUT (the output is its own card now). Drawn as Graphviz nodes with `rank=same` rows and `splines=ortho`, not the single HTML table, so the connector is a real edge. Caption says "read left to right and then down".
- 2026-09-09 — **Dashboard MR !22 (`claude/dashboard-m1`, opened 2026-08-27) brought up to main — 281 commits behind, two append-only conflicts (`requirements.txt`, this file), and THREE things main changed under it.** (a) `runsindex.discover_runs` read `rerun-cadec-d0.r3.records.jsonl` as a run called `rerun-cadec-d0.r3`: the per-rung artifacts every run writes since 2026-09-03 (plan item 12) end in `.records.jsonl` too, so the b2-menu archive listed 219 "runs" of which 150 were rung snapshots. Now `classify()` matches `<run>.r<N>.(records|calls).jsonl` FIRST and files them under the run as `r<N>.records` / `r<N>.calls`, with `state` and `aggregates` beside them. The Phase E `phaseE-r6-*.r6.ledger.jsonl` ablate stems still list as their own entries — that was decided 2026-08-26 and a `.r6.ledger` is an ablate run, not a snapshot. (b) The TRACKED `runs/archive/` (the consolidated re-run's corpus-free four per run, since 2026-09-07) was not a source, so a fresh clone opened an empty dashboard while the article's own runs sat in the tree; it is the second source now, marked archived. (c) R5's prompt drill-down reconstructed every prompt through the rung builders and looked the hash up in `.llm_cache` — the only option in August. A run now writes every call's full prompt and raw reply to `<run>.r<N>.calls.jsonl`, so `llm_view.traced_calls` reads those first (`source: "call_trace"`, status `traced`, per-record rungs narrowed to the calls carrying the record's span text) and reconstruction is the fallback for older runs, labelled `source: "reconstruction"`. Rung 4's prompt has had three menu modes since 2026-09-03 and the reconstruction only knows the blind one, which is one more reason the trace comes first. Four new tests; the M1 golden test on phaseF-test-1 still pins 0.204 [0.150–0.260].
- 2026-09-09 — **Dashboard: a LIVE RUN tab (`dashboard/live.py`, `/api/live/*`) — one document through the REAL rungs, in-process, into a scratch directory that is deleted.** The plan page's "Ladder demo" pane is a static picture over synthetic posts; the owner asked for the same picture produced by the actual code hitting the actual model. Decided: the tab calls `ladder.run.run_ladder` itself — the pipeline's driver, every rung's own `apply`, the model resolved by `llm.for_rung` from the manifest, the same `.llm_cache` — over `{doc_id: text}` for a pasted text or a dev/pool document, up to a chosen rung, with `out_dir` a `tempfile.mkdtemp` that is read back (state rows, call traces, ledger, aggregates — the files a real run writes, read the same way) and removed in `finally`. Nothing is re-implemented, which is the governing rule, and nothing becomes a run: no `out/` write, no results.csv, no entry in the runs list, and the payload carries a `live_single_document` caveat on every render because one document is an example, not a measurement. Refusals: the test split (C2 — a live look at a held-out document is still a look, 403 with the word "spent"), empty or ambiguous input, an unknown rung, and a second run while one is in flight (409, spec R2's one-live-run rule). A run that dies (`SystemExit` from `check_models`, a rung raising) reports the error on the job instead of hanging the tab. It is the ONE POST route; the C2 test now pins the set of non-GET routes to exactly that one, and the no-child-process test still holds — this is an import. It is NOT the R2 launcher (which runs `python -m ladder.run` over a split into `out/` and stays M3). CI-safe tests (17): scripted extractor and judge bound through the same `for_rung` seam, the nine-concept registry, rung 0's real bare path with `offsets: search`, rungs 1/2/4/5/6 untouched, rung 3 DISABLED in the test manifest so the payload has to carry a recorded disabled state. The corpus test scores each rung's row against gold: rung 1's row reads `incorrect` on the fake code and rung 2's `correct` after the correction, credited to the rung that made it.
- 2026-09-09 — **Dashboard tab bar: `shipped` is the flag the frontend reads; `milestone` is a label.** Until now the JS treated `milestone !== "M1"` as "placeholder", so the first M3 feature to land would have rendered as a placeholder. Health reports `milestone: "M1+live"`, `writes_files: false`, `live_run: true`.
- 2026-09-09 — **Live run, round 2 (owner's review): the view is PER KEYWORD, with rung 0 shown station by station and, for a corpus document, the diff against gold.** (a) Every highlighted span in the text and a chip strip below it select ONE record; the cards, the calls and the diff table narrow to it (rung 0 and rung 3 calls are per document and stay, flagged "whole post"; rung 2 and rung 4 calls are narrowed by the record's text AND the code it held at that rung, because a judge prompt embeds the whole post and the text alone matched every record). (b) `live.rung0_path` rebuilds Figure 1's stations for one record — INPUT → FIND → RETRIEVE → PICK → RESOLVE → TRIM → OUTPUT — from what rung 0 RECORDED on it (`checks`) and the two calls it made, never re-derived: the find-reply entry it came from (span text as the model wrote it, context, denied), the retrieved menu with the chosen line, the pick reply's choice for that reaction, and the trim from `span_untrimmed`. A pick the model never made reads as a FALLBACK with the B4 finding beside it (the smoke text's "knee" → |knee gives way| was exactly that: `no_pick`, `pick_fallback: gap`, line 0 by rule); S0/mode-A stations the step has no equivalent for are `not_in_step`, hatched, never faked as done. The stations read the record AS RUNG 0 LEFT IT (its state row), not the final record — found on the first real run, where rung 5 had withheld the code and RESOLVE said "no code". (c) `live.gold_diff` pairs every gold mention with its prediction through the scorer's own `_pair` — exact span keys first, overlap over the remainder — so the diff cannot disagree with `score_run`; found exact / found overlap / missed / spurious, and a code status that reads the answer the system HAS (`withheld_correct` when rung 5 withheld a right code, never "no code"). The text is coloured by agreement (agrees / differs / spurious / missed) instead of blue-over-green. On ARTHROTEC.139: "Lower Back Pain" found exact, right code, withheld; "pain" found by overlap as "intense daily pain", wrong code, withheld — both routed to a person. Eight new tests (26 in the file).
- 2026-09-09 — **Live run, round 3 (owner's review): the RUNG is the view's time, and gold and model share one cell.** Two asks — "gold and the model's code in one cell, two lines, two colours" and "smoother to move from rung to rung" — and one design answers both: a rung rail at the top (r0…r6 as nodes with a one-line summary from the state table: records, changed, calls, zone counts; prev/next; ← → keys) selects the rung, and everything below is the run AS THAT RUNG LEFT IT — the text's highlights from each record's state row at that rung, the comparison against gold recomputed from the run's own per-rung snapshot (`live.gold_diff_by_rung`, through the scorer's `_pair`; a disabled rung left no snapshot, so the previous set stands), each record's card ("at rung N: created / changed code X → Y, zone A → B / unchanged / dropped", the verdict that rung wrote, its outcome at that rung, a dot per rung coloured by zone that also moves the rail), and the cost-and-calls panel of that rung alone. Rung 0's stations appear at rung 0 and are a link elsewhere. The scrolling between a record's table and a rung's panel — the "movement" that was not smooth — is gone: one rung, one screen. (b) The comparison table has three columns: mention (gold over model, the words only one side has underlined in that side's colour, "= same span" when exact), code (gold code with its VOCABULARY label through `Registry.label` over the model's code with the model's own label, "withheld" and "= same code" badges), and the status at that rung. A record's card header uses the same two stacked lines. Stepping ARTHROTEC.139: at rung 1 "pain" reads found-overlap / wrong / NEW, at rung 5 wrong-withheld / ABSTAIN, at rung 6 ESCALATE — the same fact, watched move. Three new tests (29 in the file).
- 2026-09-09 — **Dashboard redesign, step 1 of 5: the Data tab, from the owner's sketch (the Workbench Redesign Sketch artifact, section 3).** Decided with the owner across the sketch's 24 revisions: three tabs (Data · Results · Live), never a page that mixes batch results with a live run; one run picker for the app; the placeholders out of the bar; the rungs drawn ONCE per page; one colour, one meaning (blue = selected, green = gold or agreement with gold, violet = the model's own output, red = wrong or rejected, amber = held for a person, grey = BAND, dashed = absent or mechanical); simple, clean, minimal. The Data tab is the first landed: a row of filters with the document picker among them; ONE summary line (split, documents, reaction mentions, drugs, gold through rung 1 as a zone bar with the backend named); then EITHER the document — the annotated text, a card on hover with the code and its vocabulary label (`Registry.label`, the desk's rule), the mention kind, excluded with reason, discontinuous segments lit together, click to pin, "send to Live" — OR the table, filtered by split and drug, with an "in run" column: the run's final zones on the document and its pairing against the document's gold (exact · overlap · missed · model only) through `dashboard.live.gold_diff`, i.e. the scorer's own `_pair`, so sorting by "missed" finds the interesting documents (TDD'd, three tests). Corpus totals, splits and exclusions moved behind a "reference" disclosure. The colour tokens changed dashboard-wide to the rule above: ABSTAIN and ESCALATE share the amber of "held", BAND is grey, NEW is a dashed outline, predictions are violet, gold is green. The Data tab defaults its run to the newest run on THIS corpus's splits — a matrix cell from another corpus is newer on disk and read as "everything missed" on the first try.
- 2026-09-09 — **Dashboard redesign, step 2 of 5: the Live grid, from the sketch's sections 4-5.** The Live tab is now: the controls; the text coloured by agreement with gold as the selected column left it; rung 0 AS A TABLE against gold — one row per mention from either side in text order (paired, model only, gold only), mention gold over model with the words only one side has underlined, the retrieved menu with the model's pick in the model's colour and a line filled by the fallback dashed, code gold over model with the vocabulary label, and three cost rows per level (find · retrieve/pick · resolve); then ONE grid — rows are keywords, columns are rungs 1-4 in their own words (r1 one word with the BAND reason; r2 "skipped" or was → now; r3 the k outputs with counts and the share that agree; r4 pass/fail, confidence and the judge's sentence) and a "person" column — the share of six shipping rules that would hold the record, drawn as six vertical tokens (V strict vocabulary ACCEPT · V+ loose · 3 all votes agree · 2 two of three · J blind judge · J+ menu-shown judge; filled = holds, outlined = ships, dashed = not run and out of the denominator); three cost rows under the grid; a vertical legend naming the six rules once with the share of keywords each holds, click to show one rule alone. Gone: the rail, the dots, the "at rung" line, the per-rung cost panels, the detail panel, the keyword strip — the rungs appear once per page. Backend, TDD'd: `live.shipping_rules` reads the six from what the rungs recorded (V+ is a `contained` lexical match computed beside the strict one, because a run checks one mode); `live.menu_judge_pass` runs rung 4 a SECOND time with `menu: ranked` on copies of the records into its own trace file so J+ has an answer — a batch run judges blind and cannot answer it. Measured on ARTHROTEC.139: the blind judge passes both records, the menu-shown judge holds both, so J+ reads 100% held against J 0%. Four new tests; 104 dashboard tests.
- 2026-09-09 — **Dashboard redesign, step 3 of 5: the Results drill-down — a batch run's document through the SAME grid as Live, from the run's own artifacts.** `dashboard/document_view.run_document_payload` builds the Live payload's shape for one document of a run: the final records, the state table's rows for the document (timeline), the per-rung call traces filtered to it (rung 0's find and pick for the menu, cost per level), the per-rung record snapshots for the diff against gold at every rung, the ledger rows for cost, the aggregates for disabled rungs — through the same helpers (`rung0_path`, `gold_diff`, `shipping_rules`), nothing re-run, nothing re-derived. ONE renderer (`drawGrid(root, res, st)`) draws both the Live result and Results' new section 7 ("a document through the ladder", a document picker over `/api/run/docs`). Runs before the state table show one row per record, the final state, with a `no_state_table` caveat; J+ (the menu-shown judge) is the live run's own second pass and reads "not run" for a batch run. The rung 6 count is what the ledger says the person received, not the record count. FOUND ON THE WAY, and fixed test-first: `AppState.registry()` handed ONE sqlite connection to every FastAPI worker thread, and the moment two requests overlapped (the Results page now fires several) sqlite raised `InterfaceError: bad parameter or other API misuse` as a 500 on unrelated routes (`/api/corpus/stats`, `/api/run/docs`). The registry is opened once per thread now; the first version of the test passed by a race (both threads saw the unset slot and each built its own), so it opens the registry on the main thread first — red on the committed code, green on the fix. Three new tests plus the thread test; 108 dashboard tests. Walkthrough and Traceability stay until the top-bar step, when the tab list is cut to three.
- 2026-09-09 — **Dashboard redesign, step 4 of 5: the top bar — three tabs, one run strip, the placeholders under "later", Walkthrough and Traceability RETIRED.** The bar is three words, Data · Results · Live, with an underline for the active one and no pill anywhere; the four planned pages (Rung workbench, Run monitor, Desk, Demo) sit under one small "later" menu at the far right instead of four dead tabs. Beneath the bar, ONE run strip read by Data and Results and hidden on Live (which has a text, not a run): the run picker, the inferred split and archived flag, the span mode, the backend and manifest hash from the run's own provenance, and "compare with" (the former baseline). The per-tab run, baseline and span selects are gone; the app defaults to the newest run on this corpus's splits that carries records. Walkthrough and Traceability are removed as tabs and as code: what they drew — a record's rung timeline, its checks, the prompts and raw replies — is now Results' section 7, the document through the Live grid, reached from the picker or from a ribbon: a ribbon's "documents" link lists the subset's documents by record count and one click opens the document in section 7. Prompts and raw replies are deliberately off the page (owner's call, round 3 of the sketch); they remain in the run's call files. Verified in the browser on the archived base run: BAND ribbon → 176 records in 35 documents → LIPITOR.460 → the grid with its 15 records. Follow-up noted, not done: the run picker lists every run on the machine (250 on this one, most matrix cells with an unknown split) — it should group or filter by corpus.
- 2026-09-09 — **Dashboard redesign, step 5 of 5: Results — the flow from the LEDGER, rungs 0-4 lane by lane, and the six shipping rules over the batch as the end of the flow (Figure 3 as a table).** (a) `dependencies.verdict_flow_from_ledger` builds the rung-1-verdict × disposition crosstab from the ledger's one row per record per rung (rung 1's word, moved to the zone rung 2 left when it rescued the record; rung 4's verdict; the last zone), so a tracked, corpus-free copy of a run draws the same flow as the copy with records — pinned on the base run: ACCEPT 53 all settled (r4 pass 39 / fail 14), BAND 176 all queued (96 / 73 / 7 unparsed), REJECT 1 queued. What only a record knows (whether rung 3 CHANGED its code) is stated unknown, never zero. `dependencies_payload` carries `flow_source: records | ledger`. (b) Section 4 of Results is now rungs 0 to 4 as columns with the three lanes carried across, from the run; the vote column reads all / 2 of 3 / none / no vote per lane from the records (ACCEPT 55/28/2/15%, BAND 45/35/8/13%) and voted / tie / no vote from the ledger alone, saying which. (c) Section 6 is the six-rule table with a proportion bar per rule — ships-right, ships-wrong, ships (right/wrong unknown), to a person — `dependencies.rules_over_run`: on the base run's records V 53 ship (39 right) / 177 person 77%, V+ 93 (39 right) / 137 60%, 3 108 (25 right) / 122 53%, 2 203 (36 right) / 27 12%, J 136 (29 right) / 94 41%, J+ not run (the live run's own second pass); from the ledger alone V, 2 and J answer and V+, 3, J+ read NOT RUN with the reason — a rule half-answerable (V+ ships on ACCEPT without the contained check) is not run at all. The run's own rule is bold; the right count is cross-checked against the run's score and flow (53 shipped, 39 right). The V2 "records' journey" chart and the "what a person still has to do" block are gone — the rule table is that section. FOUND ON THE WAY: cutting two dead render functions by "up to the next function" swallowed the two constants between them (the Live example text and the rung names), which broke every control wired after `wireLive` — caught only in the browser, since the frontend has no tests; the repo's own rule about untested code applies to `app.js` too. Four new tests; 113 dashboard tests. Follow-ups noted, not done: the run picker should group by corpus; a frontend smoke test (load the page, no console error) would have caught the swallowed constants.
- 2026-09-10 — **`docs/plan.html` v19: the published page rewritten around the consolidated re-run, and what the owner rejected on the way.** v18 had quoted a superseded run since 2026-08-28 (21% shipped at 4.0 errors per 100, 196 of 248 routed, a five-model ACCEPT lane from an earlier sweep) from a hand-typed chart array nothing checked — the "recorded once" defect on the front page. Now one JSON block (`plan-data`) feeds every table and figure and `tests/test_plan_html.py` compares it cell by cell to `runs/archive/consolidated-2026-09-03/`, `matrix.csv` and the PsyTAR full-ladder cell, forbids the superseded strings and requires the current ones. **The demo was invented posts with measured numbers pasted beside them**; the owner asked whether the examples were real and run through the code. They were not. `scripts/plan_demo.py` now follows real records of `rerun-cadec-d0` and `rerun-finer-d0` through the archived raw run (state rows, per-rung records, the `-judgemenu` arm), one example for every path a record can take, and its `--documents` mode calls the Workbench's own document builder (`dashboard.document_view.run_document_payload`) so the page draws a document exactly as the Live tab does — rung 0 against gold, the grid of records by rungs 1–4, the six shipping rules. **Licence handling, decided:** a CADEC post is drawn as a redacted paragraph — one blank block per word (offsets only), the quoted spans filled in, each within the tracked report's seven-word precedent; FiNER's excerpt is shown (CC-BY-SA); PsyTAR from the published stripped records with no text and no answer key; a leak check refuses any 24-character window of the post outside an allowed span or a vocabulary label; the judge's prose stays on the machine. The owner asked twice for the full post; the answer is `scripts/plan_local.py`, a copy written under `out/` only, red-bannered, refusing any other path. **Three flow designs were rejected in a row**: falling dots (confusing), a bar per rung (not meaningful), an animated Sankey with moving dots and a replay ("the dots have no meaning; graphs exist to simplify"). What stands: four still figures, each stating one finding above it — a dataflow diagram per dataset with routes-not-taken dashed at 0, the right-answer line against tokens per rung, the shipping rules as accuracy vs yield with numbered dots, the cross-corpus lane chart — every count from tracked files and recomputed by the test. **The Triage desk is removed** (a simulated review session that has never been timed) and the header knobs with it. **Two defects the cross-checks caught:** a rescued record's rung 1 verdict read BAND from the final record's checks where the rung 1 state row says REJECT (rung 2 re-validates; the trace now reads the rung 1 row), and a CSS class shared with the header logo squeezed the demo's legend chips into 19-pixel circles. **A finding for the Workbench's owner:** on a clean clone its Results headline shows the archived first draw as 0 shipped / F1 0.000, because the tracked archive has no records file; the page reads `results.csv` and shows 53. Merged as !44; `main` mirrored to GitHub the same day.

- 2026-09-10 — **Four stale branches merged into main on the owner's instruction ("merge them all"), one integration branch, conflicts resolved in main's favour wherever main had already recorded the same thing.** `claude/infoq-cadec-word-count-5e5c45` contributed the CADEC reference's page range (73–81) and CONORM's preprint title in `docs/article-infoq-CADEC.md`, plus the `~$*` Office-lock ignore; its regenerated `docs/article-infoq-CADEC.docx` was dropped because main had renamed the export to the Group 3 filename on 2026-09-08. `claude/phaseC-models` and `claude/frosty-aryabhata-f01ae0` were superseded — their decisions lines already exist on main verbatim (2026-08-25 Phase C setup; 2026-09-07 `vocabulary.gate` on the arms) — and only `tests/test_arm_gate.py` (eight assertions, green on main's manifests) was kept from the second. `claude/reliability-ladder-b4-slot0-7784eb` (B4, three commits, 226 behind) brought `rung0_menu_order: "shuffle"`, `ladder.menuorder.permuted`, `_menu_seed`, two arm manifests, two test files, its four 2026-09-01 decisions entries (inserted above in date order), and the longform build report's correction; `docs/article-v3.md` kept main's text on all three hunks because main had already carried the correction into the rewritten article. **The three FiNER arm manifests were regenerated from the current `manifest.finer.json`**, not taken from the branch: its copies predated the gate codes, the judge menu keys and tau's retirement, and an arm manifest that drifts from its base is two experiments in one name. `tests/test_manifest_gate.py` lists the two new arms. No run moves — the shuffle arm ships off, `rung0_pick_fallback` is declared at the value the code already used, and `manifest.json` stays undeclared with a test saying so.
- 2026-09-10 — **The public page's CADEC documents are SYNTHETIC STAND-INS now, not grey blocks.** The owner, shown the redacted view three times, asked for "a copy of the data that is similar to the original" where the original cannot be shared. What ships (`scripts/plan_synth.py`, `tests/test_plan_synth.py`, TDD'd): for each CADEC demo document, the annotators' spans and the model's quotes — exactly what the page already published under the seven-word precedent — stay at the word positions the post had, and every other slot is filled from a bank of first-person drug-review phrases that contains no symptom or body words, so filler can never read as a missed annotation. The generator takes the reduced document (positions and spans; the post is not in it) and is a pure function of the run and document id through blake2b; `build_documents` attaches it before the existing leak check, so the stand-in is still tested against 24-character windows of the real post. The page labels it "a synthetic stand-in" in the header, greys the invented words, and points at the local copy; `tests/test_plan_html.py` requires the `synthetic` mark on every CADEC document and checks each span's offsets against its text. **What was NOT done, and why:** a paraphrase of the real post by a model. That would read better and would be a derivative of non-transferable text; the stand-in shares only the spans, which the tracked report already publishes. PsyTAR documents keep the span map — nothing of theirs is quotable on this machine — and FiNER-139 keeps its excerpt. The local copy (`scripts/plan_local.py`) is unchanged: real posts, under `out/`, never committed. Also this session, on the owner's ask: the plan page joins the README's top table beside the article.
